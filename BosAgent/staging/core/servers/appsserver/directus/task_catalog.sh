#!/usr/bin/env bash
# task_catalog.sh — Directus CMS task handlers
# Ficha · Order · SBOS-049 §4.3
# R16: no hardcoded paths. All paths from env vars with defaults.

set -euo pipefail

export SBOS_FICHA_DIR="${SBOS_FICHA_DIR:-/etc/bos/blibs/servers/appsserver/directus}"
...
ficha_install() {
    ...
    sbos_k8s_core "${SBOS_FICHA_DIR}/directus.k8s.yml" "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready deployment/directus-0 -n "${SBOS_NAMESPACE}" --timeout=600s
...
ficha_update() {
    ...
    kubectl rollout restart deployment directus -n "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready deployment/directus-0 -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_uninstall() {
    ...
    kubectl delete deployment directus -n "${SBOS_NAMESPACE}" --ignore-not-found
