...
## Regla de Completitud de Capas (2026-06-17)

**Ningún átomo de Capa N+1 puede ejecutarse si la Capa N no está completa.**

Esta regla surgió del incidente M2: se marcaron átomos M2 como completados cuando solo
1 de 8 fichas del stack estaba operativa. Esto llevó a intentar avanzar a M3 (Context Plane)
sin tener PostgreSQL, Redis, Vault, Keycloak ni Kong funcionando — un error grave que
compromete la integridad del sistema.

### Verificación de capa completada

```
¿Capa N completa?
   │
   ├── NO → ejecutar átomos de Capa N (12.B)
   │
   └── SÍ → ¿Capa N+1 tiene dependencias explícitas de Capa N?
       │
       ├── SÍ → OK para avanzar
       └── NO → revisar REGISTRO-ESTADO matriz de capas
```

### Criterios de completitud por capa

| Capa | Criterio | Verificación |
|------|----------|-------------|
| **Capa 0** (OS) | bos-preflight HEALTHY + bosagent creado + certificado TLS | `systemctl status bos.service` → active |
| **Capa 1** (K8s) | k3s/kubeadm Ready + kubectl funcional + StorageClass | `kubectl get nodes` → Ready |
| **Capa 2** (Datos) | PG Running + Redis Running + Minio Running | `kubectl get pods -n sbos-data` → 3/3 Running |
| **Capa 3** (Id/GW) | Vault unsealed + Keycloak HEALTHY + Kong HEALTHY | `bosctl bootstrap verify --only=C-06,C-07,C-08` → ✅ |
| **Capa 4** (Notif) | sbos-notifier HEALTHY | `kubectl get pods -n sbos-notify` → Running |

### Consecuencia de violar esta regla

- M3 (Context Plane) no puede funcionar sin Redis DB1 (cache de ctx_id)
- M4 (JSON-RPC certificado) no puede certificarse sin Keycloak (auth)
- M5 (Autómata de capacidad) no tiene métricas sin el stack corriendo
