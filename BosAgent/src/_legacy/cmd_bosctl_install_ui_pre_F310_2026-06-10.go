...
// ── Colores y Estilos — aliases → internal/tui/styles ─────────────────────
// Eliminados cuando cada pantalla se migre a internal/tui/screens/ (F3.8).

const (
	cGreenS  = styles.HexGreen
	cCyanS   = styles.HexCyan
	cYellowS = styles.HexYellow
	cRedS    = styles.HexRed
	cDimS    = styles.HexDim
	cBlackS  = styles.HexBlack
	cWhiteS  = styles.HexWhite
	cSlateS  = styles.HexSlate
	cMutedS  = styles.HexMuted
	cBg2S    = styles.HexBg2
	cBg3S    = styles.HexBg3
)

var (
	cGreen  = styles.ColorGreen
	cCyan   = styles.ColorCyan
	cYellow = styles.ColorYellow
	cRed    = styles.ColorRed
	cDim    = styles.ColorDim
	cBlack  = styles.ColorBlack
	cWhite  = styles.ColorWhite
	cSlate  = styles.ColorSlate
	cMuted  = styles.ColorMuted
	cBg2    = styles.ColorBg2
	cBg3    = styles.ColorBg3
)
...
var (
	...
	sLabel         = styles.Label
	sLabelA        = styles.LabelActive
)

// Iconos — aliases → internal/tui/styles
func icOk() string   { return styles.IconOK() }
func icRun() string  { return styles.IconRun() }
func icPend() string { return styles.IconPending() }
func icErr() string  { return styles.IconErr() }
func icWarn() string { return styles.IconWarn() }
func icBos() string  { return styles.IconBos() }

func badge(text string, fg, bg lipgloss.Color) string { return styles.Badge(text, fg, bg) }
func renderMFARow(enabled bool, focused bool) string  { return styles.RenderMFAToggle(enabled, focused) }

// ── Tipos de estado — aliases → internal/tui/model ────────────────────────

type stepStatus = tuimodel.StepStatus
type fichaStatus = tuimodel.FichaStatus
type logLevel = tuimodel.LogLevel

const (
	sPending = tuimodel.StepPending
	sActive  = tuimodel.StepActive
	sDone    = tuimodel.StepDone
	sFailed  = tuimodel.StepFailed
	sSkipped = tuimodel.StepSkipped
)

const (
	fPending = tuimodel.FichaPending
	fActive  = tuimodel.FichaActive
	fDone    = tuimodel.FichaDone
	fFailed  = tuimodel.FichaFailed
)
...
const (
	logInfo  = tuimodel.LogInfo
	logOK    = tuimodel.LogOK
	logStep  = tuimodel.LogStep
	logError = tuimodel.LogError
	logWarn  = tuimodel.LogWarn
)
...
// ── Pantallas — aliases → internal/tui/model ──────────────────────────────

type Screen = tuimodel.Screen
...
const (
	...
	ScreenDashboard   = tuimodel.ScreenDashboard
	ScreenLogs        = tuimodel.ScreenLogs
	ScreenShutdown    = tuimodel.ScreenShutdown
	ScreenGoodbye     = tuimodel.ScreenGoodbye
)

// stepID y step* — aliases para el stepper del wizard
type stepID = tuimodel.StepID

const (
	stepWelcome    = tuimodel.StepWelcome
	stepTenant     = tuimodel.StepTenant
	stepAdmin      = tuimodel.StepAdmin
	stepConfirm    = tuimodel.StepConfirm
	stepInstalling = tuimodel.StepInstalling
	stepComplete   = tuimodel.StepComplete
)

// ── Mensajes tea ──────────────────────────────────────────────────────────

// wsEventMsg mantiene campos privados para compatibilidad con 200+ referencias.
// El struct exportado WsEventMsg vive en internal/tui/model.
// ts se asigna en awaitWS() al leer del canal — fuera de Update() (corrige P3).
type wsEventMsg struct {
	...
	ts        time.Time // capturado al consumir del canal, no en el handler
}

// wsReadyMsg, wsErrorMsg, sysInfoMsg — tipos locales (campos privados).
// Versiones exportadas en internal/tui/model/events.go (para uso de screens/).
...
// ── Tick y animaciones — aliases → internal/tui/model ─────────────────────

type tickMsg = tuimodel.TickMsg

var tickCmd = tuimodel.TickCmd
...
func connectWS(ch chan wsEventMsg) tea.Cmd {
	return func() tea.Msg {
		...
		// Un solo error → return inmediato sin reintentos (la conexión queda inválida).
		go func() {
...
func awaitWS(ch chan wsEventMsg) tea.Cmd {
	...
	// time.Now() se captura aquí (en el Cmd, fuera de Update) — corrige P3.
	return func() tea.Msg {
		select {
		case msg := <-ch:
			if msg.ts.IsZero() {
...
func (m *model) handleWS(ev wsEventMsg) tea.Cmd {
	// ev.ts fue capturado en awaitWS() fuera de Update() — P3 cumplido.
	now := ev.ts
	if now.IsZero() {
		now = time.Now() // fallback para eventos sin timestamp (demo inicial)
	}
	var progCmd tea.Cmd
...
// startDemoCmd lanza runDemo como goroutine dentro de un tea.Cmd — corrige P3:
// el goroutine se inicia en el contexto del Cmd (fuera de Update), no en Update directamente.
func startDemoCmd(ch chan wsEventMsg) tea.Cmd {
	return func() tea.Msg {
		go runDemo(ch)
		return nil
	}
...
func (m *model) startBootstrap() tea.Cmd {
	m.step = ScreenInstalling
	m.setScreen(ScreenInstalling)
	now := time.Now() // en startBootstrap: llamado desde un keyMsg handler — P3 aceptable
	m.startTime = now
	m.addLog(logEntry{ts: now, level: logInfo, msg: "🚀 SBOS Bootstrap iniciado"})
	m.addLog(logEntry{ts: now, level: logInfo,
		msg: "   Empresa: " + m.tenantInputs[0].Value() +
			"  ·  Dominio: " + m.tenantInputs[3].Value()})
	m.addLog(logEntry{ts: now, level: logInfo, msg: "   22 fichas  ·  7 niveles del DAG"})

	if demoMode {
		// startDemoCmd lanza el goroutine en el Cmd — no go directo en Update (corrige P3)
		return tea.Batch(startDemoCmd(m.wsCh), awaitWS(m.wsCh))
	}
...
// vScrollbar — alias → tuimodel.VScrollbar (F3.7)
func vScrollbar(vp viewport.Model, h int) string { return tuimodel.VScrollbar(vp, h) }

// hScrollbar — alias → tuimodel.HScrollbar (F3.7)
func hScrollbar(vp viewport.Model, w int) string { return tuimodel.HScrollbar(vp, w) }

// vpDims — wrapper → tuimodel.VpDims(m.width, m.height) (F3.7)
func (m model) vpDims() (wA, wB, wC, h int) {
	return tuimodel.VpDims(m.width, m.height)
}
...
// runCmd ejecuta un comando externo y retorna stdout (stderr descartado). Usado para detección de sysinfo.
...
// runDemo inicia la simulación de instalación usando demo.RunDemo.
// Convierte entre tuimodel.WsEventMsg (exportado) y el wsEventMsg local.
func runDemo(ch chan wsEventMsg) {
	phases := defaultPhases()
	// Convertir installPhase local → tuimodel.InstallPhase
	tPhases := make([]tuimodel.InstallPhase, len(phases))
	for i, ph := range phases {
		tPhases[i] = tuimodel.InstallPhase{Nombre: ph.nombre, Fichas: ph.fichas}
	}
	bridgeCh := make(chan tuimodel.WsEventMsg, 256)
	go demo.RunDemo(bridgeCh, tPhases)
	for msg := range bridgeCh {
		ch <- wsEventMsg{
			...
			msg:       msg.Msg,
			total:     msg.Total,
			errDetail: msg.ErrDetail,
			data:      msg.Data,
		}
	}
}

// cmdInstallUI es el punto de entrada de "bosctl setup" — modo interactivo, unattended o demo.
...
// runInteractiveTUI inicializa el modelo BubbleTea y arranca la TUI de instalación.
// Abre /dev/tty si stdin no es un TTY (tmux-safe). Señaliza stopCh al salir.
...
// versionSuffixPlain retorna " <versión>" de una ficha sin formato de color (para modo unattended).
...
// fmtMsg retorna ": <msg>" si msg no está vacío, cadena vacía en caso contrario.
