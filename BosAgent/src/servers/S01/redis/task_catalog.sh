...
_log()   { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [redis] $*" | tee -a "$FICHA_LOG"; }
_k() {
    local kubectl_bin
        ...
        - name: redis
          ...
          volumeMounts:
            ...
            - name: config
              ...
              subPath: redis.conf
            # TLS eliminado (ADR-040: mínimo viable, 2da pasada con Vault)
      volumes:
        - name: data
...
min-replicas-max-lag 10" --dry-run=client -o yaml 2>/dev/null | _k apply -f - >> "$FICHA_LOG" 2>&1
    ...
    # ── 8. Context Registry DB1 (F10.C.6 — tenant saga) ────────────
	    echo "${__STEP_START__} configurar_context_registry"
	    _redis SELECT 1 2>/dev/null || true
	    _redis CONFIG SET databases 16 2>/dev/null || true
	    _redis ACL SETUSER bos_context on ">${redis_pass}" +@all ~* 2>/dev/null || \
	        _log "ACL bos_context: ya existe — continuando"
	    _redis ACL SETUSER kong_context on ">${redis_pass}" +@read +select ~* 2>/dev/null || \
	        _log "ACL kong_context: ya existe — continuando"
	    _log "Context Registry DB1 configurado (bos=rw, kong=ro)"
	    echo "${__STEP_OK__} configurar_context_registry"

	    _log "redis instalado (Iteración 3: Sentinel HA + Context Registry DB1)"
    return 0
}
