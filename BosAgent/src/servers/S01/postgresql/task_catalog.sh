...
_log()     { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [postgresql] $*" | tee -a "$FICHA_LOG"; }
_k() {
    local kubectl_bin
...
ficha_pre_install() {
    ...
    # ADR-040: K8s usa dynamic provisioning (PV se crea solo con PVC)
...
spec:
  serviceName: etcd
  replicas: 1  # ADR-040: PASADA 1 (1 réplica), PASADA 2: 3 réplicas
  selector:
    matchLabels:
      app: etcd
  template:
      ...
      containers:
        - name: etcd
          ...
          command:
            ...
            - --initial-cluster=etcd-0=http://etcd-0.etcd.${NS}.svc.cluster.local:2380
