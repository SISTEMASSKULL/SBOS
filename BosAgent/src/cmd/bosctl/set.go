...
const envPath = paths.EtcBos + "/.env"
...
// cmdSet es el dispatcher del subcomando "bosctl set" (apikey).
...
// cmdSetApikey escribe la API key de un modelo IA en /etc/bos/.env (modo 0600).
// NUNCA registrar la key en logs — solo confirmar el nombre del modelo configurado.
...
// Crea el archivo con modo 0600 si no existe.
func updateEnvFile(envVar, value string) error {
	...
	if err := os.MkdirAll(strings.TrimSuffix(envPath, "/.env"), 0755); err != nil {
		...
		_ = os.MkdirAll(paths.EtcBos, 0755)
	}
