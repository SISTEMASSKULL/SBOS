...
// cmdRelease es el dispatcher del SKULL Release Plane (check | list | channel).
...
// cmdReleaseCheck verifica si hay actualizaciones disponibles para una ficha (P5 — pull-only).
...
// cmdReleaseList lista todos los releases en caché del release manager.
...
// cmdReleaseChannel cambia el canal de release activo (stable|early|canary — aplica tras SIGHUP).
