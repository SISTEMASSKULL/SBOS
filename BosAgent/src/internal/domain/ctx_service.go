...
//
// Thread safety: inmutable — todos los métodos son puros (sin estado compartido mutable).
type CtxService struct{}

// NewCtxService crea el servicio de contexto.
//
// Callers conocidos:
//   - cmd/bos/main.go:runNormal — inyectado en Server.
...
//
// Recibe:
//   - p.TenantID: string — requerido, identifica el tenant del contexto.
//   - p.TTL: time.Duration — TTL del ctx; 0 → 30min por defecto.
//
// Retorna: (*CtxID, error). Error si TenantID está vacío (ErrTenantIDRequired).
//
// Callers conocidos:
//   - internal/server/jsonrpc.go:rpcCtxCreate — vía JSON-RPC bos.ctx.create.
//
// Efectos secundarios: ninguno — genera IDs deterministas sin I/O.
//
// Estándares: W3C Trace Context spec, SBOS-049, RFC 7519.
...
//
// Retorna: (*ValidateResult, nil) siempre; error solo si traceParent está vacío.
//
// Callers conocidos:
//   - internal/server/jsonrpc.go:rpcCtxValidate — vía JSON-RPC bos.ctx.validate.
//
// Efectos secundarios: ninguno.
//
// Estándares: W3C Trace Context spec, SBOS-049.
