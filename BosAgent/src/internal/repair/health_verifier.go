...
// HealthVerifyResult contiene los resultados de verificación de salud post-reparación.
...
// HealthCheck representa una verificación de salud individual post-reparación.
...
// HealthVerifier ejecuta verificación de salud post-reparación en las tres capas del SBOS.
//
// Thread safety: seguro para uso concurrente — stateless salvo logger.
type HealthVerifier struct {
	logger *slog.Logger
}

// NewHealthVerifier crea un verificador de salud post-reparación.
func NewHealthVerifier(logger *slog.Logger) *HealthVerifier {
	return &HealthVerifier{logger: logger}
}

// VerifyAll ejecuta la suite completa de verificaciones post-reparación (Ubuntu + K8s).
//
// Retorna: *HealthVerifyResult con lista de checks individuales y resultado global.
//
// Callers conocidos:
//   - internal/repair/repair_manager.go:RepairManager.runPhaseVerify — Fase 4 de la reparación.
//
// Efectos secundarios:
//   - Ejecuta: dpkg --audit, systemctl is-active, df, kubectl get nodes/pods.
//   - Los checks K8s se omiten si kubectl no está disponible (no fatal).
