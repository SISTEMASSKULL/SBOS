...
-- ══════════════════════════════════════════════════════════════════════
-- Motor de Validación de Valores — RuleEngine
-- Las reglas son DATOS. El RuleEngine en Rust las carga y evalúa.
-- Extensible sin recompilar. Cada regla tiene trazabilidad a su estándar.
-- ══════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS bauth.cfg_validation_rule (
    ...
    is_active       BOOLEAN     DEFAULT true,
    tenant_id       UUID        REFERENCES bauth.idn_tenant(tenant_id) ON DELETE CASCADE,
    ctx_id          TEXT        DEFAULT 'system',
    created_at      TIMESTAMPTZ DEFAULT now(),
    updated_at      TIMESTAMPTZ DEFAULT now()
);
COMMENT ON TABLE bauth.cfg_validation_rule IS '[RuleEngine] Reglas de validación de valores. TYPE, RANGE, ENUM, SEMANTIC. Cada regla define límites y valores permitidos para un campo. Sin regla = sin validación.';
COMMENT ON COLUMN bauth.cfg_validation_rule.category IS 'TYPE (tipo de dato), RANGE (rango numérico), ENUM (valores permitidos), SEMANTIC (regla de negocio con regex).';
COMMENT ON COLUMN bauth.cfg_validation_rule.provenance_url IS 'URL de la fuente normativa (NIST, ISO, PCI, RFC).';
COMMENT ON COLUMN bauth.cfg_validation_rule.tenant_id IS 'NULL = regla global. Con valor = tenant-specific override.';
CREATE INDEX IF NOT EXISTS idx_valrule_target ON bauth.cfg_validation_rule(target_table, target_column);
CREATE INDEX IF NOT EXISTS idx_valrule_domain ON bauth.cfg_validation_rule(domain);
CREATE INDEX IF NOT EXISTS idx_valrule_active ON bauth.cfg_validation_rule(is_active) WHERE is_active = true;

CREATE TABLE IF NOT EXISTS bauth.cfg_validation_log (
    log_id          UUID        PRIMARY KEY DEFAULT uuidv7(),
    rule_id         UUID        REFERENCES bauth.cfg_validation_rule(rule_id),
    ...
    expected_rule   JSONB,
    severity        TEXT        DEFAULT 'error',
    evaluated_by    TEXT        DEFAULT 'RuleEngine',
    ctx_id          TEXT        NOT NULL,
    created_at      TIMESTAMPTZ DEFAULT now()
);
COMMENT ON TABLE bauth.cfg_validation_log IS '[RuleEngine] Registro WORM de validaciones fallidas. Alimenta Panel 5 (Compliance Dashboard).';
REVOKE UPDATE, DELETE ON bauth.cfg_validation_log FROM PUBLIC;
CREATE INDEX IF NOT EXISTS idx_vallog_rule ON bauth.cfg_validation_log(rule_id, created_at);
CREATE INDEX IF NOT EXISTS idx_vallog_table ON bauth.cfg_validation_log(table_name, column_name);
...
CREATE TABLE IF NOT EXISTS bauth.idn_role_template (
    ...
    rol_bitmask_base64    TEXT        NOT NULL DEFAULT '',
...
COMMENT ON COLUMN bauth.idn_user_template.rol_bitmask_base64 IS 'Rol BitMask N-bit one-hot en base64. OR de todos los átomos de los roles del usuario vía privilege_role_atom.';
...
-- ══════════════════════════════════════════════════════════════════════
-- BIBLIOTECA UNIFICADA DE POLÍTICAS Y CONFIGURACIONES
-- Tabla: bauth.cfg_policy_library · Función: bauth.jsonb_explode
-- ══════════════════════════════════════════════════════════════════════
-- [desenredado ORQUESTA-050 — lo carga inicializar_sbos_db.sh en orden, no escondido aquí]
-- \ir DDL_framework_unified.sql

-- ══════════════════════════════════════════════════════════════════════
-- BOS IAM INSTALLER — Control Plane Tables
-- Schema: bos · 7 tablas + ENUMs + índices
-- ══════════════════════════════════════════════════════════════════════
-- [desenredado ORQUESTA-050 — lo carga inicializar_sbos_db.sh en orden, no escondido aquí]
-- \ir DDL_bos_schema.sql
