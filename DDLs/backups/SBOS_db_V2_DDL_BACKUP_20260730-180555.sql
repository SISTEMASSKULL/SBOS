...
CREATE EXTENSION IF NOT EXISTS btree_gist;   -- requerido por WITHOUT OVERLAPS (PG18) en T-152
...
COMMENT ON COLUMN bauth.idn_tenant_calendar_assignment.is_inherited IS 'true = asignación heredada del nivel superior (no directa).';
...
CREATE TABLE IF NOT EXISTS bauth.idn_roles_rol_lifecycle_event (
    ...
    actor_id         UUID             REFERENCES bauth.idn_identidad_entidad(entidad_id) ON DELETE SET NULL,
    reason           TEXT,
...
BEGIN
    ...
    LOOP
        ...
        IF v_updated > 0 THEN
            ...
            rol_id           := v_row.id;
            estado_previo    := v_row.status;
            fecha_expiracion := v_row.vu;
            procesado_en     := now();
            RETURN NEXT;
        END IF;
    END LOOP;
...
ALTER TABLE bauth.idn_roles_rol_hierarchical
    ...
    ADD COLUMN IF NOT EXISTS approved_by     UUID REFERENCES bauth.idn_identidad_entidad(entidad_id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS bauth.idn_roles_ver_b01_audit_log (
    ...
    CONSTRAINT fk_irvb01al_changed   FOREIGN KEY (changed_by)  REFERENCES bauth.idn_identidad_entidad(entidad_id)  ON DELETE SET NULL,
    CONSTRAINT fk_irvb01al_approved  FOREIGN KEY (approved_by) REFERENCES bauth.idn_identidad_entidad(entidad_id)  ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS bauth.idn_roles_ver_b03_approval_queue (
    ...
    CONSTRAINT fk_irvb03aq_proposed_by FOREIGN KEY (proposed_by)  REFERENCES bauth.idn_identidad_entidad(entidad_id) ON DELETE RESTRICT,
    CONSTRAINT fk_irvb03aq_resolved_by FOREIGN KEY (resolved_by)  REFERENCES bauth.idn_identidad_entidad(entidad_id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS bauth.idn_roles_ver_contract_revision_log (
    ...
    CONSTRAINT fk_irvcrl_approved   FOREIGN KEY (approved_by) REFERENCES bauth.idn_identidad_entidad(entidad_id) ON DELETE RESTRICT,
...
-- ======================================================================
-- T-161b — bauth.idn_roles_template_tipo_nodo
...
-- ======================================================================
CREATE TABLE IF NOT EXISTS bauth.idn_roles_template_tipo_nodo (
...
COMMENT ON TABLE bauth.idn_roles_template_tipo_nodo IS
...
INSERT INTO bauth.idn_roles_template_tipo_nodo (
...
-- Tipos de nodo (FK a idn_roles_template_tipo_nodo): tenant|dominio|bloque|objeto|lista|
...
CREATE TABLE IF NOT EXISTS bauth.idn_roles_template (
    ...
    -- tipo: FK a idn_roles_template_tipo_nodo (catálogo T-161b). Reemplaza el CHECK chk_irt_tipo.
    tipo           TEXT        NOT NULL REFERENCES bauth.idn_roles_template_tipo_nodo(codigo) ON UPDATE CASCADE,
...
COMMENT ON COLUMN bauth.idn_roles_template.tipo          IS '[FK→idn_roles_template_tipo_nodo] Tipos: tenant|dominio|bloque|objeto|lista|set_politicas|politica|regla|atomo|atributo|enum|evento (G-11). diagnostico solo en Dart.';
...
-- ======================================================================
-- T-158 — bauth.idn_identidad_atributo_history
-- Historial WORM de cambios en idn_identidad_atributo. Particionado por mes.
-- Hash-chain por (entidad_id, attr_namespace, attr_key): detecta manipulación retroactiva.
...
CREATE TABLE IF NOT EXISTS bauth.idn_identidad_atributo_history (
    ...
    atributo_id     UUID        NOT NULL
                    REFERENCES bauth.idn_identidad_atributo(atributo_id) ON DELETE RESTRICT,
    ...
    changed_by      UUID        NOT NULL
                    REFERENCES bauth.idn_identidad_entidad(entidad_id) ON DELETE RESTRICT,
    ...
    -- prev_hash: SHA-256 de la fila anterior para (entidad_id, attr_namespace, attr_key)
    -- NULL en la primera fila de esa clave — marca inicio de cadena
    prev_hash       TEXT        NULL,

    -- row_hash: SHA-256(history_id || atributo_id || attr_key || attr_value_new::text || changed_at::text || COALESCE(prev_hash,''))
    ...
    CONSTRAINT idn_identidad_atributo_history_pkey
...
CREATE TABLE IF NOT EXISTS bauth.idn_identidad_atributo_history_2026_09
    PARTITION OF bauth.idn_identidad_atributo_history
    FOR VALUES FROM ('2026-09-01') TO ('2026-10-01');

CREATE TABLE IF NOT EXISTS bauth.idn_identidad_atributo_history_2026_10
    PARTITION OF bauth.idn_identidad_atributo_history
    FOR VALUES FROM ('2026-10-01') TO ('2026-11-01');

CREATE TABLE IF NOT EXISTS bauth.idn_identidad_atributo_history_2026_11
    PARTITION OF bauth.idn_identidad_atributo_history
    FOR VALUES FROM ('2026-11-01') TO ('2026-12-01');

CREATE TABLE IF NOT EXISTS bauth.idn_identidad_atributo_history_2026_12
    PARTITION OF bauth.idn_identidad_atributo_history
    FOR VALUES FROM ('2026-12-01') TO ('2027-01-01');

-- ─── Índices (heredados por todas las particiones) ────────────────────
CREATE INDEX IF NOT EXISTS idx_iah_atributo
    ON bauth.idn_identidad_atributo_history(atributo_id, changed_at DESC);

CREATE INDEX IF NOT EXISTS idx_iah_entidad_key
    ON bauth.idn_identidad_atributo_history(entidad_id, attr_namespace, attr_key, changed_at DESC);

CREATE INDEX IF NOT EXISTS idx_iah_changed_by
    ON bauth.idn_identidad_atributo_history(changed_by, changed_at DESC);

CREATE INDEX IF NOT EXISTS idx_iah_operation
    ON bauth.idn_identidad_atributo_history(operation, changed_at DESC);

-- ─── WORM: revocar UPDATE y DELETE ───────────────────────────────────
REVOKE UPDATE, DELETE ON bauth.idn_identidad_atributo_history FROM bauth_app_role;
...
COMMENT ON TABLE bauth.idn_identidad_atributo_history IS
  '[ISO 27001:2022 A.8.15] [PCI DSS 4.0.1 Req 10.3.2] [GDPR Art. 30] [NIST AU-9] [A.65.02 T-158] [D00-B05]
   Historial WORM append-only de cada cambio en idn_identidad_atributo. Particionado por mes.
   Hash-chain por (entidad_id, attr_namespace, attr_key): encadena cada fila a la anterior.
   row_hash = SHA-256(history_id||atributo_id||attr_key||attr_value_new||changed_at||COALESCE(prev_hash,'''')).
...
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.atributo_id    IS 'FK RESTRICT a idn_identidad_atributo. El historial persiste aunque el atributo se elimine logicamente.';
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.attr_value_old IS 'NULL en INSERT (sin valor previo). Copia del valor anterior para forensia sin JOIN a T-157.';
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.attr_value_new IS 'Valor nuevo del atributo despues del cambio. Siempre presente.';
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.operation      IS 'INSERT=nuevo atributo · UPDATE=modificacion de valor · SOFT_DELETE=is_active puesto a false.';
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.prev_hash      IS '[NIST AU-9] Hash SHA-256 de la fila anterior de esta misma clave. NULL = primera fila de la cadena.';
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.row_hash       IS '[NIST AU-9] SHA-256 del contenido de esta fila. Verificado por bauth.fn_verify_attr_hash_chain(atributo_id).';
COMMENT ON COLUMN bauth.idn_identidad_atributo_history.changed_at     IS 'Timestamp de la modificacion. Columna de particionado — inmutable post-INSERT.';

-- ======================================================================
-- T-159 — bauth.idn_identidad_requisito
...
CREATE TABLE IF NOT EXISTS bauth.idn_identidad_requisito (
    ...
    error_message       JSONB       NOT NULL
                        DEFAULT '{"es":"Atributo requerido","en":"Required attribute"}',
...
CREATE INDEX IF NOT EXISTS idx_ir_entity_ial
    ON bauth.idn_identidad_requisito(entity_type, ial_level)
    WHERE is_active = true;

CREATE INDEX IF NOT EXISTS idx_ir_tenant
    ON bauth.idn_identidad_requisito(tenant_id, entity_type)
    WHERE tenant_id IS NOT NULL AND is_active = true;

CREATE INDEX IF NOT EXISTS idx_ir_namespace
    ON bauth.idn_identidad_requisito(attr_namespace, attr_key);
...
COMMENT ON COLUMN bauth.idn_identidad_requisito.tenant_id        IS 'NULL = global (aplica a todos los tenants). NOT NULL = override específico del tenant.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.entity_type      IS '[ENUM entidad_nivel] actor es el caso principal; también aplica a bdomain/bsubdomain/pos.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.ial_level         IS '[NIST 800-63A-4] IAL1=autoafirmado · IAL2=verificado remoto · IAL3=verificado presencial.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.must_be_verified  IS '[NIST 800-63A-4 §5] true = atributo.verified=true obligatorio. IAL2+: CI, email, phone deben estar verified.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.accepted_sources  IS '[NIST 800-63A-4 §5.1] Fuentes de verificación aceptadas. IAL3 solo acepta government/biometric.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.validation_regex  IS 'Regex de validación del valor. Ej: CI boliviana → ''^\\d{7,8}$''. NULL = sin validación de formato.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.max_age_days      IS 'Antigüedad máxima del atributo. IAL2: típicamente 365 días. NULL = sin límite de antigüedad.';
COMMENT ON COLUMN bauth.idn_identidad_requisito.error_message     IS 'Mensaje de error bilingüe {es, en} devuelto por el Motor de Identidad al frontend cuando falla la validación.';

-- ─── Seeds base: requisitos globales del sistema ───────────────────────
INSERT INTO bauth.idn_identidad_requisito
...
VALUES
    ...
    (NULL, 'actor', 'IAL1', 'core',    'full_name',    true,  false,
     '{self}',                    NULL,
     '{"es":"Nombre completo requerido","en":"Full name required"}'),

    (NULL, 'actor', 'IAL1', 'contact', 'email',         true,  false,
     '{self}',                    '^[^@]+@[^@]+\.[^@]+$',
     '{"es":"Email requerido","en":"Email required"}'),

    -- IAL2: verificado remotamente — nombre + CI + email verificados
    (NULL, 'actor', 'IAL2', 'core',    'full_name',    true,  true,
     '{document,government}',     NULL,
     '{"es":"Nombre completo verificado requerido (IAL2)","en":"Verified full name required (IAL2)"}'),
    ...
    (NULL, 'actor', 'IAL2', 'contact', 'email',         true,  true,
     '{self,employer}',            '^[^@]+@[^@]+\.[^@]+$',
     '{"es":"Email verificado requerido (IAL2)","en":"Verified email required (IAL2)"}'),

    -- IAL3: verificado presencialmente — CI + biometría obligatoria
    (NULL, 'actor', 'IAL3', 'core',    'full_name',    true,  true,
     '{government}',               NULL,
     '{"es":"Nombre verificado presencialmente requerido (IAL3)","en":"In-person verified name required (IAL3)"}'),
...
-- T-165 — bauth.idn_identidad_proofing
-- [NIST SP 800-63A-4 §4-6] [ISO/IEC 29115:2013] [ISO 24760-2:2025 §7.2] [eIDAS 2.0 Art. 24] [D00-B06]
CREATE TABLE IF NOT EXISTS bauth.idn_identidad_proofing (
    proofing_id     UUID        PRIMARY KEY DEFAULT uuidv7(),
    entidad_id      UUID        NOT NULL REFERENCES bauth.idn_identidad_entidad(entidad_id)
                    ON DELETE CASCADE,
    tenant_id       UUID        NOT NULL REFERENCES bauth.idn_tenant(tenant_id)
                    ON DELETE CASCADE,
    ...
    reviewer_id     UUID        NULL REFERENCES bauth.idn_identidad_entidad(entidad_id)
                    ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS idx_iip_entidad
    ON bauth.idn_identidad_proofing(entidad_id, status);
CREATE INDEX IF NOT EXISTS idx_iip_tenant_status
    ON bauth.idn_identidad_proofing(tenant_id, status, ial_achieved);
CREATE INDEX IF NOT EXISTS idx_iip_reproofing
    ON bauth.idn_identidad_proofing(reproofing_at)
    WHERE status = 'PASSED' AND reproofing_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_iip_expires
    ON bauth.idn_identidad_proofing(expires_at)
    WHERE status = 'PASSED' AND expires_at IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_iip_reviewer
    ON bauth.idn_identidad_proofing(reviewer_id)
    WHERE reviewer_id IS NOT NULL;

COMMENT ON TABLE bauth.idn_identidad_proofing IS
  '[NIST SP 800-63A-4 §4-6] [ISO/IEC 29115:2013] [ISO 24760-2:2025 §7.2] [eIDAS 2.0 Art. 24] [D00-B06]
...
COMMENT ON COLUMN bauth.idn_identidad_proofing.ial_achieved IS '[NIST 800-63A-4] IAL alcanzado en ESTE proofing. Diferente del ial_min requerido.';
COMMENT ON COLUMN bauth.idn_identidad_proofing.proofing_type IS '[NIST 800-63A-4 §5] SELF_ASSERTED=IAL1 · REMOTE=IAL2 · IN_PERSON/TRUSTED_REFEREE=IAL3.';
COMMENT ON COLUMN bauth.idn_identidad_proofing.evidence IS '[NIST 800-63A-4 §5.2] FAIR: doc débil · STRONG: doc oficial · SUPERIOR: biometría+doc.';
COMMENT ON COLUMN bauth.idn_identidad_proofing.reviewer_id IS '[NIST 800-63A-4 §6.3] Obligatorio para IN_PERSON y TRUSTED_REFEREE (IAL3).';
COMMENT ON COLUMN bauth.idn_identidad_proofing.expires_at IS 'IAL2: típicamente 365 días · IAL3: 180-730 días según política del tenant.';
COMMENT ON COLUMN bauth.idn_identidad_proofing.reproofing_at IS 'Cuándo iniciar el re-proofing. Job: WHERE reproofing_at <= NOW() AND status=PASSED → notificar via bNotify.';

-- T-166 — bauth.idn_identidad_consentimiento
...
CREATE INDEX IF NOT EXISTS idx_ic_entidad_active
    ON bauth.idn_identidad_consentimiento(entidad_id, is_active)
    WHERE is_active = true;
...
COMMENT ON COLUMN bauth.idn_identidad_consentimiento.policy_version IS '[GDPR Art. 7.1] Versión de política vigente al momento del consentimiento.';
COMMENT ON COLUMN bauth.idn_identidad_consentimiento.processing_scope IS '[ISO 29184] Alcances: analytics, marketing, third_party_sharing.';
COMMENT ON COLUMN bauth.idn_identidad_consentimiento.legal_basis IS '[GDPR Art. 6] Base legal. CONSENT solo cuando requerido — CONTRACT para empleados.';
COMMENT ON COLUMN bauth.idn_identidad_consentimiento.withdrawn_at IS '[GDPR Art. 7.3] Fecha de retirada. NULL = vigente. Único UPDATE permitido.';
COMMENT ON COLUMN bauth.idn_identidad_consentimiento.is_active IS 'Generado: true = withdrawn_at IS NULL.';

-- T-167 — bauth.idn_identidad_vc
...
CREATE TABLE IF NOT EXISTS bauth.idn_identidad_vc (
    ...
    proofing_id         UUID        NULL REFERENCES bauth.idn_identidad_proofing(proofing_id)
                        ON DELETE SET NULL,
    ctx_id              TEXT        NOT NULL,
    created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at          TIMESTAMPTZ NOT NULL DEFAULT now()
...
CREATE INDEX IF NOT EXISTS idx_ivc_entidad
    ON bauth.idn_identidad_vc(entidad_id, status);
...
COMMENT ON COLUMN bauth.idn_identidad_vc.vc_uri IS '[W3C VCDM 2.0 §4.1] Identificador único. UNIQUE.';
COMMENT ON COLUMN bauth.idn_identidad_vc.vc_type IS '[W3C VCDM 2.0 §4.1] Siempre incluye "VerifiableCredential".';
COMMENT ON COLUMN bauth.idn_identidad_vc.credential_subject IS '[W3C VCDM 2.0 §4.1] Claims sobre el sujeto. No almacenar PII en texto plano.';
COMMENT ON COLUMN bauth.idn_identidad_vc.proof IS '[W3C VCDM 2.0 §4.8] DataIntegrityProof con eddsa-rdfc-2022 (Ed25519) vía Vault.';
COMMENT ON COLUMN bauth.idn_identidad_vc.vc_format IS 'SD_JWT_VC: selective disclosure.';
COMMENT ON COLUMN bauth.idn_identidad_vc.status_list_url IS '[W3C VC Status List 2021] Revocación escalable sin consultar bAuth.';


-- ======================================================================
-- T-500 — bauth.idn_registro_atributo_schema
-- PIP: esquema canónico de atributos de identidad. D98-B01.
-- SCIM 2.0 RFC 7643 §4 · ISO/IEC 24760-1:2019 §5 · NIST SP 800-162 §3.3
-- Permite que idn_identidad_atributo (T-157, D00) sea extensible sin hardcode.
-- GAP-D01-01: clasificacion + display_mask habilitan acceso a nivel de campo.
-- ======================================================================
...
-- ======================================================================
-- T-201 — bauth.idn_acceso_contrato
...
-- WORM parcial: campos de gobernanza inmutables una vez estado != 'BORRADOR'.
-- FK inversa: privilege_atom_grant.contrato_id apunta aquí (no array anti-patrón).
-- ======================================================================
...
CREATE TABLE IF NOT EXISTS bauth.privilege_atom_grant (
    ...
    contrato_id   UUID        NULL REFERENCES bauth.idn_acceso_contrato(contrato_id) ON DELETE SET NULL,
    ctx_id        TEXT        NOT NULL,
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
