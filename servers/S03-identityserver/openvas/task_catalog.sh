#!/usr/bin/env bash
# task_catalog.sh — OpenVAS/Greenbone task handlers
# Ficha · Order · SBOS-049 §4.3
# R16: no hardcoded paths. All paths from env vars with defaults.

set -euo pipefail

export SBOS_FICHA_DIR="${SBOS_FICHA_DIR:-/etc/bos/blibs/servers/identityserver/openvas}"
...
ficha_install() {
    ...
    sbos_k8s_core "${SBOS_FICHA_DIR}/openvas.k8s.yml" "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready deployment/openvas-0 -n "${SBOS_NAMESPACE}" --timeout=600s
...
ficha_update() {
    ...
    kubectl rollout restart deployment openvas -n "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready deployment/openvas-0 -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_uninstall() {
    ...
    kubectl delete deployment openvas -n "${SBOS_NAMESPACE}" --ignore-not-found
