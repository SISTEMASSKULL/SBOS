#!/usr/bin/env bash
# task_catalog.sh — Grafana Alloy 1.7 log collector task handlers
# Ficha · Order 215 · SBOS-049 §4.3 · DaemonSet
# Degraded mode: tolerates missing Loki
# R16: no hardcoded paths. All paths from env vars with defaults.

set -euo pipefail

export SBOS_FICHA_DIR="${SBOS_FICHA_DIR:-/etc/bos/blibs/servers/monitorserver/alloy}"
...
ficha_install() {
    ...
    sbos_k8s_core "${SBOS_FICHA_DIR}/alloy.k8s.yml" "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready daemonset/alloy -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_update() {
    ...
    kubectl rollout restart daemonset alloy -n "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready daemonset/alloy -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_uninstall() {
    ...
    kubectl delete daemonset alloy -n "${SBOS_NAMESPACE}" --ignore-not-found
...
ficha_health() {
    ...
    curl -sf http://localhost:12345/health
