#!/usr/bin/env bash
# task_catalog.sh — Kube State Metrics 2.13 task handlers
# Ficha · Order 202 · SBOS-049 §4.3 · DaemonSet
# Degraded mode: tolerates missing Prometheus
# R16: no hardcoded paths. All paths from env vars with defaults.

set -euo pipefail

export SBOS_FICHA_DIR="${SBOS_FICHA_DIR:-/etc/bos/blibs/servers/monitorserver/kube-state-metrics}"
...
ficha_install() {
    ...
    sbos_k8s_core "${SBOS_FICHA_DIR}/kube-state-metrics.k8s.yml" "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready daemonset/kube-state-metrics -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_update() {
    ...
    kubectl rollout restart daemonset kube-state-metrics -n "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready daemonset/kube-state-metrics -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_uninstall() {
    ...
    kubectl delete daemonset kube-state-metrics -n "${SBOS_NAMESPACE}" --ignore-not-found
