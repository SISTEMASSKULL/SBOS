# BAUTH-D1-MANUAL-COMPLETO.md — El Núcleo de bAuth (D1)

**Versión:** 5.0 · **Fecha:** 2026-06-25 · **Estado:** ✅ PROYECTO COMPLETADO
**Tablas totales:** 177 (160 bauth + 8 bglobal + 9 bcalendar) · **Errores DDL:** 0
**Idempotencia:** ×3 verificada · **Seeds:** 57 idempotentes
**DDL:** `BauthAgent/db/migrations/DDL_skSBOS_db.sql` (~5000 líneas)
**VPS:** 13.140.128.230 · **DB test:** `bauth_test`
**Visión:** `SBOS-CONTEXT-PLANE-VISION.md`
**Inventario:** `BAUTH-INVENTARIO-TABLAS-DECISION.md` v7.0
**Propósito:** Documento ÚNICO del ecosistema de tablas de bAuth. Cubre: arquitectura,
**91 tablas en schema bauth** (migradas del DDL antiguo + existentes + nuevas),
guías de llenado de plantillas (RolTemplate 14 secciones, UserTemplate 16 bloques),
matriz de 40 métodos de autenticación, combinaciones válidas y prohibidas (NIST SP 800-63B-4),
niveles de aseguramiento AAL1-3 + Step-Up RFC 9470, 12 dominios de soberanía,
procedimientos operativos, 42+ estándares de referencia, y 23+ seeds poblados con datos reales.
Sin D1 no hay autenticación, no hay roles, no hay permisos, no hay facturación,
no hay acceso físico. **Si D1 falla, el SBOS no funciona.**
...
### 12.7 Próximos pasos (post-proyecto)
- 12 `ath_policy_d*` — Políticas por dominio
- 12 `ath_config_d*` — Configuraciones por dominio
- 12 `idn_role_d*` — Templates de rol por dominio
- 4 `zone_*` — Restricciones de zona (field, button, record, data)
- 2 `ath_auth_flow*` — Flujos compuestos de autenticación
- 1 `ath_step_up_rule` — Reglas RFC 9470
- 2 `fis_*` — Métodos de zona física + emergencia
- 2 D4 — Horas extra + descansos
- 2 D7 — ZTNA + ses_risk_policy + ses_caep_config
- 2 D14 — SoD validation + conflict_interest
- 4 complementarias — tryton_action_visibility + sod_validation + conflict_interest + net_ztna
