...
**Última actualización:** 2026-06-25 · **Progreso REAL:** 273✅ código / 452📄 diseño / 26🔴 pendiente (B47 nuevos) / 0⚠️ bloqueado
...
| # | Requisito | Tablas | Estado |
|---|-----------|--------|:---:|
| BOS-16 | `ses_risk_policy` + `ses_caep_config` (D8) | T-326, T-327 | 🔴 PENDIENTE |
...
| **22** | **`SBOS-MANUAL-BAUTH-COMPONENT-ROLES.md`** | **1.7** | **236** | **2026-06** | **🔑 B1, B12-B15 — Arquitectura de motores: KC identidad, bAuth Identity Control Plane — BitMask Dual + 12 dominios** |
...
|---|---|---|---|---|---|---|---|---|
| B0.T01 | Workspace Cargo + `Cargo.toml` + perfil LTO | 2h | ✅ | `066062e` | 📝 Cargo.toml 25 deps. 115 archivos Rust. `cargo check` 256w 0e. `cargo test` 275/275 ✅. Binario 5.1MB MUSL static. | ☑ | 0,2,4 | BAUTH-050 |
| B0.T02 | `src/main.rs` + `src/bin/bauthctl.rs` | 1h | ✅ | `24fef4d` | 📝 main.rs 370 líneas, 59 handlers. bauthctl 11 comandos 13 ops, ayuda español, 339 líneas. 0 alertas (sign+tenant implementados). | ☑ | 0 | BAUTH-050 |
| B0.T03 | Build MUSL estático x86_64-unknown-linux-musl | 1h | ✅ | `21fd383` | 📝 Release 5.1MB < 15MB. .cargo/config.toml. Makefile 9 targets. static-pie stripped. | ☑ | 0,1 | C-08 |
| B0.T04 | Config TOML + serde — carga/validación | 4h | ✅ | `fd801c9` | 📝 434 líneas. 11 structs + Default impls. 10 tests. bauth.toml.example 45 líneas. Preflight integrado. Errores en español. | ☑ | 0,1,2 | BAUTH-050 || B0.T05 | Señales SIGTERM/SIGHUP (tokio::signal) | 4h | ✅ | `93f8996` | 📝 196 líneas. DrainManager AtomicU64. Drain timeout 5s. SIGHUP reload sin reinicio. 5 tests. Probado en VPS. | ☑ | 0,1 | BAUTH-010 || B0.T06 | systemd unit bauth.service + sd_notify | 2h | ✅ | `2c0687a` | 📝 Type=notify WatchdogSec=30. 17 directivas hardening. 95 líneas. sd_notify_ready() en main.rs:404. Probado en VPS: stop/start/restart OK. | ☑ | 0,1,3 | BAUTH-180 |
| B0.T07 | Unix socket /run/bos/bauth.sock | 4h | ✅ | `246251e` | 📝 UnixListener bind 0660 grupo bosagent. Cleanup socket huérfano. Interface Dual: JSON-RPC vs WebSocket detectados por primer byte. 47 métodos. 100 req concurrentes OK. Probado en VPS. | ☑ | 0,1,2 | ADR-020 |
| B0.T08 | CI pipeline GitHub Actions | 2h | ✅ | `—` | 📝 .github/workflows/bauth.yml: 5 jobs (build-musl, test, fmt, clippy, audit). Cache cargo. YAML válido. Creado Jun 27. | ☑ | 0,1,3 | ECO-007 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B1.T01 | Trait `AuthEngine` — definición de interfaz | 2h | ✅ | `—` | 📝 `engine/mod.rs`: trait con 5 métodos. Verificado en VPS: 16 protocolos federation. | ☑ | 0,2,4 | BAUTH-020 |
| B1.T02 | Trait `DomainEvaluator` — definición de interfaz | 1h | ✅ | `—` | 📝 4 métodos (domain_code, evaluate, name, is_fast_path). Verificado con 12 dominios. | ☑ | 0,2,4 | BAUTH-020 |
| B1.T03 | **BitMask Átomo (64-bit label encoding) — `domain/bitmask/atom.rs`** | 4h | ✅ | `46d917b` (DESCARTADO) | 📝 Estructura real: [8 dev][4 dom][9 app][11 grp] + [3 trust][2 bind][1 blk][2 pol][24 verb]. 1584 átomos D1 decodificados correctamente. | ☑ | 0,1,2,4 | MANUAL-PRIVILEGIOS §4 |
| **B1.T10** | **BitMask Átomo Parser** | 2h | ✅ | — | 📝 Extraer domain/app/group/verb con máscaras. Verificado con 1584 átomos D1. | ☑ | 0,1,2 | MANUAL-PRIVILEGIOS §4 |
| **B1.T11** | **Policy State Overlay** | 2h | ✅ | — | 📝 PolicyState (2 bits): NoAplica(00)→Aprobado(10). Idempotente, sin perder bits. | ☑ | 0,1,2 | MANUAL-PRIVILEGIOS §4.3 |
| B1.T04 | **Token bAuth 4-capas** | 4h | ✅ | — | 📝 Capa 1: JWT Ed25519 liviano ~1.1 KB (sin bitmasks). Capa 2: Cookie RolBitMask opcional. Capa 3: Merkle D12 Keccak-256. Capa 4: RS256 legacy opcional. Ver TOKEN-TESTING.md + BAUTH-VISION §2.5. | ☑ | 0,1,2,4 | BAUTH-VISION §2.5 |
| B1.T05 | `EngineRegistry` — `register_engine()` + `sync_all()` | 4h | ✅ | — | 📝 EngineRegistry con HashMap. Verificado con 16 protocolos federation. | ☑ | 0,1,2 | BAUTH-020 |
| B1.T06 | `DomainRegistry` — `register_domain()` + `evaluate_all()` 12 dominios | 6h | ✅ | — | 📝 12 evaluadores. Cortocircuito en D9. 71ms evaluación completa. | ☑ | 0,1,2,4 | METHODOLOGY v1.2 §3 |
| B1.T07 | `ComputeRolBitMask` — RolTemplates → Rol BitMask (N bits one-hot) | 4h | ✅ | — | 📝 5808 átomos totales, 8 activos vía privilege_role_atom. One-hot, 91 words u64. | ☑ | 0,1,2,4 | MANUAL-PRIVILEGIOS §5 |
| B1.T08 | `MergeRoles` — OR de múltiples roles (Rol BitMask, one-hot) | 2h | ✅ | — | 📝 OR merge de 8 átomos vía privilege_role_atom. SoD pre-merge. | ☑ | 0,1,2,4 | MANUAL-PRIVILEGIOS §5-6 |
| B1.T09 | `InheritFromParent` — Herencia DAG sobre Rol BitMask (OR transitivo) | 2h | ✅ | — | 📝 42 átomos efectivos. Closure table idn_role_closure. OR transitivo. | ☑ | 0,1,2,4 | MANUAL-PRIVILEGIOS §6 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B5.T01 | Átomos D5 — Tipos biométricos | 2h | ✅ | — | 📝 192 átomos D5 en catálogo global. FastPath verifica atom_position en RolBitMask. Superado → External-Path (Keycloak). | ☑ | 0,1,2,4 | COMPONENT-ROLES §1 |
| B5.T02 | D5 — LoA (Level of Assurance) + Step-Up | 2h | ✅ | — | 📝 LoA verificado post-FastPath. Step-Up (RFC 9470): elevación temporal. | ☑ | 0,1,2,4 | MANUAL-PRIV §7.2 |
| B5.T03 | BiometricEvaluator — impl DomainEvaluator | 2h | ✅ | — | 📝 FastPath + External-Path. Sin raw data biométrico. | ☑ | 0,1,2,4 | COMPONENT-ROLES §1 |
| B5.T04 | BiometricEvaluator::evaluate() — LoA check + Step-Up | 4h | ✅ | — | 📝 FastPath verifica bit → LoA_token vs LoA_required → PERMITIDO/PENDIENTE. | ☑ | 0,1 | MANUAL-PRIV §7 |
| B5.T05 | DDL biométrico — PBKDF2-SHA256 | 2h | ✅ | — | 📝 Sin raw data. 310K iteraciones. Salt único. RGPD Art.9. | ☑ | 0,1,3,4 | RGPD Art.9 |
| B5.T06 | Integración BiometricDomain — catálogo global + External-Path | 2h | ✅ | — | 📝 D5 usa el MISMO catálogo global y RolBitMask. 192 átomos con atom_position. FastPath primero, External-Path después. Sin BitmaskBundle. | ☑ | 1,3 | COMPONENT-ROLES §0.1 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B6.T01 | Átomos D4 — Catálogo temporal | 1h | ✅ | — | 📝 576 átomos D4 en catálogo global. FastPath verifica atom_position. requires_policy=true → PolicyPath. | ☑ | 0,1,2,4 | MANUAL-PRIV §7.3 |
| B6.T02 | D4 Policy — Turnos, ventanas, feriados | 2h | ✅ | — | 📝 Restricciones en Policy-Path (tablas PostgreSQL: horarios, turnos, feriados). Post-FastPath. | ☑ | 0,1,2 | MANUAL-PRIV §7.2 |
| B6.T03 | D4 Policy — Expiración sesión, delegación, max_session_duration | 2h | ✅ | — | 📝 Configurado en privilege_atom_policy con policy_domain=4. | ☑ | 0,1,2 | MANUAL-PRIV §7.2, §15 |
| B6.T04 | TemporalEvaluator — impl DomainEvaluator | 2h | ✅ | — | 📝 domain_name() → "temporal". FastPath + PolicyPath. Evalúa horario, día hábil, expiración. | ☑ | 0,1,2 | MANUAL-PRIV §7 |
| B6.T05 | TemporalEvaluator::evaluate() — día + turno + ventana + expiración | 4h | ✅ | — | 📝 Fuera de turno → DENEGADO. Delegación expirada → DENEGADO. Feriado → según tenant. | ☑ | 0,1 | MANUAL-PRIV §7 |
| B6.T06 | Integración D4 — catálogo global + PolicyPath | 2h | ✅ | — | 📝 D4 usa el MISMO catálogo global. 576 átomos con atom_position. FastPath primero, PolicyPath después. Sin BitmaskBundle. | ☑ | 1,3 | MANUAL-PRIV §7.3 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B7.T01 | Átomos D6 — Catálogo geoespacial | 2h | ✅ | — | 📝 192 átomos D6 en catálogo global. FastPath verifica atom_position. requires_policy=true → PolicyPath. | ☑ | 0,1,2,4 | MANUAL-PRIV §7 |
| B7.T02 | D6 Policy — Geo-fencing: país, IP, sucursal, fence radius | 2h | ✅ | — | 📝 Configurado en privilege_atom_policy. Validación post-FastPath. | ☑ | 0,1,2 | MANUAL-PRIV §7.3 |
| B7.T03 | GeospatialEvaluator — impl DomainEvaluator | 2h | ✅ | — | 📝 domain_name() → "geospatial". FastPath + PolicyPath. País, IP, jurisdicción. | ☑ | 0,1,2 | MANUAL-PRIV §7 |
| B7.T04 | GeospatialEvaluator::evaluate() — país + IP + jurisdicción | 4h | ✅ | — | 📝 IP fuera de Bolivia → DENEGADO. Viaje imposible >900km/h → DENEGADO. | ☑ | 0,1 | MANUAL-PRIV §7 |
| B7.T05 | Integración D6 — catálogo global + PolicyPath | 2h | ✅ | — | 📝 D6 usa el MISMO catálogo global. 192 átomos con atom_position. FastPath primero, PolicyPath después. Sin BitmaskBundle. | ☑ | 1,3 | MANUAL-PRIV §7.3 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B8.T01 | Átomos D7 — Catálogo de red | 2h | ✅ | — | 📝 192 átomos D7 en catálogo global (DMZ, INTERNAL, VPN, mTLS). FastPath + External-Path (Kong). | ☑ | 0,1,2,4 | MANUAL-PRIV §7 |
| B8.T02 | D7 Policy — Protocolos, Rate Limit, QoS, mTLS | 2h | ✅ | — | 📝 Configurado post-FastPath. Kong plugin evalúa zona + protocolo + device posture. | ☑ | 0,1,2 | SBOS-054 §10 |
| B8.T03 | NetworkEvaluator — impl DomainEvaluator | 2h | ✅ | — | 📝 domain_name() → "network". Zero Trust: CIDR, VPN, mTLS, device posture. | ☑ | 0,1,2 | NIST 800-207 |
| B8.T04 | NetworkEvaluator::evaluate() — zona + protocolo + VPN + Rate Limit | 4h | ✅ | — | 📝 DMZ → sin acceso interno. Sin mTLS → DENEGADO. Rate limit excedido → DENEGADO. | ☑ | 0,1 | SBOS-054 §4 |
| B8.T05 | Integración D7 — catálogo global + External-Path | 2h | ✅ | — | 📝 D7 usa el MISMO catálogo global. 192 átomos con atom_position. FastPath primero, External-Path (Kong) después. Sin BitmaskBundle. | ☑ | 1,3 | SBOS-054 |

---

## B9 — Policies Authentication Framework + Policy Engine + Admin Frameworks SSOT (36 átomos)

**SSOT:** `Policies_Authentication_Framework_v4.json` v4.0 (780 líneas, 10 secciones, 18 métodos KC) + `Authentication_Framework.json` v3.0.0 (13,213 líneas, 27+1 grupos) · 📄 Documentos actualizados 2026-06-20
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B9.T24** | **`PolicyEngine`** — motor de evaluación de políticas: cargar, cachear, evaluar | 4h | ✅ | ✅ 2026-06-28 | ✅ Implementado. `ath_converter.rs`: 50+ tipos de regla → `PolicyRule` (data-driven, parametrizado). `ath_loader.rs`: carga `ath_policy_d1..d12` en paralelo, filtro `config ? 'rule'`. `handlers/policy_domain.rs`: 3 métodos JSON-RPC — `bauth.policy.domain.evaluate`, `bauth.policy.domain.list`, `bauth.policy.library.search`. Motor: `PolicyEngine::evaluate()` XACML 3.0 existente reutilizado sin modificar. Compilación MUSL limpia. 5 archivos, 0 código existente tocado. | ☑ | 0,1,2,4 | NIST SP 800-207 §3, XACML 3.0 |
| **B9.T25** | **`PolicyAdministrator`** — CRUD de políticas vía JSON-RPC + Core UI | 4h | ✅ | — | 📝 **NIST PA role.** Interfaz de administración de políticas: `bauth.policy.create/update/delete/list/validate`. Cada política tiene: `policy_slug`, `policy_name`, `policy_domain`, `policy_tier` (a qué tiers aplica), `policy_params` (JSONB con parámetros configurables). Cambios de política → SIGHUP al PolicyEngine → recarga sin reiniciar. Ref: NIST SP 800-207 §3 (PA role), ADR-020. | ☐ | 0,1,2,3,4 | NIST SP 800-207 §3, ADR-020 |
| **B9.T26** | **`PolicyConflictDetector`** — detectar conflictos entre políticas al cargar/modificar | 4h | ✅ | — | 📝 Validación al cargar políticas: (1) ¿dos políticas asignan valores contradictorios al mismo parámetro? (ej: `max_session=8h` vs `max_session=4h` para el mismo tier), (2) ¿una política de DENY anula una de ALLOW para el mismo átomo?, (3) ¿dependencias circulares entre políticas? Si conflicto ALTO → rechazar carga. Si conflicto MEDIO → warning + aprobación requerida. Ref: OASIS XACML 3.0 (Policy Combining Algorithms). | ☐ | 0,1,2,4 | XACML 3.0 |
...
**Principio:** El archivo JSON del RolTemplate **ES** la plantilla que alimenta los registros de los motores. No es un archivo que se "guarda y luego se sincroniza" — es el documento activo cuya estructura JSON se traduce DIRECTAMENTE a objetos nativos en Keycloak.

```
RolTemplate JSON (bauth_db)
    │
    ├──► role.id            → Keycloak Composite Role name
    │                         Group name
    ├──► role.bits          → Keycloak Realm Roles (1 por bit activo)
    │                         Group permissions
    ├──► role.parent_id     → Herencia H-RBAC en Keycloak 
    ├──► role.auth_flows    → Keycloak Authentication Flows (MFA por rol)
    ├──► role.zones         → rule (SQL por zona)
    └──► role.constraints   → model.access (CRUD por modelo)
```

**SSOT:** `SBOS-ROLTEMPLATE-v6_0.md` (actualizado 2026-06-20: Argon2id, NIST 800-63B Rev.4, BitMask 64-bit, SMS deprecado). **DoD:** editar template JSON → validado → Keycloak reflejan el cambio en < 5s.

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B10.T01 | DDL `bos_rol_template` — la tabla que almacena el template JSONB | 2h | 📄 | — | 📄 `SBOS-ROLTEMPLATE-v6_0.md` (1,212 líneas, 14 bloques JSONB). `BAUTH-CATALOGO-ROLES-EMPRESARIALES.md`: 66 plantillas base. ISO 24760-2:2025 para atributos. | ☐ | 0,1,3 | ROLTEMPLATE-v5 §5 |
...
| B10.T03 | Struct `RolTemplate` — campos JSON que alimentan Keycloak | 2h | 📄 | — | 📄 `SBOS-ROLTEMPLATE-v6_0.md`: struct con 14 bloques (identificación, vigencia, lógico, físico, financiero, SAM-128, delegación, SoD, compliance). `SBOS-008-ROLFRAMEWORK-v1_0.md` §5. | ☐ | 0,1,2 | ROLTEMPLATE-v5 §1 |
...
| B10.T10 | `bauthctl role list/get/archive` — consulta de templates | 2h | 📄 | — | 📝 📄 Diseño completo. RolTemplate v6.0 §1: API JSON-RPC 2.0. ADR-020 Interface Dual. | ☐ | 0,1,2 | ADR-020 |
...
| B10.T12 | Traducción Template : `template_to__objects()` | 4h | 📄 | — | 📄 `SBOS-ROLTEMPLATE-v6_0.md` §Mapping: RolTemplate→ (5 capas). `SBOS-008-ROLFRAMEWORK-v1_0.md` §2.2: 5 capas de enforcement . | ☐ | 0,1,2 | SYMBIOSIS, BOS_V8 §9 |
...
| B10.T14 | Sync automático al aprobar: template → KC  en < 5s | 4h | 📄 | — | 📄 `BAUTH-CONTRATO-SYMBIOSIS.md` §2-4: sync < 5s atómico con rollback. `SBOS-ROLTEMPLATE-v6_0.md` §14: sync_state con Keycloak targets. | ☐ | 0,1,2 | SYMBIOSIS |
| B10.T15 | Tests integrales — template → KC  ida y vuelta | 4h | 📄 | — | 📝 📄 Diseño: casos de prueba definidos. RolTemplate v6.0 §Reglas: 6 validaciones schema + 6 semánticas. | ☐ | 1 | BAUTH-050 |
...
| **B10.T89** | **`TemplateDryRunSync`** — simular sync sin ejecutar: ¿qué cambiaría en Keycloak si aplico este template? | 2h | 📄 | — | 📝 `bauthctl role sync --dry-run <role_slug>`. Compara estado actual de Keycloak vs. lo que el template especifica. Output: `{keycloak: {roles_to_create: [...], roles_to_update: [...], roles_to_delete: [...]}, : {groups_to_create: [...], rules_to_update: [...]}}`. Sin modificar nada. Útil para: validar antes de deployar cambios, auditar drift sin corregir. | ☐ | 0,1,2,3 | BAUTH-060 |

---

## B11 — UserTemplate Framework + Administración de Usuarios (32 átomos)

**Principio:** El archivo JSON del UserTemplate **ES** la plantilla que alimenta los registros de usuario en Keycloak. No es un archivo que se "guarda y luego se sincroniza" — es el documento activo cuya estructura JSON se traduce DIRECTAMENTE a objetos nativos User en ambos motores.

```
UserTemplate JSON (bauth_db)
    │
    ├──► user.id             → Keycloak User ID + User ID
    ├──► user.username       → Keycloak username + login
    ├──► user.roles[]        → Keycloak Groups + Groups
...
**SSOT:** `SBOS-USERTEMPLATE-v6_0.md` (actualizado 2026-06-20: NIST 800-63B Rev.4, ISO 24760-2:2025, OAuth 2.0 RFCs). **DoD:** editar template JSON → validado → Keycloak reflejan el cambio en < 5s.
...
| B11.T03 | Struct `UserTemplate` — campos JSON que alimentan Keycloak | 2h | 📄 | — | 📄 `SBOS-USERTEMPLATE-v6_0.md`: struct con 16 bloques. Separación identidad (UserTemplate) vs autoridad (RolTemplate). 6 invariantes de seguridad. | ☐ | 0,1,2 | USERTEMPLATE-v5 |
...
| B11.T12 | Traducción Template : `user_template_to_()` | 4h | 📄 | — | 📝 📄 Diseño completo. UserTemplate→: res.user + grupos + delegación temporal. | ☐ | 0,1,2 | SYMBIOSIS, BOS_V8 §9 |
| B11.T13 | Sync automático al modificar template → KC  en < 5s | 4h | 📄 | — | 📝 📄 Diseño completo. Sync < 5s. Atómico con rollback. KC  simultáneo. | ☐ | 0,1,2 | SYMBIOSIS |
| B11.T14 | Tests integrales — template → KC  ida y vuelta | 4h | 📄 | — | 📝 📄 Diseño: casos de prueba definidos. Crear→assign→verify→revoke→verify→delegate→expire. | ☐ | 1 | BAUTH-050 |
...
| B11.T21 | `onboard_step5_sync_and_verify()` — sincronizar KC  + verificar acceso | 2h | 📄 | — | 📝 UserTemplate §Onboarding P5: sync user→Keycloak. Verificar login funcional. | ☐ | 0,1 | BAUTH-050 |
...
| B11.T24 | `offboard_step3_deactivate_and_archive()` — desactivar KC  + archivar PII según retención | 4h | 📄 | — | 📝 UserTemplate §Offboarding P3-P6: active=false, retención por jurisdicción (BO: 8 años fiscal). | ☐ | 0,1,2,3,4 | RGPD Art.17 |
...
## B13 — Motor PDP (Policy Decision Point) (22 átomos)

**Principio:** bAuth **NO acepta intervención directa en sus bases de datos.** Toda operación de creación, actualización o consulta de usuarios, grupos y permisos debe hacerse a través de su API JSON-RPC nativa. bAuth debe construir un cliente JSON-RPC especializado que respete las políticas del motor.

> **Reglas del motor que bAuth DEBE respetar:**
...
**SSOT:** [Protocol Handlers](https://deepwiki.com/tryton/policy-engine/7.2-protocol-handlers), [User Design](https://docs.tryton.org/7.0/server/modules/res/design.html)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B13.T01 | `bAuthEngine` struct — impl `AuthEngine` trait | 2h | 📄 | — | 📄 `BAUTH-ARQUITECTURA-FRAMEWORK.md` §R1:  como motor de autoridad. `BAUTH-CONTRATO-SYMBIOSIS.md` §3: JSON-RPC exclusiva. | ☐ | 0,1,2 | BAUTH-060 |
| B13.T02 | JSON-RPC client — `common.db.login` + session | 4h | 📄 | — | 📝 *Pendiente* — POST `/<database_name>/` JSON-RPC 2.0. `common.db.login(user, password)` → session token. Token cacheado 60s. Auth: API key desde Vault. Custom serialization: Decimal→`{__class__:'Decimal'}`, datetime→ISO 8601. Test: login → session cookie válido | ☐ | 0,1,2 | RPC |
| B13.T03 | `bAuthEngine::sync_role()` — RolTemplate  Group via JSON-RPC | 4h | 📄 | — | 📄 `SBOS-008-ROLFRAMEWORK-v1_0.md` §2.2: 5 capas de enforcement. `SBOS-ROLTEMPLATE-v6_0.md` §7: privileges (model_access, visible_actions, field_restrictions, button_rules, record_rules). | ☐ | 0,1,2 | BOS_V8 §9, RPC |
| B13.T04 | `bAuthEngine::sync_user()` — UserTemplate  User via JSON-RPC | 4h | 📄 | — | 📝 *Pendiente* — `model.res.user.create(name=user.username, login=user.username, email=user.email, groups=[role.group_id])`. Asignar grupos desde `user.roles[]`. Delegación temporal: agregar/quitar grupo según `valid_until`. **NUNCA usar DELETE** — solo `active: false`. Test: crear user → verificar  UI | ☐ | 0,1,2 | BOS_V8 §9, RPC |
| B13.T05 | `bAuthEngine::reconcile()` — comparar  vs bauth_db via JSON-RPC | 4h | 📄 | — | 📝 *Pendiente* — `model.res.group.search_read()` → comparar con `bos_rol_template`. `model.res.user.search_read()` → comparar con `bos_user_template`. Detectar drift: grupo falta  → `create`. grupo sobra → `write(active=false)`. Test: drift detectado → corregido en < 60s | ☐ | 0,1,2 | BAUTH-060 |
| B13.T06 | Bootstrap simbiótico  — reconstrucción total via JSON-RPC | 4h | 📄 | — | 📝 *Pendiente* — Si  fue destruido y recreado: 1) login, 2) iterar `bos_rol_template` → `create` grupo+permisos, 3) iterar `bos_user_template` → `create` usuario+grupos. Idempotente: si ya existe → `write`. Test:  vacío → reconstruir 100% desde bauth_db | ☐ | 0,1,2 | SYMBIOSIS §5 |
| B13.T07 | Tests integrales bAuthEngine — JSON-RPC real | 4h | 📄 | — | 📝 *Pendiente* — Test: login → crear grupo → crear usuario → asignar grupo → verificar permisos → modificar template → update  → verificar cambios → deactivate user → verificar no eliminado. Cobertura ≥ 80% | ☐ | 1 | BAUTH-050 |

### B13.1 — Operaciones Detalladas PDP (7 átomos · 20h)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B13.T08 | `_login_session()` — autenticar vía JSON-RPC common.db.login + cache token 60s | 2h | 📄 | — | 📝 POST /{database}/ JSON-RPC 2.0. Custom serialization: Decimal→{__class__:'Decimal'}, datetime→ISO 8601. | ☐ | 0,1,2 | RPC |
| B13.T09 | `sync_group_create()` — crear/actualizar res.group con name=role.id | 4h | 📄 | — | 📝 model.res.group.create(name, parent). Jerarquía de grupos = jerarquía de roles. | ☐ | 0,1,2 | RPC |
| B13.T10 | `sync_ir_model_access()` — CAPA 1: CRUD por modelo (ir.model.access) | 4h | 📄 | — | 📝 model.ir.model.access.create(model, group, perm_read, perm_write, perm_create, perm_delete). | ☐ | 0,1,2 | Access |
| B13.T11 | `sync_ir_rule()` — CAPA 2: reglas de registros SQL por zona (ir.rule) | 4h | 📄 | — | 📝 model.ir.rule.create(model, group, domain_pyson, rule_field). 1 regla por zona de negocio. | ☐ | 0,1,2 | Access |
| B13.T12 | `sync_ir_button()` — CAPA 3: control de botones con PYSON + SoD | 2h | 📄 | — | 📝 model.ir.model.button.create(model, button, group, condition_pyson, users_required, sod_cannot_also). | ☐ | 0,1,2 | Access |
| B13.T13 | `sync_ir_field()` — CAPA 4: restricción de campos individuales (ir.model.field) | 1h | 📄 | — | 📝 model.ir.model.field.create(model, field, group, perm_read, perm_write). | ☐ | 0,1,2 | Access |
| B13.T14 | `sync_ir_action()` — CAPA 5: acciones y menús visibles (ir.action.groups) | 1h | 📄 | — | 📝 model.ir.action.group.create(action, group). Solo acciones listadas en privileges.visible_actions. | ☐ | 0,1,2 | Access |

### B13.2 — Motor bAuth-PDP: Policy Decision Point para Recursos de Gobierno (8 átomos · 28h)

**Principio:** bAuth-PDP es un **pod separado del bAuth-ERP**, corriendo el mismo motor `policy-engine` pero **sin ningún módulo de negocio** — dedicado exclusivamente a servirle a bAuth como PDP (Policy Decision Point) para recursos de gobierno: zonas físicas, límites financieros, delegaciones. Una instancia ligera por tenant.

> **bAuth-PDP ≠ bAuth-ERP:** aunque comparten el motor `policy-engine`, son dos despliegues independientes con roles completamente distintos. bAuth-PDP NO tiene módulos de contabilidad, ventas, inventario, RRHH. Solo tiene los módulos núcleo `ir`/`res` + módulos custom de bAuth (`bauth.zone`, `bauth.financial_limit`, `bauth.delegation`).
>
> **Reglas de seguridad:**
> - bhnexus, Kong y otras apps consultan a bAuth — **nunca directamente a bAuth-PDP**
...
>
> **SSOT:** `SBOS-MANUAL-BAUTH-COMPONENT-ROLES.md` v1.7 §3 ·  Server RPC docs · Plan Maestro M-22, M-23, M-24

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B13.T15** | **Despliegue bAuth-PDP como pod separado por tenant** | 4h | 📄 | — | 📝 Crear namespace K8s `-pdp-{tenant_id}`. Desplegar `d` SIN módulos de negocio (solo `ir`, `res`, `web`). Instalar módulos custom bAuth: `bauth.zone`, `bauth.financial_limit`, `bauth.delegation`. Configurar TLS para JSON-RPC. Una instancia por tenant — siguiendo el mismo patrón de aislamiento que los realms de Keycloak. Ref: COMPONENT-ROLES §3 (descripción de pod separado), M-22, M-23. | ☐ | 0,1,2,3,4 | COMPONENT-ROLES §3 |
| **B13.T16** | **User Application Key — creación, validación, rotación en Vault** | 4h | 📄 | — | 📝 Crear key vía `POST /{database}/user/application/` con body `{"user": "bauth-sync", "application": "bauth"}`. Paso manual único de validación desde preferencias de usuario  (documentar en runbook de alta de tenant). Almacenar key en Vault. Usar en cada llamada RPC como `Authorization: Bearer <key>`. Rotación cada 90 días con período de transición de 24h. Revocación: `DELETE /{database}/user/application/`. Ref: COMPONENT-ROLES §3.2, §3.3. | ☐ | 0,1,2,4 | COMPONENT-ROLES §3.2-3.3 |
| **B13.T17** | **PDP Zone Engine — `bauth.zone` module** (acceso a zonas físicas D2) | 4h | 📄 | — | 📝 Módulo custom de bAuth-PDP: define zonas físicas (oficina, bóveda, servidor, depósito) y las asigna a grupos. `res.group` + `ir.rule`: filtrar qué zonas ve cada grupo. `ir.model.access`: CRUD por zona. Integración con D2: bAuth consulta PDP para decisiones de acceso físico. Ref: COMPONENT-ROLES §3 (alcance: zonas físicas). | ☐ | 0,1,2 | COMPONENT-ROLES §3 |
...
> Cada aplicación del SBOS (, Saleor, OrangeHRM, Grafana, Prometheus, Core UI, etc.) tiene
...
| B14.T02 | Catálogo de aplicaciones SBOS — `AppAuthRegistry` | 4h | 📄 | — | 📝 *Pendiente* — Registrar cada aplicación del ecosistema con su método de auth:  (OIDC+JWT), Saleor (OAuth2+JWT), OrangeHRM (SAML→OIDC bridge), Grafana (OAuth2), Prometheus (basic auth→OAuth2 proxy), Core UI (OIDC+PKCE). `AppAuthConfig { app_id, auth_method, oauth2_proxy_port, upstream_url, allowed_routes, rate_limit }`. Test: registro de 6 apps | ☐ | 0,1,2,3 | SBOS_V8 apps |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B14.T09 | `AppAuthRegistry` — registrar 6 apps: , Saleor, OrangeHRM, Grafana, Prometheus, Core UI | 4h | 📄 | — | 📝 Struct AppAuthConfig { app_id, auth_method, oauth2_proxy_port, upstream_url, allowed_routes, rate_limit }. | ☐ | 0,1,2,3 | SBOS_V8 apps |
...
| B14.T11 | `config_()` — OAuth2-Proxy para  ERP | 2h | 📄 | — | 📝 upstream: -sbos:8000. skip-auth: /health, /metrics. email-domain: *.sbos.skull.bo. | ☐ | 0,1,2 |  |
...
| B19.T02 | Saga Sync RolTemplate → KC  atómico | 4h | 📄 | — | 📝 *Pendiente* — 1=guardar BD, 2=sync KC, 3=sync , 4=verificar, 5=auditar. Rollback si falla | ☐ | 0,1,2 | BOS_V8 §4 |
| B19.T03 | Saga Repair — detectar y corregir drift | 4h | 📄 | — | 📝 *Pendiente* — 1=comparar KC, 2=comparar , 3=corregir, 4=verificar, 5=auditar | ☐ | 0,1,2 | BAUTH-060 |
...
| B19.T05 | Saga Simbiosis Bootstrap — reconstrucción total | 4h | 📄 | — | 📝 *Pendiente* — KC destruido → reconstruir desde bauth_db.  destruido → reconstruir. Idempotente | ☐ | 0,1,2 | SYMBIOSIS §5 |
...
| B19.T10 | Saga Install: P4 `sync_all` — sincronizar 66 roles → KC  | 4h | 📄 | — | 📝 B12.T08-T14 + B13.T03-T04 para cada RolTemplate. Atómico: rollback si cualquier sync falla. | ☐ | 0,1,2 | SYMBIOSIS |
| B19.T11 | Saga Install: P5 `verify` — verificar KC (roles, flows, users)  (grupos, permisos) | 4h | 📄 | — | 📝 Comparar estado Keycloak vs bauth_db. Reportar diff. Gate: 100% match. | ☐ | 0,1,2 | BAUTH-060 |
...
| B19.T13 | Saga Repair: `detect_drift()` + `correct_drift()` — comparar Keycloak vs bauth_db | 4h | 📄 | — | 📝 GET /roles + /users → diff. Drift: crear falta, eliminar sobra, actualizar diff. Idempotente. | ☐ | 0,1,2 | BAUTH-060 |
...
| B19.T15 | Saga Tenant: `offboard_tenant()` — baja: notificar + exportar datos + eliminar realm | 2h | 📄 | — | 📝 Exportar datos PII del tenant. Eliminar realm KC. Soft-delete . Auditoría. | ☐ | 0,1,2,4 | RGPD Art.17 |
...
| B19.T17 | Saga Simbiosis: `bootstrap_from_zero()` — reconstruir Keycloak desde bauth_db | 4h | 📄 | — | 📝 KC realm destruido → recrear desde cero.  destruido → recrear. Idempotente. | ☐ | 0,1,2 | SYMBIOSIS §5 |
...
### B19.2 — Sagas Faltantes: D12 -PDP + Políticas + Dispositivos + Tokens + Backup + DR (8 átomos · 32h)
...
| **B19.T26** | **Saga Disaster Recovery** — reconstrucción total desde backups + blockchain | 6h | 📄 | — | 📝 **El peor escenario:** pérdida total de PostgreSQL. Pasos: (1) restaurar último backup de BD desde MinIO → verificar SHA-256, (2) restaurar RolTemplates + UserTemplates desde YAML, (3) reconstruir KC desde bauth_db (Simbiosis Bootstrap, B19.T17), (4) reconstruir  desde bauth_db, (5) reconstruir bAuth-PDP desde bauth_db (B19.T21), (6) verificar Merkle roots en Arbitrum One contra `bos_blockchain_anchor_log` → confirmar que los datos restaurados son consistentes con los anclajes blockchain (D12-A como bauth_db de último recurso), (7) reconstruir red Besu QBFT desde snapshots (B29), (8) health check completo: todos los motores operativos, (9) emitir certificado de recuperación firmado digitalmente. RPO ≤ 24h, RTO ≤ 4h. | ☐ | 0,1,2,3,4 | ADR-016, B29 |
...
| B21.T02 | `VdiProfile` — estructura JSON del perfil de escritorio | 2h | 📄 | — | 📝 *Pendiente* — `{user_id, username, tenant_id, aplicaciones: [, Firefox, LibreOffice, ...], shell_unlock: bool, usb_allowed: bool, printer_allowed: bool, network_external: bool, desktop_prefs: {wallpaper, theme, shortcuts}, session_timeout_minutes, max_sessions}`. Derivado directamente del BitmaskBundle del usuario. Test: profile JSON válido | ☐ | 0,2 | NEXUS-v3 §5 |
| B21.T03 | `VdiProfile::from_bitmask()` — traducir BitMask → perfil VDI | 4h | 📄 | — | 📝 *Pendiente* — `PhysicalDomainMask` → shell_unlock, usb_allowed, printer_allowed. `LogicalDomainMask` → aplicaciones autorizadas (APP_ERP, APP_ORANGEHRM, APP_FIREFOX...). `NetworkDomainMask` → network_external, vpn_required. `TemporalDomainMask` → session_timeout, shift_start/end. Test: BitMask con APP_ERP+APP_FIREFOX → perfil con 2 apps | ☐ | 0,1,2 | SAM-128 §8.3-8.6 |
...
| B23.T02 | **SPI 1 — `RolBitMaskProtocolMapper`** (OIDC Protocol Mapper) | 4h | 📄 | — | 📝 **OBSOLETO.** El RolBitMask ya NO se inyecta en JWT. Token liviano Ed25519 ~1.1 KB sin bitmasks. Permisos evaluados server-side vía access.evaluate. Cookie RolBitMask opcional para contingencia. Ref: BAUTH-VISION §2.5, B1.T04. | ☑ | 0,1,2,3 | BAUTH-VISION §2.5 |
...
### B24.1 — Infraestructura D12 -PDP + Multi-Region + Costos (7 átomos · 24h)
...
| **B24.T08** | **bAuth-PDP Infrastructure** — dimensionar pods PDP por tenant | 4h | 📄 | — | 📝 Un pod bAuth-PDP por tenant (CPU: 0.5, RAM: 512MB, storage: 1GB). Sin módulos de negocio → recursos mínimos. Aislamiento: namespace K8s por tenant. Auto-scaling: HPA min=1, max=3 por tenant. Límite: 100 tenants → 100 pods PDP (~50 vCPU, 50GB RAM). Ref: COMPONENT-ROLES §3, B13.T15. | ☐ | 0,1,2,3 | COMPONENT-ROLES §3 |
...
| **B24.T12** | **NetworkPolicy Matrix** — Calico NetworkPolicy para todos los componentes nuevos | 4h | 📄 | — | 📝 Reglas de red para: (1) bAuth→KC (solo puerto 9000 admin + 8443 OIDC), (2) bAuth→bAuth-PDP (solo JSON-RPC TLS), (3) bAuth→Vault (solo API), (4) bAuth→Redis (solo DB0-5), (5) bAuth→PostgreSQL (solo puerto 5432), (6) bhnexus→banexus (solo WebSocket mTLS), (7) biedata→Arbitrum RPC (solo HTTPS outbound), (8) biedata→Besu QBFT (solo JSON-RPC), (9) validadores Besu entre sí (solo P2P + consensus). Deny-all por defecto. Ref: SBOS-050 P9, SBOS-054. | ☐ | 0,1,2,4 | SBOS-054, SBOS-050 |
| **B24.T13** | **Cost Optimization** — right-sizing + auto-scaling + presupuesto total | 4h | 📄 | — | 📝 **Presupuesto mensual estimado:** (1) VPS bAuth HA: 3 nodos × $50 = $150/mes, (2) PostgreSQL HA: 1 primary + 2 replicas = $120/mes, (3) Redis Sentinel: 3 nodos × $30 = $90/mes, (4) Vault: incluido en bAuth, (5) Kong: 2 nodos × $30 = $60/mes, (6) KC: 2 nodos × $40 = $80/mes, (7) bAuth-PDP: 100 tenants × $5 = $500/mes, (8) Besu QBFT: 4 validadores × $50 + 2 RPC × $30 = $260/mes, (9) MinIO S01: $50/mes, (10) Arbitrum gas: $0.15/mes. **TOTAL: ~$1,310/mes** (sin HSM, sin multi-region). Con HSM (YubiHSM 2 FIPS × 4): +$2,600 one-time. Con multi-region (2x): ~$2,620/mes. Ref: EVALUACION §14. | ☐ | 0,1,2,3 | EVALUACION §14 |

## FICHA — Declaración como Ficha BOS (9 átomos)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| FICHA.T01 | `manifest.yml` — identity, workload, dependencies, ports, health | 2h | 📄 | — | 📝 *Pendiente* — deps: keycloak, , -pdp, postgresql, redis, vault, kong, bhnexus. ports: 9450/9451. Unix socket: /run/bos/bauth.sock. health: systemctl+watchdog. Ficha crítica: orden 125. | ☐ | 0,2,3 | BAUTH-180 |
| FICHA.T02 | `task_catalog.sh` — ficha_pre_install | 2h | 📄 | — | 📝 *Pendiente* — Verificar: KC /health/ready,  common.db.login, PG accesible, Redis PING, Vault unsealed, bAuth-PDP responde. Crear usuario bauth + grupo bosagent. | ☐ | 0,1,2,3 | BAUTH-180 |
...
| FICHA.T04 | `task_catalog.sh` — ficha_repair + ficha_test + ficha_uninstall | 4h | 📄 | — | 📝 *Pendiente* — Repair: reinstalar binario+restart, verificar sync Keycloak+PDP funcional. Test: `bauthctl role list`, `bauthctl user list`, login funcional. Uninstall: eliminar binario + config + limpiar DDL (solo en dev). | ☐ | 0,1,2,3 | BAUTH-180 |
...
| FICHA.T06 | Integración en `deploy.go` + `seed-skull.yml` | 2h | 📄 | — | 📝 *Pendiente* — Paso [9/9]: KC→→PDP→bauth. Orden simbiótico. Incluye D12 (si aplica): deploy AuditAnchor.sol en Arbitrum, inicializar pipeline de anclaje, desplegar red Besu QBFT. `bosctl deploy` instala todo. | ☐ | 0,1,2 | BAUTH-180 |
| FICHA.T07 | `bauth.toml.example` — archivo de configuración canónico | 2h | 📄 | — | 📝 *Pendiente* — Configuración completa: [socket], [keycloak], [], [_pdp], [vault], [redis], [postgresql], [blockchain] (anchor tier, L2 RPC, contract address), [besu] (validator config), [domains] (D1-D12 active), [policies] (path), [logging], [metrics]. Documentar cada campo. | ☐ | 0,2,3 | BAUTH-050 |
...
| B26.ADR04 | ADR-004: bAuth como Framework orquestador — no como engine monolítico | 2h | 📄 | — | 📝 Contexto: monolito vs microservicios vs framework. Decisión: bAuth = framework con 5 engines (KC, , OAuth2Proxy, NEXUS, Vault). | ☐ | 2,3,4 | BAUTH-ARQUITECTURA |
...
| B26.ADR08 | ADR-008: Simbiosis trilateral bAuth-KC- con bauth_db como única bauth_db | 4h | 📄 | — | 📝 Contexto: quién es dueño de la identidad. Decisión: bauth_db única verdad. Keycloak son réplicas operacionales. Reconcile 60s. | ☐ | 2,3,4 | BAUTH-CONTRATO-SYMBIOSIS |
...
| B28.T03 | `access_remove_permanent()` — offboarding completo: desactivar KC  + archivar PII | 4h | 📄 | — | 📝 NIST AC-2(3): disable account. Retención PII por jurisdicción (BO: 8 años fiscal). Soft-delete. NUNCA borrado físico. | ☐ | 0,1,2,3,4 | NIST AC-2(3) |
...
| B28.T09 | `ghost_account_detection()` — detectar cuentas huérfanas (usuario desactivado en RRHH pero activo en KC) | 2h | 📄 | — | 📝 ISACA: 37% de organizaciones tienen ghost accounts. Cron semanal: comparar OrangeHRM vs Keycloak. Auto-revocar. | ☐ | 0,1,2 | ISACA |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| B32.T01 | `dr_plan()` — Disaster Recovery Plan: RTO/RPO por componente, runbooks de recuperación | 4h | 📄 | — | 📝 RTO: bauth_db=5min, KC=15min, =30min, bAuth-PDP=30min, NEXUS=30min, Besu QBFT=2h, Arbitrum anchoring=N/A (externo). RPO: bauth_db=0 (sync), KC=60s, =60s, Besu=último snapshot (6h). | ☐ | 2,3,4 | ISO 27001 A.17 |
...
| B32.T03 | `dr_test_schedule()` — simulacro de DR trimestral: restaurar bauth_db → bootstrap Keycloak+PDP → verificar | 4h | 📄 | — | 📝 Trimestral: DR test completo. Tiempo objetivo < 4h. Escenarios: (1) pérdida PostgreSQL, (2) pérdida KC, (3) pérdida total VPS, (4) corrupción de datos. Informe post-mortem con hallazgos y mejoras. Verificar integridad Merkle roots contra Arbitrum One post-restauración. | ☐ | 0,1,2,3 | ISO 27001 A.17.1 |
| B32.T04 | `ha_architecture()` — PostgreSQL HA (Patroni + etcd), Redis Sentinel, KC multi-node | 2h | 📄 | — | 📝 3 nodos PostgreSQL (1 primary + 2 standby). Auto-failover < 30s. Redis Sentinel 3 nodos. KC 2 nodos + Infinispan. bAuth-PDP mínimo 1 por tenant. Besu QBFT: 4 validadores distribuidos geográficamente. | ☐ | 0,1,2,4 | BAUTH-180 |
...
bauth_state:
  ...
  arquitectura:
    ...
    engines:
      ...
      - bAuthEngine: "XML-RPC — grupos, ir.model.access, ir.rule (5 capas enforcement)"
  ...
  documentacion:
    ...
    estandares: 46
      ...
      - "BAUTH-CONTRATO-SYMBIOSIS.md (v1.0, 332 líneas): simbiosis trilateral bAuth-KC-"
      - "Policies_Authentication_Framework_v4.json (v4.0.0, 1119 líneas): 18 métodos KC, NIST 800-63B Rev.4"
      - "SBOS-008-ROLFRAMEWORK-v1_0.md (v2.0, 2056 líneas): 5 SPIs Java, 5 capas "
  ...
  next: "B45.A01 — Crear 12 ath_policy_d* + seeds desde JSON frameworks"
  prioridad: "Cerrar gaps del MANUAL_DB_DDL.md v18.0. B45+B46 primero (tablas base), luego B47 (dashboards + cierre gaps)."
```
...
| **B33.T05** | **Producto C — IAM Soberano:** empaquetado de los 11 dominios como servicio gestionado | 8h | 📄 | — | 📝 Desplegar SBOS completo para un tenant externo: D1-D11 completos con BitMask Dual, Keycloak realm aislado, bAuth-PDP dedicado, Vault namespace. Panel de administración de identidad para el cliente. Ref: D12 §7.3. | ☐ | 0,1,2,3,4 | D12 §7.3 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B36.T01** | **`JmlEngine`** — motor Joiner-Mover-Leaver | 4h | ✅ | — | 📝 Orquestador central de ciclo de vida. **Joiner:** HR webhook `employee.hired` → identity verification (SEGIP) → role assignment (SoD check) → credential provisioning (TOTP+recovery) → sync Keycloak → verify access → audit. **Mover:** `employee.promoted/transferred` → SoD check on new roles → recalculate Rol BitMask → adjust Keycloak → notify user. **Leaver:** `employee.terminated` → revoke all sessions (KC+ctx_id) → revoke all tokens → deactivate KC (active=false) → soft-delete  → archive PII (10 años fiscal) → export audit trail → notify manager. Tiempo maximo offboarding: <30 min desde trigger HR. Ref: ISO 27001 A.9.2.5, B11.T17-T24. | ☐ | 0,1,2,3,4 | ISO 27001 A.9 |
...
| **B36.T04** | **`GhostAccountDetector`** — deteccion de cuentas huerfanas (HR vs KC/) | 2h | ✅ | — | 📝 Job diario: comparar OrangeHRM (active=false, termination_date < NOW()) vs Keycloak (enabled=true) vs  (active=true). Si ghost detectado → (1) revocar todas las sesiones inmediatamente, (2) desactivar KC (enabled=false), (3) soft-delete , (4) notificar al manager, (5) auditoria `ghost_account_detected`. Estadistica: 37% de organizaciones tienen ghost accounts (ISACA 2025). | ☐ | 0,1,2,3 | ISACA |
...
| **B38.T12** | **`SeedInitialAtoms`** — registrar átomos base en `bos_atom_catalog` | 4h | ✅ | — | 📝 INSERT de átomos iniciales para las 6 aplicaciones seed:  (Plan Cuentas, Comprobantes, Menús), OrangeHRM (Empleados), Saleor (Catálogo), CoreUI (sesion.ingresar), bSearch (busqueda.consultar), Sistema (sistema.sesion.ingresar). Usar `bos_build_atom_bitmask()` para calcular contextual_mask + logical_mask. Asignar atom_position secuencial inmutable. | ☐ | 0,1,2,3 | MANUAL-PRIV §8.1 |
...
## B41 — Reconciler bAuth → Keycloak — 3 átomos · ~12h

**Objetivo:** Sincronizar identidades de negocio desde bauth_db hacia Keycloak (authorization server) cada 60 segundos. **DoD:** Rol creado en bauth_db aparece en Keycloak en ≤60s. Rol desactivado en bauth_db se deshabilita en Keycloak.

**SSOT:** `BAUTH-PLAN-IMPLEMENTACION-GAPS.md` Fase G · `Policies_Authentication_Framework_v4.json` §5 (ecosystem_integration.bauth_db)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B41.T01** | `src/sync/_reconciler.rs` — `Reconciler` struct + `reconcile_all()` | 8h | ✅ | — | 📝 Bucle tokio::select: cada 60s ejecuta reconcile_all() o espera drain signal. reconcile_all(): (1) Client.get_res_users(active=true) → Vec<User>, (2) para cada user: KeycloakAdminClient.create_or_update_user(), (3) mapear res_company→group, res_branch→group, res_department→group, (4) mapear res_groups→bos_role (vía bAuth reconciler), (5) desactivar en KC usuarios que ya no están activos , (6) insertar en audit_log los cambios. Errores por usuario no detienen el ciclo. Métricas: `_sync_total`, `_sync_errors`. Ref: GAPS Fase G. | ☐ | 0,1,2,3 | GAPS G |
| **B41.T02** | `src/sync/_client.rs` — Cliente JSON-RPC para  | 2h | ✅ | — | 📝 Wrapper sobre reqwest para llamar al JSON-RPC de . Métodos: `get_res_users(domain, fields)`, `get_res_company()`, `get_res_branch()`, `get_res_department()`, `get_res_groups()`. Autenticación: api_key desde Vault KV v2 (`secret/bauth//api_key`). Timeout 10s. Retry 3 veces con backoff exponencial. Ref: GAPS Fase G. | ☐ | 0,1,2 | GAPS G |
...
**Total B41:** ~12h (3 átomos) · Entregable: sincronización bAuth→Keycloak cada 60s, con métricas y audit log.
...
**SSOT:** `BAUTH-AUTHENTICATION-FRAMEWORK.md` · `BAUTH-FRAMEWORK-CONTRASTE-KC.md` · `BAUTH-PLAN-IMPLEMENTACION-GAPS.md`
...
| **B43.T02** | Documento `BAUTH-FRAMEWORK-CONTRASTE-KC.md` — análisis de gaps | 3h | ✅ | `—` | 📝 Contraste completo: 15 métodos auth (15/15 KC), 12 protocolos (10/10 KC), 12 algoritmos (6/12 KC + 2 Vault + 4 bAuth), 24 compliance controls (16/24 KC, 6 parciales, 2 faltan).  analizado como fuente de identidad. 8 herramientas externas recomendadas. 3 gaps críticos. 350+ líneas. | ☐ | 3 | BAUTH-050 |
...
> **Impacto:** Cierra el 20% de gaps no cubiertos por Keycloak 
...
## B45 — Context Plane Vision: DDL + Templates + Motor de Contexto (30 átomos · ~80h)
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B45.B01** | **Crear 12 `ath_policy_d*`** + seeds. Políticas pre-diseñadas por dominio D1-D12. Extraer de `Policies_Authentication_Framework.json` (104KB) y `Authentication_Framework.json` (602KB). Estructura uniforme: `policy_id UUIDv7 PK`, `policy_name TEXT`, `policy_slug TEXT UNIQUE`, `config JSONB`, `standard_ref TEXT[]`. | 4h | 🔴 | — | 📝 D1: record_rules, scope, field_rules. D2: anti_passback, escort, two_person, mantrap. D3: dual_approval, sod, limits. D4: schedules, holidays, overtime. D8: session_ttl, reauth, ctx_id. D9: password, mfa, recovery, lockout, phishing. D11: retention, review_freq, hash_chain. | ☐ | 0,2,3 | NIST 800-63B-4, RFC 9470, OWASP ASVS V2 |
| **B45.B02** | **Crear 12 `ath_config_d*`** + seeds. Configuraciones por dominio D1-D12. Estructura: `config_id UUIDv7 PK`, `config_key TEXT UNIQUE`, `config_value JSONB`, `standard_ref TEXT`. | 3h | 🔴 | — | 📝 D1: token_ttl, rate_limit. D3: currency, sin_env, approval_timeout. D4: timezone_default, shift_max. D8: session_ttl_max, inactivity_timeout, reauth_timeout. D9: password_min_length, hibp_enabled, lockout_levels, rotation_days. | ☐ | 0,2,3 | NIST 800-63B-4, ISO 8601 |
| **B45.B03** | **Crear 12 `idn_role_d*`** — Templates de rol por dominio. Cada tabla: `role_id UUIDv7 PK`, `role_code TEXT UNIQUE`, `role_name JSONB`, `domain_code SMALLINT`, `config JSONB`, `parent_role_id UUID FK self`. | 4h | 🔴 | — | 📝 `config JSONB` contiene la porción del template v6.0 correspondiente a ese dominio. Ej: `idn_role_d1.config` = sección `logical_access`. `idn_role_d3.config` = sección `financial_limits`. | ☐ | 0,2,3 | ANSI/INCITS 359-2004 |
| **B45.B04** | **Crear tablas restantes:** `ath_auth_flow` + `ath_auth_flow_method` + `ath_step_up_rule` + `zone_field_restriction` + `zone_button_rule` + `zone_record_rule` + `zone_data_policy` + `_action_visibility` + `fis_zone_method_requirement` + `fis_emergency_config` + `cal_overtime_policy` + `cal_break_policy` + `net_ztna_policy` + `ses_risk_policy` + `ses_caep_config` + `sod_validation_config` + `conflict_interest_policy` | 5h | 🔴 | — | 📝 17 tablas complementarias. Ver `BAUTH-INVENTARIO-TABLAS-DECISION.md` BLOQUE 4 para especificación completa de cada una. | ☐ | 0,2,3 | RFC 9470, OpenID CAEP 1.0, NIST 800-207 ZTA |

### Bloque C — Seeds y Datos Reales (3 átomos · ~12h)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B45.C01** | **Seeds para las 46 tablas migradas.** Cada seed: idempotente (TRUNCATE + RESTART IDENTITY CASCADE + REINDEX + INSERT), datos reales desde estándares, FKs con subquery. | 5h | 🔴 | — | 📝 Priorizar seeds con datos de negocio que requieren revisión del humano: `org_empresa`, `org_sucursal`, `org_pos_logico`, `fin_decision_matrix`, `aud_compliance_map`. | ☐ | 0,2,3 | Ver BAUTH-SEED-PLAN.md |
| **B45.C02** | **Seeds para las 57 tablas nuevas.** Poblar `ath_policy_d*` desde los JSON frameworks, `ath_config_d*` con defaults por dominio, `idn_role_d*` con roles pre-configurados. | 4h | 🔴 | — | 📝 `idn_role_d1`: OPERADOR_CAJA, GERENTE_REGIONAL, AUDITOR, ADMIN_SISTEMA. `idn_role_d9`: AAL1_BASICO, AAL2_MFA, AAL3_HARDWARE, M2M_MTLS. | ☐ | 0,2,3 | Ver BAUTH-ROLTEMPLATE-SECCIONES.md |
| **B45.C03** | **Seed generator:** Actualizar `seed_idn_role_template_data.sql` para que genere templates usando la nueva arquitectura de merge (12 `idn_role_d*` → 1 `idn_role_template`). | 3h | 🔴 | — | 📝 El generador debe: (1) consultar las 12 tablas `idn_role_d*`, (2) para cada rol base, mergear las secciones de cada dominio, (3) producir el JSONB completo de 14 secciones. | ☐ | 0,2,3 | BAUTH-ROLTEMPLATE-SECCIONES.md v6.0 |

### Bloque D — Motor de Contexto: bAuth ↔ BOS Integration (3 átomos · ~12h)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B45.D01** | **Handler JSON-RPC `bauth.context.evaluate`.** Recibe ctx_id del BOS, evalúa los 12 dominios en orden D8→D9→D1→D3→D2→D10→D4→D6→D7→D5→D12→D11, retorna `DomainResult` con la decisión de cada dominio + trust_level + permisos efectivos. Latencia <5ms P99. | 5h | 🔴 | — | 📝 Usa el RolTemplate mergeado + PrivilegeEngine. Cache Redis TTL 30s. Si un dominio DENIEGA → cortocircuito. El resultado se expone en el Context API del BOS. | ☐ | 0,1,2 | SBOS-049, NIST 800-207 |
| **B45.D02** | **Función `merge_role_templates()`.** Recibe 12 role_id (uno por dominio), valida precedencia, conjuga las secciones en un JSONB completo de 14 secciones, retorna el template listo para `idn_role_template`. | 4h | 🔴 | — | 📝 Precedencia: D9>D8>D1>D3>D2>D10>D4>D6>D7>D5>D12>D11. Validación: cada dominio solo puede contribuir una vez. Conflictos se resuelven por precedencia. | ☐ | 0,1,2 | ANSI/INCITS 359-2004 §4 |
| **B45.D03** | **Reconcile loop extendido:** Además de sync Keycloak, el reconcile loop (60s) ahora también: (1) verifica drift en políticas por dominio, (2) re-evalúa contextos activos si hubo cambio de políticas, (3) detecta sesiones expiradas y las invalida, (4) emite eventos CAEP (session-revoked, assurance-level-change). | 3h | 🔴 | — | 📝 El reconcile loop actual solo sincroniza Keycloak. Extender para que sea el corazón del Context Plane runtime. | ☐ | 0,1,2 | OpenID CAEP 1.0, SBOS-049 |

### Bloque E — `ath_method` con Clasificación por Dominio (2 átomos · ~4h)

| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B45.E01** | **ALTER `ath_method`:** Agregar columna `domain_classification JSONB NOT NULL DEFAULT '{}'`. Cada método se etiqueta con los dominios donde aplica. | 1h | 🔴 | — | 📝 Ej: `{"D1":true,"D2":true,"D3":true,"D5":true,"D7":true,"D9":true}` para WEBAUTHN_PWDLESS. El admin filtra métodos por dominio al armar un RolTemplate. | ☐ | 0,2,3 | NIST 800-63B-4 |
| **B45.E02** | **Actualizar `seed_ath_method.sql`:** Clasificar los 40 métodos existentes con `domain_classification`. | 3h | 🔴 | — | 📝 Mapeo: PASSWORD→D1,D2,D9; TOTP→D1,D3,D9; WEBAUTHN_PWDLESS→D1,D2,D9; PASSKEY_DEVICE→D1,D2,D3,D9; SMARTCARD_X509→D1,D2,D3,D9; OAUTH_M2M→D7,D9; BACKUP_CODES→D9; EMAIL_OTP→D9; MAGIC_LINK→D1,D9; CONDITIONAL_OTP→D1,D3,D9; CIBA_DECOUPLED→D1,D9. Métodos físicos (QR_DYNAMIC, NFC, FINGERPRINT, FACE) → D2,D5. | ☐ | 0,2,3 | NIST 800-63B-4 |
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B45.G01** | **Verificar cobertura del template v6.0:** Ejecutar el generador de seeds contra cada dominio. Validar que cada sección del template tiene datos poblados desde las tablas correspondientes. | 3h | 🔴 | — | 📝 Query de verificación: `SELECT template->'logical_access'->'zones' FROM idn_role_template WHERE id='ROL-ORG-CAJ'` debe retornar datos reales, no `null` ni `{}`. | ☐ | 0,1,2 | BAUTH-ROLTEMPLATE-SECCIONES.md |
...
| **B45.G03** | **Actualizar `BAUTH-D1-MANUAL-COMPLETO.md`:** Reflejar la nueva estructura de 155 tablas, templates por dominio, políticas y configuraciones separadas, y la integración con el Context Plane del BOS. | 3h | 🔴 | — | 📝 El manual debe ser la bauth_db para cualquier desarrollador que necesite entender el ecosistema de tablas de bAuth. | ☐ | 0,2,3 | ISO 27001 A.8.15 |
...
## B46 — Cierre de Migración: Seeds + Clasificación + Organización (7 átomos · ~22h)
...
| ID | Átomo | E | Estado | Commit | Solución / Bitácora | Rev | G | D |
|---|---|---|---|---|---|---|---|---|
| **B46.B01** | **ALTER `ath_method`** — agregar columna `domain_classification JSONB DEFAULT '{}'`. Actualizar `seed_ath_method.sql` para clasificar los 32 métodos por dominio. | 2h | 🔴 | — | 📝 Mapeo: PASSWORD→D1,D2,D9; TOTP→D1,D3,D9; WEBAUTHN_PWDLESS→D1,D2,D9; PASSKEY_DEVICE→D1,D2,D3,D9; SMARTCARD_X509→D1,D2,D3,D9; CLIENT_CREDENTIALS→D7,D9; TOKEN_EXCHANGE→D7,D9; BACKUP_CODES→D9; EMAIL_OTP→D9; CIBA→D1,D9; TOUCH_ID→D2,D5,D9; FACE_ID→D2,D5,D9; etc. | ☐ | 0,2,3 | NIST 800-63B-4 |
...
## B47 — CIERRE DE GAPS DEL MANUAL v18.0 + PANELES DASHBOARD + CORRECCIONES DDL (26 átomos · ~62h)
...
| **B47.D10** | **Panel 7 — Sync Status (Keycloak) + Panel 8 — ctx_id + Menús.** Widgets Panel 7: estado sync KC (✅/❌), estado sync  (✅/❌), último sync timestamp, delta de drift, log de sync (`sync_log`). Widgets Panel 8: sesiones activas por ctx_id, transferencias de contexto (`ses_context_switch`), editor de menús (árbol jerárquico 105 ítems drag&drop), editor de contextos ENUM (57 dropdowns). | 2h | 🔴 | — | 📝 Paneles 7+8 combinados. Panel 7 monitorea la simbiosis bAuth-KC-. Panel 8 administra el Context Plane y la navegación. | ☐ | 0,1,2,3 | SBOS-049, ADR-020 |
...
| **B47.E03** | **Documentar Redis Cache Schema (G7).** El manual §38.3 G7 identifica que Redis cache schema no está documentado. Crear `BAUTH-REDIS-CACHE-SCHEMA.md` documentando: DB0 (ctx_id sessions, TTL 8h), DB1 (Rol BitMask cache, TTL 30s), DB2 (PolicyEngine, TTL 5min), DB3 (Permission cache, TTL 30s), DB4 (Rate limit counters, TTL 1min), DB5 (OAuth2-Proxy sessions, TTL 8h). Estructura de cada key, size estimado, invalidación triggers. | 2h | 🔴 | — | 📝 Gap G7 del manual. Redis es cache volátil — PostgreSQL es bauth_db. Pero el equipo necesita saber qué hay en Redis y cómo invalidarlo. | ☐ | 0,3 | Redis 8.6.2 Best Practices |
