...
```bash
# Full cycle test
podman rm -f sbos-greenfield
podman run -d --name sbos-greenfield --privileged --cgroupns=private \
  -v /sys/fs/cgroup:/sys/fs/cgroup:rw \
  -v /opt/skull/orquestador/proyectos/desarrollo/sbos/BosAgent/staging:/staging:ro \
  --hostname sbos-greenfield localhost/ubuntu-systemd:26.04 /sbin/init

sleep 5
podman exec sbos-greenfield bash /staging/install.sh --zip /staging/sbos-bootstrap.zip &
sleep 120

# Verificar las 3 fichas
for f in sbos-bootstrap-os sbos-bootstrap-k8s sbos-lifecycle; do
  podman exec sbos-greenfield bosctl status $f
done

# Verificar K8s
podman exec sbos-greenfield kubectl get nodes

# Test stop
podman exec sbos-greenfield systemctl stop bos.service
sleep 15
podman exec sbos-greenfield ps aux | grep -E 'kubelet|containerd' | grep -v grep
# Esperado: vacío

# Test start
podman exec sbos-greenfield systemctl start bos.service
sleep 30
podman exec sbos-greenfield kubectl get nodes | grep Ready
```
