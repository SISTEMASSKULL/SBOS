...
**Estándares:** ISO 27001 · NIST 800-63B-4 · PCI DSS 4.0 · OWASP ASVS 5.0 · RFC 9562
...
### 2.3 Prefijos del Schema bauth

| Prefijo | Dominio | Tablas | Significado |
|---------|---------|--------|-------------|
| **idn_** | Identidad | 35 | Identity & Organization |
| **ath_** | Autenticación | 15 | Authentication |
| **ses_** | Sesiones | 2 | Session (Context Plane) |
| **fin_** | Financiero | 6 | Financial |
| **aud_** | Auditoría | 7 | Audit & Compliance |
| **sec_** | Seguridad | 9 | Security & Physical |
| **geo_** | Geografía | 5 | Geography |
| **cfg_** | Configuración | 4 | Configuration |
| **blk_** | Blockchain | 7 | Blockchain (D12) |
| **privilege_** | Privilege Engine | 11 | Motor de privilegios H-RBAC (de `bos_privilege`) |
...
| bauth | bos_tenant_verification | bAuth | idn_tenant_verification | Verificación KYC del tenant |
| bauth | bos_tenant_domain | bAuth | idn_tenant_domain | Dominios FQDN por tenant |
| bauth | bos_tenant_network | bAuth | idn_tenant_network | Redes y CIDRs autorizadas por tenant |
| bauth | bos_tenant_currency | bGlobal | global_tenant_currency | Monedas habilitadas por tenant (catálogo ISO 4217) |
| bauth | bos_tenant_language | bGlobal | global_tenant_language | Idiomas habilitados por tenant (catálogo ISO 639/BCP 47) |
| bauth | bos_tenant_gestion | bCalendar | cal_interval | Períodos de gestión contable por tenant → fusión en cal_interval |
...
| bauth | bos_pais | bAuth | geo_pais | Países (ISO 3166-1: BO, AR, BR, CL, PE...) |
| bauth | bos_ciudad | bAuth | geo_ciudad | Ciudades y divisiones administrativas |
| bauth | bos_moneda | bGlobal | global_currency | Catálogo de monedas ISO 4217 (BOB, USD, EUR...) |
| bauth | bos_idioma | bGlobal | global_language | Catálogo de idiomas ISO 639 / BCP 47 (ES, EN, PT, QU, AY, GN) |
| bauth | bos_timezone | bAuth | geo_timezone | Zonas horarias (IANA TZ Database) |
...
| Prefijos | 1 (`bos_`) | **10** (idn_, ath_, ses_, fin_, aud_, sec_, geo_, cfg_, blk_, privilege_) |
...
|-------|-----------|----------|
| `bos_idioma` → `bGlobal.global_language` | Catálogo de idiomas | ISO 639 / BCP 47 (RFC 5646) |
| `bos_moneda` → `bGlobal.global_currency` | Catálogo de monedas | ISO 4217 |
| `bos_tenant_language` → `bGlobal.global_tenant_language` | Idiomas habilitados por tenant | — |
| `bos_tenant_currency` → `bGlobal.global_tenant_currency` | Monedas habilitadas por tenant | — |
...
## 6. Diagnóstico y Estado Actual — Junio 22, 2026

### 6.1 Lo que FUNCIONA (no tocar)

| Componente | Estado | Detalle |
|-----------|--------|---------|
| `bauth_db` (VPS producción) | 🟢 85 tablas | Base de datos en producción, funcionando |
| `.bak` (diseño DDL) | 🟢 103 tablas | Fuente de verdad del diseño completo |
| UUID en PKs | 🟢 Aplicado | 103 PKs son UUID DEFAULT gen_random_uuid() |
| Seeds extraídos | 🟢 4 archivos | 050_geo.sql, 051_tier_policy.sql, 052_financial.sql, 053_security.sql |
| Seeds VPS respaldo | 🟢 2 archivos | 033_vps_data_resguardo.sql (8805 líneas), 034_vps_completo.sql (9355 líneas) |
| Plan de calendario | 🟢 Documentado | 9 tablas definidas en §4 |
| Estándares documentados | 🟢 12 estándares | §6 con controles específicos |
| Prefijos y schemas | 🟢 Definidos | §2 con 8 prefijos + bGlobal + bos |

### 6.2 Lo que FALTA (priorizado)
...
| 🟡 P4 | `bauth_test` inconsistente | No se puede probar | Recrear desde cero con el DDL corregido |
| 🟡 P5 | Tablas del calendario no creadas | Calendario solo en papel | Agregar CREATE TABLEs al .bak o como archivo separado |
| 🟢 P6 | Seeds del calendario | Festivos Bolivia, gestiones base | Crear seeds 060+, 061+ con datos de referencia |

### 6.3 Reglas para la DDL final
...
### 6.4 Plan de Restauración de Integridad Referencial

**Objetivo:** El .bak perdió las REFERENCES (fueron eliminadas por sed). Hay que restaurarlas desde git (`git show HEAD:db/migrations/001_bauth_init.sql`) y verificar que cada FK apunte a una tabla existente con el tipo correcto (UUID).

#### Paso 1: Recuperar REFERENCES desde git
...
# Extraer solo las REFERENCES del original
grep -o 'REFERENCES [a-z_]*\.[a-z_][a-z0-9_]*([a-z_][a-z0-9_, ]*)' /tmp/ddl_original.sql | sort -u
```

**Resultado esperado:** ~104 REFERENCES extraídas. Cada una: `REFERENCES schema.tabla(columna)`.

#### Paso 2: Verificar que cada FK apunta a tabla existente

Para cada REFERENCES extraída:
1. Identificar la tabla origen (CREATE TABLE donde aparece)
2. Identificar la tabla destino y columna referenciada
3. Verificar que la tabla destino EXISTE en el .bak
4. Verificar que el tipo de la columna origen coincide con el tipo de la columna destino (UUID)
5. Verificar que la tabla destino se crea ANTES que la tabla origen (orden topológico)

**Script de verificación:**
```bash
for ref in $(grep -o 'REFERENCES [a-z_]*\.[a-z_][a-z0-9_]*([a-z_][a-z0-9_, ]*)' 001_bauth_init.sql.bak); do
  dest_table=$(echo $ref | sed 's/REFERENCES //' | sed 's/(.*//')
  grep -q "CREATE TABLE.*$dest_table" 001_bauth_init.sql.bak || echo "❌ $ref → tabla NO EXISTE"
done
```

#### Paso 3: Insertar REFERENCES en el .bak

Para cada CREATE TABLE en el .bak, restaurar las REFERENCES que le corresponden (extraídas del git original).

**Regla:** Si una REFERENCES causaría error de orden (tabla referenciada se crea después), mover la tabla a la posición correcta según el orden topológico (tsort).

#### Paso 4: Verificar que todos los tipos coinciden (UUID ↔ UUID)

Con la estandarización UUID, todas las FKs deben ser `UUID REFERENCES schema.tabla(UUID_PK)`. Verificar que ningún TEXT FK quede apuntando a un UUID PK.

### 6.5 Plan de Verificación de Normalización

**Objetivo:** Verificar que el esquema cumple 1NF, 2NF, 3NF después de restaurar las REFERENCES. Los desvíos previos (7 documentados) deben seguir siendo los mismos.

#### Verificación 1NF

| # | Verificación | Método |
|---|-------------|--------|
| 1 | Toda tabla tiene PK | `grep -L "PRIMARY KEY"` en cada CREATE TABLE |
| 2 | No hay grupos repetidos | Arrays TEXT[] son atómicos en PostgreSQL. JSONB es documento atómico. |
| 3 | Cada columna tiene tipo definido | Revisar que no haya TEXT genérico sin newtype |

#### Verificación 2NF

| # | Verificación | Método |
|---|-------------|--------|
| 1 | Tablas con PK compuesta sin dependencias parciales | Revisar `bos_role_atom`, `bos_atom_catalog`, `bos_rol_closure` |
| 2 | `atom_position` en `bos_role_atom`: ¿depende solo de atom_code? | Sí — desnormalización documentada por rendimiento |

#### Verificación 3NF

| # | Verificación | Método |
|---|-------------|--------|
| 1 | `tier → loa_required, mfa_required` | `bos_tier_policy` ya resuelve esto (tabla separada) |
| 2 | `mask_own_hex → sam128_*` | Columnas cacheadas por PrivilegeEngine — documentado |
| 3 | Jerarquía Context Plane (sucursal→empresa→tenant) | Desnormalización aceptada por SBOS-049 |
| 4 | `sync_status → sync_error` | Columna de diagnóstico — NULL si != ERROR |

#### Verificación de tipos post-UUID

| # | Verificación | Método |
|---|-------------|--------|
| 1 | 0 TEXT PKs (excepto natural keys) | `grep -c "TEXT.*PRIMARY KEY"` |
| 2 | 0 SERIAL/BIGSERIAL PKs | `grep -c "SERIAL.*PRIMARY KEY"` |
| 3 | 103 UUID PKs con DEFAULT gen_random_uuid() | `grep -c "UUID.*PRIMARY KEY.*DEFAULT gen_random_uuid()"` |
| 4 | Todas las FKs son UUID → UUID | Sin TEXT FK apuntando a UUID PK |

### 6.6 Orden de Ejecución Actualizado

| # | Tarea | Estado | Depende de |
|---|-------|--------|-----------|
| 1 | Leer PROTOCOLO-COMUNICACION-AGENTE.md | ✅ | — |
| 2 | **Recuperar REFERENCES desde git** | 🔴 | — |
| 3 | **Verificar integridad referencial** (104 FKs) | 🔴 | #2 |
| 4 | **Verificar normalización 1NF/2NF/3NF post-UUID** | 🔴 | #2, #3 |
| 5 | **Insertar REFERENCES en .bak con orden topológico** | 🔴 | #3, #4 |
| 6 | Integrar 80 columnas ALTER TABLE en CREATE TABLE | 🔴 | #2 |
| 7 | Re-escribir `build_ddl.sh` con orden topológico | 🔴 | #5 |
| 8 | Probar en `bauth_test` (DROP + CREATE) hasta 0 errores | 🔴 | #7 |
| 9 | Probar idempotencia (2ª ejecución) | 🔴 | #8 |
| 10 | Agregar tablas del calendario al .bak | 🟡 | #8 |
| 11 | Probar seeds (050-053) sobre `bauth_test` | 🟡 | #8 |
...
| 4.2 | Ejecutar DDL | **0 ERRORES** | ⚪ |
| 4.3 | Ejecutar seeds en orden | **0 ERRORES** | ⚪ |
| 4.4 | Verificar integridad referencial | **0 FK violations** | ⚪ |
| 4.5 | Verificar normalización 1NF/2NF/3NF | Cumplimiento confirmado | ⚪ |
| 4.6 | Ejecutar DDL segunda vez (idempotencia) | **0 ERRORES**, solo NOTICEs | ⚪ |
| 4.7 | Ejecutar seeds segunda vez (idempotencia) | **0 ERRORES**, ON CONFLICT | ⚪ |
| 4.8 | Si hay errores → corregir → volver a 4.1 | Iterar hasta 0 errores | ⚪ |
...
|---|---------------|-------------|--------|--------|
| 1 | `bauth.bos_tenant` | `bAuth.idn_tenant` | bAuth | Entidad raíz multi-tenant. Prefijo idn_ = identidad. |
...
| # | Nombre Actual | Nombre Nuevo | Schema | Motivo |
|---|---------------|-------------|--------|--------|
| 68 | `bauth.bos_pais` | `bAuth.geo_pais` | bAuth | Países (ISO 3166-1). Referencia geográfica de bAuth. |
| 69 | `bauth.bos_ciudad` | `bAuth.geo_ciudad` | bAuth | Ciudades y divisiones administrativas. |
| 70 | `bauth.bos_timezone` | `bAuth.geo_timezone` | bAuth | Zonas horarias IANA TZ Database. |
...
| # | Nombre Actual | Nombre Nuevo | Schema | Motivo |
|---|---------------|-------------|--------|--------|
| 93 | `bauth.bos_idioma` | `bGlobal.global_language` | bGlobal | Catálogo ISO 639/BCP 47. Uso transversal: bAuth, bCalendar, bnotify, biedata. |
| 94 | `bauth.bos_moneda` | `bGlobal.global_currency` | bGlobal | Catálogo ISO 4217. Uso transversal: bAuth, bRates, biedata. |
...
| 96 | `bauth.bos_tenant_currency` | `bGlobal.global_tenant_currency` | bGlobal | Monedas habilitadas por tenant. Referencia global no exclusiva de bAuth. |
...
## ANNEXO C — Propuesta de Stack Open-Source para Calendario y Notificaciones

> **Estado:** PENDIENTE DE APROBACIÓN — 2026-06-22
> Esta propuesta reemplazaría el enfoque de DDL personalizado para las tablas de eventos
> de bCalendar. Requiere decisión del humano antes de implementar.

### C.1 Principio

No construir lo que ya existe. Usar aplicaciones open-source maduras como fichas BOS
y personalizarlas para las necesidades SBOS. Solo construir DDL custom para la lógica
de negocio que ninguna herramienta existente cubre (períodos fiscales, compliance, D4/D10).

---

### C.2 Notificaciones → Novu + FerretDB 2.0

**Novu**
- GitHub: [github.com/novuhq/novu](https://github.com/novuhq/novu)
- Licencia: MIT
- Estrellas: 37K+ · commits semanales · versión v3.15.x (junio 2026)

**FerretDB 2.0**
- GitHub: [github.com/FerretDB/FerretDB](https://github.com/FerretDB/FerretDB)
- Licencia: Apache 2.0
- GA producción: marzo 2025 (testeado con Novu en producción — artículo oficial)

#### Canales que cubre Novu (reales, no roadmap)

| Canal | Proveedores |
|-------|------------|
| Email | SendGrid, SMTP genérico, SES, Mailgun, Resend |
| SMS | Twilio, Telnyx, Plivo, AWS SNS, +11 más |
| WhatsApp Business API | Meta Cloud API (nativo desde mayo 2024) |
| Push Web/Mobile | FCM, APNS, OneSignal |
| In-App (inbox) | Componente React embebible + WebSocket real-time |

#### Lo que Novu da sin programar nada

- Workflow engine con delays, digest, reintentos con backoff exponencial
- Activity feed por mensaje: canal, proveedor, timestamps, payload, error → **delivery log WORM**
- Preferencias de usuario por canal (quiet hours, frecuencia)
- Motor de plantillas versionadas (visual drag-and-drop + code editor)
- Multi-tenant nativo (Organizations + Environments separados)
- `transactionId` en cada trigger → **enganche directo al ctx_id del SBOS-049**

#### Solución al problema de MongoDB

Novu usa MongoDB internamente. FerretDB 2.0 implementa el protocolo wire de MongoDB 5.0+
y almacena todo en PostgreSQL via extensión `DocumentDB` de Microsoft. Novu se conecta a
FerretDB exactamente igual que a MongoDB — cero cambios en código de Novu.

**Resultado:** todos los datos de Novu (plantillas, subscribers, delivery logs, workflows)
viven en el **PostgreSQL de S01** del SBOS. Sin base de datos separada.

#### Integración con el Context Plane (SBOS-049)

```json
POST /v1/events/trigger
{
  "name": "cal-alarm-vencimiento-asfi",
  "to": { "subscriberId": "user-uuid" },
  "payload": { "deadline": "2026-09-30", "tipo": "ASFI" },
  "transactionId": "ctx_id-del-sbos-049"
}
```

#### Impacto en sbos-notifier

Reemplaza el stub Python actual. Los puertos no cambian:
`:28200` (API JSON-RPC) · `:28201` (WebSocket) · `:28202` (MFA Step-Up)

---

### C.3 Calendario → SOGo + Nager.Date

**SOGo**
- GitHub: [github.com/Alinto/sogo](https://github.com/Alinto/sogo)
- Licencia: LGPL-2.1 / GPL-2.0
- Versión: v5.12.8 — mayo 2026 (parches de seguridad activos)
- Mantenido por Alinto desde 2012

**Nager.Date**
- GitHub: [github.com/nager/Nager.Date](https://github.com/nager/Nager.Date)
- Licencia: MIT · self-hosted (Docker) · Bolivia ✅ Perú ✅ Colombia ✅ +100 países

#### Por qué SOGo gana sobre los demás

| Candidato | Motivo de descarte |
|-----------|-------------------|
| Cal.com | Separó multi-tenant a repo privado (abril 2026). Cal.diy es "solo uso personal". |
| Radicale | Almacenamiento en archivos. Sin PostgreSQL. Sin API REST. |
| Baïkal | Solo MySQL/SQLite. Sin soporte PostgreSQL oficial. |
| Nextcloud Calendar | Requiere toda la suite Nextcloud. Demasiado pesado para lo que se necesita. |
| DAViCal | PostgreSQL nativo, pero en modo mantenimiento mínimo desde 2014. Riesgo de abandono. |
| Bedework | Java + Hibernate, PostgreSQL soportado pero API REST escasa y despliegue K8s complejo. |

SOGo: PostgreSQL como primera clase, CalDAV RFC 4791 completo (+ RFC 6638 Auto-Scheduling),
iCalendar RFC 5545 completo (RRULE, EXDATE, VALARM, VTIMEZONE), multi-tenant Domain Separation,
Kubernetes HA-ready con imagen Docker oficial.

#### PostgreSQL nativo — apunta al cluster existente

```
SOGoProfileURL   = "postgresql://bos_admin@localhost:5432/skSBOS_db/sogo_user_profile"
OCSFolderInfoURL = "postgresql://bos_admin@localhost:5432/skSBOS_db/sogo_folders"
OCSStoreURL      = "postgresql://bos_admin@localhost:5432/skSBOS_db/sogo_store"
OCSAclURL        = "postgresql://bos_admin@localhost:5432/skSBOS_db/sogo_acl"
```

SOGo crea sus propias tablas en `skSBOS_db`. Sin base de datos separada.

#### Integración de identidad — bAuth como capa unificada

SOGo, Dovecot y Roundcube **no tienen base de usuarios propia**. Los tres apuntan al
realm KC que bAuth administra. bAuth emite el JWT con sus 5 SPIs (BitMask, Step-Up,
Geo, Financial, ctx_id). SOGo y Dovecot validan ese mismo token — sin LDAP, sin
passwords en archivos de configuración.

```
Usuario → SOGo/Roundcube → bAuth KC (realm tenant-{id})
                            │  5 SPIs activos (BitMask, Step-Up,
                            │  Geo, Financial, ctx_id)
                            └── JWT firmado con claims completos
                                      │
              ┌───────────────────────┴───────────────────────┐
              ▼                                               ▼
    SOGo valida JWT                                Dovecot valida JWT
    SOGoAuthenticationType = oauth2                módulo auth_oauth2
    → KC /userinfo endpoint                        → KC /token/introspect
```

**Configuración SOGo** (extracto `sogo.conf`):
```
SOGoAuthenticationType = oauth2;
SOGoOAuth2ClientID      = "sogo";
SOGoOAuth2ClientSecret  = "{{ vault:sogo/client-secret }}";
SOGoOAuth2AuthorizationURL = "https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/auth";
SOGoOAuth2TokenURL         = "https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/token";
SOGoOAuth2UserinfoURL      = "https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/userinfo";
```

**Configuración Dovecot** (`auth-oauth2.conf.ext`):
```
passdb {
  driver = oauth2
...
# introspection_url = https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/token/introspect
# introspection_mode = post
# client_id     = dovecot
# client_secret = {{ vault:dovecot/client-secret }}
```

**Configuración Roundcube** (`config.inc.php`):
```php
$config['oauth_provider']         = 'generic';
$config['oauth_client_id']        = 'roundcube';
$config['oauth_client_secret']    = '{{ vault:roundcube/client-secret }}';
$config['oauth_auth_uri']         = 'https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/auth';
$config['oauth_token_uri']        = 'https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/token';
$config['oauth_identity_uri']     = 'https://auth.sbos.local/realms/tenant-{id}/protocol/openid-connect/userinfo';
```

Los tres `client_secret` se almacenan en **Vault S04** y los inyecta BOS en el momento
del despliegue. Ningún secret vive en disco.

#### Multi-tenant Domain Separation

Cada empresa tiene su propia fuente de autenticación, sus propios calendarios y recursos.
Los usuarios de la empresa A no acceden a datos de la empresa B. Cada tenant tiene su
propio realm KC (`tenant-{id}`) administrado por bAuth (ADR-007). SOGo se configura con
un `SOGoUserSources` por dominio apuntando al realm correspondiente.

#### Feriados LATAM con Nager.Date

Nager.Date expone una API REST self-hosted:
```
GET /api/v3/PublicHolidays/2026/BO  → JSON feriados Bolivia 2026
GET /api/v3/IcsFile/2026/BO         → archivo .ics importable directamente en SOGo
```
Los feriados se importan como calendarios suscritos en SOGo por tenant y país.
Actualización anual via cronjob BOS. bAuth D4 también los puede consultar via CalDAV REPORT.

#### Cobertura de los 5 casos de uso SBOS

| Caso | Solución |
|------|---------|
| Empleado X solo opera 08:00-17:00 L-V | VEVENT con RRULE en calendario de políticas. bAuth hace REPORT CalDAV para verificar. |
| Feriado Bolivia bloquea operaciones | Nager.Date → .ics → SOGo. bAuth consulta REPORT CalDAV. D4 bloquea. |
| Año fiscal empresa Y cierra 31/03/2026 | VEVENT con DTSTART/DTEND en calendario fiscal del tenant. biedata verifica antes de registrar transacciones. |
| Notificar 30 días antes del vencimiento ASFI | VALARM en SOGo → dispara Novu (workflow "vencimiento-asfi") → WhatsApp/Email. |
| Recertificación de accesos cada 90 días (ISO 27001) | VEVENT recurrente RRULE:FREQ=DAILY;INTERVAL=90. bAuth detecta y activa campaña. |

---

### C.4 Impacto en bCalendar DDL — de 13 tablas a 7

Con SOGo, 6 tablas personalizadas del ANNEXO B desaparecen:

| Tabla eliminada | Reemplazada por SOGo |
|----------------|---------------------|
| `cal_calendar` | SOGo calendar collections (OCS) |
| `cal_event` | SOGo VEVENT en CalDAV store |
| `cal_instance` | SOGo recurrence expansion automática |
| `cal_exception` | SOGo EXDATE / RECURRENCE-ID |
| `cal_attendee` | SOGo ATTENDEE + RSVP nativo |
| `cal_alarm` | SOGo VALARM → dispara Novu |

**Quedan 7 tablas custom** — lógica de negocio SBOS que SOGo no puede manejar:

| # | Tabla | Razón de permanencia |
|---|-------|---------------------|
| 1 | `cal_interval` | Máquina de estados OPEN→SOFT_CLOSED→HARD_CLOSED. Herencia jerárquica tenant→empresa→sucursal. Concepto fiscal que no existe en CalDAV. |
| 2 | `cal_schedule` | Turnos laborales para bAuth D4. D4 la consulta directo en SQL para velocidad. |
| 3 | `cal_holiday` | La puebla Nager.Date via seed. D4 la consulta directo en SQL para bloquear accesos. |
| 4 | `cal_compliance_deadline` | Vencimientos ASFI/UIF + D9 rotación. Flujo PENDING→IN_PROGRESS→COMPLETED→OVERDUE. |
| 5 | `cal_review_campaign` | ISO 27001 A.9.2.5 workflow de recertificación cada 90 días. |
| 6 | `cal_delegation_window` | D10 auto-revocación de delegaciones temporales bAuth. |
| 7 | `cal_audit_log` | Log bi-temporal ISO SQL:2011 de todas las operaciones bCalendar. |

---

### C.5 Arquitectura resultante

```
┌──────────────────────────────────────────────────────────────┐
│                   SBOS — Stack Calendar + Notif              │
│                                                              │
│  bAuth D4 ───REPORT CalDAV───► SOGo (S06 :9060-:9062)       │
│                                │ LGPL-2.1 · v5.12.8         │
│                                │ CalDAV RFC 4791             │
│                                │ RFC 5545 VEVENT+RRULE       │
│                                │ Multi-tenant domains        │
│                                ▼                             │
│                    PostgreSQL S01 (skSBOS_db)                │
│                    ├── sogo_store (eventos)                  │
│                    ├── sogo_acl                              │
│                    └── bCalendar (7 tablas custom)           │
│                                ▲                             │
│  Nager.Date ──.ics──► SOGo (feriados Bolivia/LATAM)          │
│  (S06 :9063 · MIT)                                           │
│                                                              │
│  bCalendar VALARM ──JSON-RPC──► Novu (S06 :28200-:28202)     │
│                                 │ MIT · 37K+ ★               │
│                                 │ Email, SMS, WhatsApp       │
│                                 │ Push, In-App               │
│                                 │ transactionId = ctx_id     │
│                                 ▼                            │
│                           FerretDB 2.0                       │
│                           │ Apache 2.0 · GA 2025            │
│                           │ MongoDB protocol →              │
│                           ▼ PostgreSQL storage              │
│                    PostgreSQL S01 (notifier_db)              │
└──────────────────────────────────────────────────────────────┘
```

---

### C.6 Fichas BOS a crear/actualizar

| Ficha | Server | Puertos | Acción |
|-------|--------|---------|--------|
| `sbos-notifier` | S06 | :28200-:28205 | **Actualizar** — reemplazar stub Python por Novu + FerretDB 2.0 |
| `sogo` | S06 | :9060-:9062 | **Nueva** — servidor CalDAV multi-tenant |
| `nager-date` | S06 | :9063 | **Nueva** — API feriados LATAM (sidecar liviano) |

> **Si se aprueba esta propuesta**, los pasos siguientes serían:
> 1. Actualizar ANNEXO A y ANNEXO B — reducir bCalendar a 7 tablas custom
> 2. Crear fichas `sogo` y `nager-date` en `servers/S06/`
> 3. Actualizar `sbos-notifier` con Novu + FerretDB en `task_catalog.sh`
> 4. Actualizar el DDL `002_bauth_reconstruccion.sql` con solo las 7 tablas custom
