...
**Fuentes:** K8s docs (KubeletInUserNamespace), rootlesscontaine.rs, KIND source, experimentación en sbos-greenfield
...
```bash
podman exec sbos-greenfield grep "restrict_oom_score_adj" /etc/containerd/config.toml
...
```bash
podman exec sbos-greenfield systemctl is-active containerd
# → active
```
...
```bash
podman exec sbos-greenfield cat /sys/fs/cgroup/cgroup.subtree_control
...
```bash
podman exec sbos-greenfield systemctl is-active cgroup-delegate-cpu.service
# → active
```
...
```bash
podman exec sbos-greenfield bash -c 'mkdir -p /sys/fs/cgroup/test-inherit && cat /sys/fs/cgroup/test-inherit/cgroup.controllers && rmdir /sys/fs/cgroup/test-inherit'
