...
# PARTE V — COMPLETAR EL PRODUCTO Y ASCENDER A LA CÚSPIDE
## Detalle de los átomos pendientes: complementos de F0-F10 + fases F11-F17
## Agregado en Junio 2026 · El REGISTRO-ESTADO.md es la fuente del estado

---

## V.0 — Principios

Las fases F0-F10 repararon el daemon (89 átomos ✅). Esta parte detalla el
trabajo restante, organizado donde temáticamente pertenece: los complementos
de fases existentes conservan su numeración de fase (F3.11+, F5.7+, F6.12,
F10.10) y el trabajo genuinamente nuevo ocupa las fases F11-F17.

Aplican sin cambios: políticas SFP-01..06, DoD Universal (§2.2), protocolo
de sesión, gates de GESTION-RIESGOS-OPERATIVOS.md. Un átomo = un commit.

**Regla del despliegue (F12-F16):** cada capa se certifica con sus criterios
C-XX (BOS-REPAIR-01) ANTES de instalar la siguiente.

**Aclaración de estados:** SBOS-019 (documento contextual inicial) describía
5 estados de ficha. Ese modelo fue REEMPLAZADO por la máquina de 18 estados
de ADR-021 (`internal/state/manager.go`). Los conceptos útiles sobreviven
como derivados: "BLOQUEADA" la calcula el resolver (F11.2) y "ACTUALIZACIÓN
DISPONIBLE" el drift detector (F11.7). Ningún estado nuevo se agrega.

**Prioridad al recuperar SSH del staging:** F10.10 y F0.6.S antes que todo.

---

## V.1 — Complemento FASE 3 (bloque 3.B): renders reales de las 15 pantallas

F3.8 entregó la arquitectura correcta (dispatcher + 15 archivos) con renders
stub. El rendering real vive en `_legacy/` y en
`_snapshots/ORIGINAL_pre-reparacion_*/cmd/bosctl/install_ui.go`.
**Garantía: paridad visual con el original, sobre la arquitectura TEA pura.**

### F3.11 — Auditoría de paridad
Para cada pantalla S00-S14: comparar stub actual vs view* original del
snapshot → `internal/tui/PARIDAD.md` (pantalla | función legacy | líneas |
stub/parcial/completo).
**DoD:** 15 filas verificadas contra `_snapshots/`.

### F3.12 a F3.16 — Migración de renders por grupo
```
F3.12 — Splash  (S00 welcome, S14 goodbye)
F3.13 — Wizard  (S01-S04) con validación de formularios y MFA real
F3.14 — Install (S05 3-columnas + buildColA/B/C + vpA/B/C, S06 log,
        S07 panel de error) — el más complejo
F3.15 — Post    (S08 done, S09 reboot countdown, S10 boot)
F3.16 — Runtime (S11 dashboard, S12 logs con filtros, S13 shutdown)
```
**DoD por grupo:** render no-stub verificado contra legacy ·
`TestScreen<N>_RenderSinPanic` · `TestScreen<N>_ParidadGolden` (golden files
a 80×24 y 120×40) · `bosctl install --demo` muestra la pantalla real ·
race ×10 · TEA puro (POLICY.md reglas 1-7).

### F3.17 — Modularidad garantizada
1. POLICY.md con el procedimiento exacto de adición (7 pasos)
2. Plantilla `screens/_template.go.tmpl`
3. Generador `bosctl dev new-screen <nombre> --group=<grupo>` (crea archivo,
   registra constante Screen, caso del dispatcher, KeysFor y test)
4. Test guardián `TestScreens_TodasRegistradasEnDispatcher`
**DoD:** pantalla S15-demo creada con el generador en <5 min, build+tests
verdes, eliminada después — documentado en PARIDAD.md.

---

## V.2 — Complemento FASE 5 (bloque 5.B): Context Plane efectivo

### F5.7 — Suite contra PG+Redis reales
Aplicar el DDL de bkernel_db (EJECUCION-F5.1-F5.3; en staging lo hace F12.5)
y correr la suite completa contra instancias reales: registro → promoción →
switch → TTL → invalidación → multi-tenant → trace.
**DoD:** integración verde · `device.register` <2s (C-13) · cache-first
verificado (hit de Redis sin tocar PG).

### F5.8 — Bajo carga
500 dispositivos, 100 promociones concurrentes, 50 switches/s × 60s.
**DoD:** race verde ×3 · bitmask_cero_count=0 · métricas bos_ctx_* visibles.

---

## V.3 — Complemento FASE 6: catálogo certificado

### F6.12 — `docs/RPC-CATALOG.md` generado desde rpcRegistry
Método | auth | timeout | categoría | ejemplo curl | probado-en-staging ✓/✗.
**DoD:** 100% de métodos probados en staging · discrepancias → issues.

---

## V.4 — Complemento FASE 10: validación real

### F10.10 — Deploy + validación F10 en staging (PRIORITARIO)
El VPS bloqueó SSH por conexiones intensas; el binario en staging es F9.
UNA sesión SSH multiplexada (`ssh -o ControlMaster=auto -o ControlPersist=10m`)
ejecutando `scripts/DEPLOY-VALIDACION-F10.sh` completo.
**DoD:** `bosctl rpc bos.ai.catalog` → 17 acciones · `systemctl status bos`
activo · nodo Ready · batería bos.ai.* verde.

(F0.6.S — usuario `bos` en staging — ya está definido en FASE 0; se ejecuta
en la misma ventana de recuperación de SSH.)

---

## V.5 — FASE 11: Ficha Engine — el motor completo del sistema de fichas

El bos implementa el contrato completo de SBOS-019 (estados según ADR-021):
manifest.yml + yaml_engine.yml + task_catalog.sh + resources/ por ficha.
"Agregar la app 97 es crear una carpeta" — promesa verificada con tests.

```
F11.1 — Auditoría vs contrato: qué soporta hoy el bos de cada elemento
        (parser F9.1 ✓, sagas ✓, ¿resolver? ¿executor? ¿governance?
        ¿drift?) → docs/FICHA-ENGINE-GAP.md — contra código, no supuestos.

F11.2 — DEPENDENCY_RESOLVER (SBOS-019 §6): manifests → grafo dirigido →
        detección de ciclos → Kahn → desempate execution_order → deps
        insatisfechas = BLOQUEADA (derivado). hostserver: solo orden.
        DoD: bosctl ficha plan muestra el DAG de 22 · ciclo artificial →
        error explícito · TestResolver_Kahn22Fichas.

F11.3 — Ejecutor yaml_engine: pre_install/post_install/update/repair/
        uninstall — tasks en orden, params a task_catalog.sh, on_failure
        abort|continue, diagnosis_first en repair, señales __SBOS__STEP_*__
        → TUI (wsEventMsg) y audit.
        DoD: ficha de prueba instalada SOLO desde sus contratos, sin que el
        Core la conozca · señales visibles en S05.

F11.4 — Consistencia 18 estados ADR-021 en toda la superficie: TUI, bosctl,
        bos.state.read y bos.query.* muestran los mismos 18; cero
        referencias al modelo de 5; derivados documentados en
        internal/state/doc.go.
        DoD: grep de estados legados = 0 · bosctl ficha list ≡ RPC.

F11.5 — Governance 1-3 + Dual-Control (SBOS-019 §13): según
        governance.category del manifest. Nivel 3: DOS admins sbos-admin
        distintos, ventana 60 min, texto exacto ("DESINSTALAR-POSTGRESQL").
        Reutiliza HITL F10.6. ⛔ GATE DE APROBACIÓN.
        DoD: TestGovernance_Cat3RequiereDosAdmins · audit con ambas firmas.

F11.6 — Health checker declarativo: el observer ejecuta health.check_command
        via check_via con interval_seconds y failure_threshold del manifest.
        DoD: cambiar manifest cambia comportamiento sin recompilar · ALERTA
        exactamente al 3er fallo · race ×10.

F11.7 — Drift detection: reconcile_file + drift_check (hash de resources/
        vs estado vivo) → derivado ACTUALIZACIÓN DISPONIBLE + bosctl ficha
        diff <id>.
        DoD: config modificado en vivo → drift en <interval ·
        update_strategy:hot sin reinicio cuando corresponde.

F11.8 — Versionado semántico (§14): N-1 MAJOR verificado por el resolver
        (X-2 bloqueado, error explícito) · migración MAJOR como saga:
        backup → SQL → manifest → health → OK o rollback automático
        (SagaEngine F10.4).
        DoD: TestMigracionMajor_RollbackAutomatico.

F11.9 — Descubrimiento automático (§15): escaneo de servers/<srv>/<app>/
        al arrancar y con bosctl ficha rescan; valida los 3 contratos
        (validate_sp02); integra al grafo sin tocar el Core.
        DoD: carpeta nueva → en list · eliminada → fuera tras rescan.

F11.10 — Matriz operaciones × 22 fichas en staging →
         docs/FICHAS-ADMIN-MATRIX.md.
         DoD: sin "no implementado" en operaciones core.

F11.11 — bosctl ficha UX: list|plan|status|install|repair|scale|describe|
         diff|pause|logs|rescan — tabular + --json, help por subcomando.
```

---

## V.6 — FASES 12-15: despliegue del stack por capas

### FASE 12 — Capa 3: datos
```
F12.1 — postgresql HA + WAL slot bkernel_slot → C-04 ✓
F12.2 — redis DB0/1/2 + AOF → C-05 ✓ (PONG ×3)
F12.3 — vault init + unseal Shamir (3/5 llaves al operador) → C-06 ✓
        ⛔ GATE: llaves custodiadas por humano — JAMÁS en logs/repo
F12.4 — Vault PKI: CA interna + AppRole (TTL 24h)
F12.5 — bkernel_db: DDL Context Plane + \dt   ← habilita F5.7
F12.6 — Backups pg_dump + snapshot estado bos
F12.7 — bos.query.system con semáforos verdes reales
F12.8 — Certificación: C-01..C-06 ✓ + datos sobreviven reinicio
```

### FASE 13 — Capa 4: identidad
```
F13.1 — keycloak: realm {tenant} + health/ready UP → C-07 ✓
F13.2 — Hardening: MFA, token TTLs, brute-force  🔴 gate lockout
F13.3 — kong: database.reachable → C-08 ✓
F13.4 — Rutas Kong api.{tenant} + TLS de Vault PKI
F13.5 — Linkerd mTLS en namespaces sbos-*
F13.6 — bos↔Keycloak: JWT en JSON-RPC (extiende F6.1)
F13.7 — Certificación: C-07, C-08 ✓ + login OIDC e2e
```

### FASE 14 — Capa 5: daemons soberanos como STUBS de contrato (ADR-007)
Decisión BOS-REPAIR-16-ADR007 (Cuestionario-01 §A): no existe código real de
daemons hermanos — el bos es la piedra fundamental. Se crean stubs con
contratos CANÓNICOS y datos de escenario (YAML, sin recompilar) en los repos
definitivos de cada daemon: BauthAgent, BkernelAgent, BnexusAgent,
BiedataAgent. Regla de oro: contrato real, datos ficticios — jamás al revés.
```
F14.1 — bauth-stub: Unix socket /run/bos/bauth.sock, framing length-prefix
        JSON (4B BE + payload), escenarios SAM-128/BitmaskBundle v3
        (admin total · cajero limitado · AUTH_003 fuera de horario ·
        AUTH_004 límite financiero · SRV_002 sobrecarga · superusuario 0x0)
        Contrato: SBOS-BAUTH-DECISIONES §C1-C3 · habilita F11.5
F14.2 — bhnexus-stub: servidor WS mTLS :9444 (frames banexus) que llama al
        bos.ctx.device.register REAL (F5.4) → dctx_id pre-auth, y hace PUSH
        de context.promoted/expired (trigger manual hasta Keycloak F13.1).
        Entrega la config que el bos reconoce como válida (A4).
        🔴 gate: camino crítico del VDI · Contrato: SBOS-NEXUS §6-8
F14.3 — banexus-stub: flujo soberano de prueba — auth_request HMAC
        (QR sbos://auth/...) → auth_response con SAM <50ms · topología
        vetada respetada (jamás bAuth/KC directo)
F14.4 — bkernel-stub + bkernel_db creada por el bos si la requiere
        (DDL definitivo = desarrollo bkernel — Cuestionario A6)
F14.5 — biedata/bcompass/bsearch: stubs bajo demanda (solo lo que las
        pruebas del bos referencien)
F14.6 — Certificación Capa 5: suite e2e del bos contra los 5 stubs sin
        bloqueos, escenarios denied/sobrecarga incluidos · dctx <2s (pre-C-13)
Restricción de staging (incidente Contabo): los stubs escuchan SOLO en
localhost/red interna del cluster — cero tráfico saliente.
```

### FASE 15 — Capa 6: fichas de aplicación
```
F15.1 — Set mínimo del seed file (via Ficha Engine F11)
F15.2 — Tryton (o ficha de negocio del seed) para C-14
F15.3 — bNotifier + soporte
F15.4 — DAG verificado: el resolver F11.2 ordenó correctamente en vivo
F15.5 — Certificación: seed completo INSTALADA + probe ✓
```

---

## V.7 — FASE 16: Capa 7 — VDI LAYER, LA CÚSPIDE
Spec: SBOS-052 + bos-repair/BOS-REPAIR-14-SBOS-CLIENT-SPEC.md

```
Bloque 16.A — Las tres fichas VDI:
F16.1 — nextcloud: nextcloud_db + OIDC + PVC 500Gi + carpetas
        tenant/empresa/sucursal + cuotas seed + Kong files.{tenant} → C-09 ✓
F16.2 — guacamole: guacamole_db + OIDC + pool VNC + Kong vdi.{tenant} → C-10 ✓
F16.3 — Imagen fedora-logico: Containerfile (Fedora 42 + GNOME + TigerVNC +
        nextcloud-client) — build CI + registry interno
F16.4 — Ficha fedora-logico: HPA min=2/max=20 + mTLS Vault PKI

Bloque 16.B — sbos-client (DESARROLLO NUEVO — spec: bos-repair/BOS-REPAIR-14-SBOS-CLIENT-SPEC.md v2.0):
El sbos-client vive en el MONOREPO del bos (src/cmd/sbos-client/) y habla
WebSocket mTLS con bhnexus :9444 — mismo protocolo que banexus. El flujo es
event-driven: el bos hace PUSH de context.promoted/context.expired.
F16.5 — src/cmd/sbos-client/ + internal/sbosclient/ (mode/link/identity/
        session/desktop/home/config) — reutiliza internal/wslib, paths y
        audit del bos · 3 modos autodetectados (PHYSICAL/LOGICAL/WSL)
F16.6 — identity: WS mTLS :9444 con headers X-Node-ID/X-Agent-Version/
        X-Agent-Cert-Fingerprint → device_register → dctx_id pre-auth
        (bitmask 0x0, Redis + bkernel_db.context_sessions) + heartbeat 30s
        + reconexión backoff 1→5→15→30→60s
F16.7 — session: al recibir PUSH context.promoted (ctx_id + BitMask) →
        aplicar políticas dconf en GNOME (apps según BitMask) + montar home
        Nextcloud (uid de sesión, noexec, nosuid) + env vars con ctx_id +
        notificar banexus en MODE_PHYSICAL
F16.8 — Ciclo de vida: context.expired → limpiar dconf (GNOME a login) →
        desmontar home (disco local VACÍO verificado — P1) → pod al pool /
        físico a login · TTL vencido sin conexión = fail-secure ·
        systemd unit (físico) + supervisor (pod)
F16.9 — Tests contra bhnexus stub WS: TestModeDetection ·
        TestRegister_DctxPreAuth<2s · TestPromoted_AplicaDconfYMontaHome ·
        TestExpired_DiscoVacio · TestReconexion_Backoff ·
        TestTTLSinConexion_FailSecure

Bloque 16.C — Cierre de la cúspide:
F16.10 — Pods ≥2 Running + sbos-client registrado → C-11 ✓
F16.11 — Home montado (ls ~/Documentos) → C-12 ✓ · register <2s → C-13 ✓
F16.12 — sbos-fedora.iso: kickstart + lorax CI + SHA256 + Ed25519 +
         bosctl iso download   🔴 gate: clave privada fuera del repo
F16.13 — bosctl vdi verify --tenant=skull → 6/6 contra fichas REALES
         (reemplaza los probe stubs de F9.9)
F16.14 — C-14 end-to-end: bosctl vdi test-user → login web → Keycloak →
         GNOME <10s → archivo guardado aparece en Nextcloud → logout limpio
         ✦ LA CÚSPIDE: "El SBOS está instalado." ✦
```

---

## V.8 — FASE 17: estándares internacionales + certificación final
Detalle del marco normativo: bos-repair/BOS-REPAIR-15-ESTANDARES-INTERNACIONALES.md

```
F17.1 — CIS K8s Benchmark v1.12 (K8s 1.32-1.34): kube-bench en CI, L1 ≥95%,
        excepciones con control compensatorio
        🔴 gate: flags del API server pueden tumbar el cluster
F17.2 — CIS host Ubuntu (Lynis/CIS-CAT)
F17.3 — NIST SP 800-190: trivy + cosign, registry TLS, tags por digest,
        pods sin privilegios, seccomp RuntimeDefault, VNC solo interno
F17.4 — NIST SSDF 800-218: mapeo PO/PS/PW/RV + gosec + govulncheck en CI
F17.5 — SLSA L2: provenance firmada (bos, bosctl, sbos-client, imagen, iso)
F17.6 — SBOM por release (syft → CycloneDX)
F17.7 — ISO 27001:2022: matriz de controles técnicos (A.8.15 ✓ desde F1.1;
        sumar A.8.16, A.5.23, A.8.5, A.8.24, A.8.28)
        Nota honesta: conformidad técnica ≠ certificación organizacional
F17.8 — ISO 25010: 7 gates medibles (disponibilidad críticas 99.5% / VDI
        99.0%, MTTR <10min, cobertura ≥60% sostenida, login VDI <10s,
        query <4s, cliente en HW 2010/4GB)
F17.9 — bosctl bootstrap verify --full → 14/14 ✓ + reporte de certificación
        con evidencia por estándar
F17.10 — Informe final + actualización de toda la documentación
```

---

## V.9 — Gates de aprobación nuevos (extienden GESTION-RIESGOS-OPERATIVOS.md)

| Átomo | Riesgo | Por qué |
|---|---|---|
| F11.5 | ⛔ CRÍTICO | Dual-control mal hecho = protección falsa sobre PostgreSQL/Vault |
| F12.3 | ⛔ CRÍTICO | Llaves Shamir — custodia humana exclusiva |
| F13.2 | 🔴 ALTO | Lockout de operadores por política Keycloak |
| F14.2 | 🔴 ALTO | bhnexus nuevo en el camino crítico del VDI |
| F16.12 | 🔴 ALTO | Clave privada Ed25519 fuera del repo |
| F17.1 | 🔴 ALTO | Flags del API server pueden tumbar el cluster |

## V.10 — Criterio de éxito del proyecto completo

```bash
bosctl bootstrap verify --full          # 14/14 criterios ✓
bosctl vdi verify --tenant=skull        # 6/6 OK contra fichas reales
bosctl vdi test-user --tenant=skull ... # GNOME <10s · archivo persiste
kube-bench run                          # L1 ≥95%
ls releases/ | grep sbos-fedora.iso     # ISO firmado disponible
# Y el momento humano: el operador abre un navegador →
# vdi.skull.sksistemas.com → trabaja. "El SBOS está instalado."
```

---

*PARTE V · BOS-REPAIR-PLAN-MAESTRO-v3 → v4.0 · SKULL · SBOS · Junio 2026*
*Anexos nuevos: bos-repair/BOS-REPAIR-14-SBOS-CLIENT-SPEC.md · bos-repair/BOS-REPAIR-15-ESTANDARES-INTERNACIONALES.md — registrar en ANEXOS-INDEX.md*
