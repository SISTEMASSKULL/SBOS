...
# EXTENSIÓN v2 — Gates de aprobación de las fases F11-F17 · Junio 2026

## Inventario nuevo por nivel

### ⛔ CRÍTICOS — Gate de aprobación obligatorio

#### F11.5 — Governance Dual-Control (categoría 3)
**Riesgo:** un dual-control mal implementado es protección FALSA sobre
PostgreSQL/Vault/Keycloak — peor que no tenerlo.
**Plan de cambio que el agente presenta:**
```
PLAN DE CAMBIO — F11.5
Diseño: governance.category del manifest → nivel 3 exige DOS sbos-admin
distintos (identidades del bauth-stub F14.1 — break-glass G4), ventana
60 min, texto exacto ("DESINSTALAR-POSTGRESQL"), un solo uso (HITL F10.6).
Tests previos: TestGovernance_Cat3RequiereDosAdmins ·
TestGovernance_MismoAdminDosVeces_Rechazado ·
TestGovernance_VentanaExpirada_Rechazado
Audit: ambas aprobaciones en JSONL ANTES de ejecutar (A.8.15).
Reversión: flag BOS_GOVERNANCE_V2=false → comportamiento HITL actual.
¿Aprueba? [sí/no]
```

#### F12.3 — Vault unseal Shamir
**Riesgo:** las llaves Shamir en logs/repo = compromiso total del stack.
**Plan de cambio:** init con 5 llaves/umbral 3 ENTREGADAS al operador por
canal seguro fuera del repo; el agente verifica `vault status` sellado/
desellado sin jamás imprimir llaves; audit del evento sin material sensible.
Verificación: grep de llaves en logs/commits = 0. ¿Aprueba? [sí/no]

### 🔴 ALTOS

| Átomo | Riesgo | Mitigación obligatoria |
|---|---|---|
| F13.2 Keycloak hardening | Lockout de operadores | Cuenta break-glass local verificada ANTES de aplicar políticas · rollback documentado |
| F14.2 bhnexus-stub | Camino crítico VDI · puerto :9444 | Solo red interna del cluster (incidente Contabo) · contrato SBOS-NEXUS §6-8 sin desviaciones |
| F16.12 ISO firmado | Clave privada Ed25519 | La clave JAMÁS toca repo/CI logs · firma en máquina del operador o KMS · verificación con clave pública |
| F17.1 kube-bench remediación | Flags del API server pueden tumbar el cluster | Un flag por commit · snapshot etcd previo · ventana declarada en SESION-LOG |

## Regla transversal de staging (incidente Contabo — vigente)

Toda sesión en 13.140.128.230 se rige por la skill
sbos-staging-security-monitor: monitoreo activo durante la prueba, CERO
tráfico saliente nuevo, sesión SSH multiplexada única
(ControlMaster=auto, ControlPersist=10m), e informe en
/var/log/sbos-reports/. Cualquier tráfico saliente del software en prueba
se registra como [BUG-SBOS] y detiene la sesión.

*EXTENSIÓN v2 · GESTION-RIESGOS-OPERATIVOS · SKULL · SBOS · Junio 2026*
