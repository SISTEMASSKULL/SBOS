...
# ACTUALIZACIÓN DEL PROTOCOLO · Junio 2026 — fases F11-F17

## Señal de retoma extendida (agregar al paso 1.3)
...
echo -n "F14 stubs:     "; [ -d /opt/skull/orquestador/proyectos/desarrollo/sbos/BauthAgent/src ] && echo "✅ BauthAgent" || echo "❌"
echo -n "F16 sbos-client:"; [ -d cmd/sbos-client ] && echo "✅" || echo "❌"
```

## Reglas nuevas de sesión

1. **Staging bajo vigilancia (incidente Contabo):** antes de cualquier ssh a
   13.140.128.230, activar el protocolo de la skill
   sbos-staging-security-monitor. UNA sesión multiplexada
   (`-o ControlMaster=auto -o ControlPersist=10m`). Declarar la ventana en
   SESION-LOG ANTES de conectar. Cero tráfico saliente nuevo.
2. **Prioridad al recuperar SSH:** F10.10 + F0.6.S en la misma ventana,
   antes que cualquier otro átomo.
3. **Repos hermanos (F14):** los stubs se desarrollan en BauthAgent/,
   BnexusAgent/, BkernelAgent/, BiedataAgent/ — cada uno con su propio
   git, CI y SESION-LOG espejo de este protocolo.
4. **Gates nuevos:** F11.5, F12.3, F13.2, F14.2, F16.12, F17.1 — plan de
   cambio según GESTION-RIESGOS-OPERATIVOS v2 antes de tocar código.
5. **Dudas:** documentación → legacy/_snapshots → BosAgent/ raíz → escalar
   (en ese orden — Cuestionario-01).
