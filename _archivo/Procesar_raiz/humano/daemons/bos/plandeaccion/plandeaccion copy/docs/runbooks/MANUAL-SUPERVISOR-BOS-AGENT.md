...
## 3. Entorno de pruebas: sbos-greenfield

El contenedor `sbos-greenfield` simula un host Ubuntu limpio para desarrollo y pruebas.
Es **desechable** — se recrea desde cero para cada ciclo de prueba.

### 3.1 Parámetros de creación

```bash
podman run -d --name sbos-greenfield \
  --privileged \
  --cgroupns=private \
...
| Aspecto | sbos-greenfield (pruebas) | Producción |
|---|---|---|
...
podman exec sbos-greenfield grep "watchdog cycle" /var/log/bos/bos.log | tail -5
...
### 12.2 Despliegue en sbos-greenfield

```bash
# Matar daemon anterior + desplegar binarios + reiniciar
podman exec sbos-greenfield kill $(pgrep -f "bos.*--config") 2>/dev/null; sleep 2
podman cp /tmp/bos-build/bos sbos-greenfield:/usr/local/bin/bos
podman cp /tmp/bos-build/bosctl sbos-greenfield:/usr/local/bin/bosctl
podman cp /tmp/bos-build/bos sbos-greenfield:/opt/bos/bin/bos
podman exec sbos-greenfield nohup /opt/bos/bin/bos --config /etc/bos/bos.toml \
  > /var/log/bos/bos.log 2>&1 &
rm -rf /tmp/bos-build

# Verificar
sleep 2 && podman exec sbos-greenfield bosctl top
```
...
podman cp staging/core/servers/hostserver/sbos-bootstrap-k8s/task_catalog.sh \
  sbos-greenfield:/etc/bos/blibs/servers/hostserver/sbos-bootstrap-k8s/task_catalog.sh
```

### 12.4 Recrear contenedor limpio

```bash
podman stop sbos-greenfield && podman rm sbos-greenfield
...
3. Desplegar en sbos-greenfield (ver §12.2)
...
podman exec sbos-greenfield pgrep -la bos

# Check 2: Watchdog activo (debe mostrar ciclos cada ~30s)
podman exec sbos-greenfield grep "watchdog cycle" /var/log/bos/bos.log | tail -3

# Check 3: CLI funcional
podman exec sbos-greenfield bosctl top

# Check 4: State file íntegro
podman exec sbos-greenfield bosctl health
```
