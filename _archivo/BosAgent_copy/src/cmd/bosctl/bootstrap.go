...
// cmdBootstrap handles the 'bosctl bootstrap' subcommand tree.
...
// cmdBootstrapStart triggers the bootstrap installation.
...
// cmdBootstrapStatus shows the current bootstrap state.
...
// cmdBootstrapVerify checks the 8 certification criteria (C-01 to C-08).
...
// runLocalVerify checks certification criteria directly without the daemon.
// Used when daemon is not available (e.g., during initial bootstrap).
...
// cmdBootstrapResume resumes an interrupted bootstrap.
...
// cmdBootstrapReset resets the bootstrap state (destructive).
...
// stateToIconBootstrap maps bootstrap-specific states to display icons.
