...
_log()   { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [vault] $*" | tee -a "$FICHA_LOG" >&2; }
...
# _vault_field: extrae un campo del JSON de vault status.
_vault_field() {
    local field="$1" default="${2:-}"
    _vault_status_json | \
        python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('${field}','${default}'))" \
...
ficha_pre_install() {
    ...
    if ! _k get pv sbos-vault-pv > /dev/null 2>&1; then
        echo "${__STEP_FAIL__} verificar_pv: sbos-vault-pv no existe — requiere sbos-bootstrap-storage"
        return 1
    fi
    local pv_phase
