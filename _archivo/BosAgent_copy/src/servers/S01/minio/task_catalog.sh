...
_log()   { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [minio] $*" | tee -a "$FICHA_LOG" >&2; }
...
ficha_pre_install() {
    ...
    if ! _k get pv sbos-minio-pv > /dev/null 2>&1; then
        echo "${__STEP_FAIL__} verificar_pv: sbos-minio-pv no existe — requiere sbos-bootstrap-storage"
        return 1
    fi
    local pv_phase
