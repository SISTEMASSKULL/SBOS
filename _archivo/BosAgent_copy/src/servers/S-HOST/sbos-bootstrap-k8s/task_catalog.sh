...
_log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [sbos-bootstrap-k8s] $*" | tee -a "$FICHA_LOG" >&2; }
...
ficha_test() {
    ...
    if kubectl get storageclass sbos-local > /dev/null 2>&1; then
...
ficha_status() {
    ...
    echo ""
    echo "StorageClass:"
    KUBECONFIG="$KUBECONFIG_DEST" kubectl get storageclass sbos-local 2>/dev/null \
        || echo "  sbos-local: NO existe"
}
