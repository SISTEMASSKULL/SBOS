// Package repair implements unified multi-layer repair for SBOS (D1).
//
// Fase 1 — Ubuntu repair:
//
//	dpkg --audit → apt --fix-broken → systemctl restart services
//
// All operations are idempotent. Dry-run mode logs without executing.
package repair
...
// OSRepairResult holds the outcome of the Ubuntu repair phase.
...
// OSRepairer performs Ubuntu/Debian OS-level repair.
type OSRepairer struct {
	dryRun bool
	logger *slog.Logger
}

// NewOSRepairer creates an OS-level repairer.
func NewOSRepairer(dryRun bool, logger *slog.Logger) *OSRepairer {
	return &OSRepairer{dryRun: dryRun, logger: logger}
}

// Repair runs the full Ubuntu repair sequence.
