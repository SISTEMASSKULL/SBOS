// Package ai implements D4 — thin AI model router and client.
// bCompass is the real AI orchestrator (Ollama local, declarative routes).
// This package exposes bosctl ia as a lightweight bridge to cloud models
// (DeepSeek/Claude) via the Anthropic-compatible Messages API, plus a
// configurable local tier (Ollama native or OpenAI-compatible).
//
...
const (
	StatePrimary  RouterState = iota // Tier 1: Claude
	StateFallback                     // Tier 2: DeepSeek
...
// Route returns the currently active backend.
// If a manual override was set via SetOverride, it uses that for one call.
// Otherwise follows the circuit breaker: primary → fallback → local.
...
func (r *Router) checkRecovery() {
	...
	// If a higher tier recovered, promote state.
