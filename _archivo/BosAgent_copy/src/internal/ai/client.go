...
// Client sends prompts to the AI backend resolved by the Router.
// On HTTP 429 (rate limit), it cascades: primary → fallback → local.
type Client struct {
	router *Router
	http   *http.Client
}

// NewClient creates an AI client backed by the Router's three-tier circuit breaker.
...
// SetModelOverride sets a one-shot model override for the next Route() call.
...
// Ask sends a prompt with context through the router's circuit breaker.
// On rate-limit errors it cascades: primary → fallback → local.
// modelOverride is from --model= CLI flag (empty = auto).
...
// ActiveBackend returns a human-readable description of the current backend.
func (c *Client) ActiveBackend() string {
	return c.router.ActiveBackendName()
}

// RouterStats returns router observability data.
...
func (c *Client) callOllama(backend Backend, prompt string, ctx *AIContext) (string, error) {
	...
	// Ollama returns ndjson when streaming; we request stream=false so it's a single JSON object.
	// If multiple lines are returned, take the last one (final response).
