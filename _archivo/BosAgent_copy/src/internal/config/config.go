...
// InstallConfig holds client-specific installation parameters from bos-install.toml.
// Schema: SBOS-035 §1.1 + SBOS-018 §23a + SBOS-050 §DNS.
...
// Subdomain returns the FQDN for a named service.
...
// Config holds all configuration for the bos daemon.
// Install-specific values come from InstallConfig (bos-install.toml).
// Runtime values come from bos.toml or built-in architectural defaults.
type Config struct {
...
// Load reads bos.toml and bos-install.toml, applies env overrides,
// and returns a merged Config. Returns ErrInstallConfigMissing if
// bos-install.toml is not found and no BOS_INSTALL_TOML env var is set.
...
// The daemon should enter config-pending (staged) mode.
var ErrInstallConfigMissing = fmt.Errorf("config: bos-install.toml not found — config-pending mode")

// IsConfigPending returns true if the error is ErrInstallConfigMissing.
