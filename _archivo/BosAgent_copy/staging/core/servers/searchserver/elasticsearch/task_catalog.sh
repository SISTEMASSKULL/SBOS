#!/usr/bin/env bash
# task_catalog.sh — Elasticsearch task handlers
# Ficha · Order · SBOS-049 §4.3
# R16: no hardcoded paths. All paths from env vars with defaults.

set -euo pipefail

export SBOS_FICHA_DIR="${SBOS_FICHA_DIR:-/etc/bos/blibs/servers/searchserver/elasticsearch}"
...
ficha_install() {
    ...
    sbos_k8s_core "${SBOS_FICHA_DIR}/elasticsearch.k8s.yml" "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready statefulset/elasticsearch-0 -n "${SBOS_NAMESPACE}" --timeout=600s
...
ficha_update() {
    ...
    kubectl rollout restart statefulset elasticsearch -n "${SBOS_NAMESPACE}"
    kubectl wait --for=condition=Ready statefulset/elasticsearch-0 -n "${SBOS_NAMESPACE}" --timeout=300s
...
ficha_uninstall() {
    ...
    kubectl delete statefulset elasticsearch -n "${SBOS_NAMESPACE}" --ignore-not-found
    kubectl delete pvc elasticsearch-data -n "${SBOS_NAMESPACE}" --ignore-not-found
