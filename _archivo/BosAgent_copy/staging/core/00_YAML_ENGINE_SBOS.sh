...
# yq is required for YAML operations but NOT at source time.
# Defer check — some fichas (e.g. sbos-bootstrap-os) don't need yq and can
# install it before any YAML-dependent ficha runs.
_require_yq() {
    if ! command -v yq &>/dev/null; then
        echo "FATAL: yq (https://github.com/mikefarah/yq) is required for YAML operations"
        echo "Install it first: sbos-bootstrap-os should provide it"
        return 99
    fi
    return 0
}

# Guard every yq call site with _require_yq || return 1
# This function is called at the top of each function that uses yq.
