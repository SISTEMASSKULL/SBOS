...
* 10 fichas con código Docker migrar a Podman 4.9.3 validado del TODOIAM legacy (4,495 líneas)
...
|Fichas con relaciones|No solo depends\_on sino tipo de relación|SBOS-MP05 §5|
|task\_catalog.sh permanece en Bash|Las fichas son Bash, el daemon bos es Go|SBOS-005 header|
|Ed25519 para firma de fichas|SLSA framework para Release Plane|SBOS-038|
|Monorepo único sbos/|Todo en github.com/SISTEMASSKULL/sbos. Independencia de build via GitHub Actions path filters por directorio, no repos separados|SBOS-018 §14|
...
|SBOS-006|**Sistema de Fichas** (manifest.yml, yaml\_engine.yml, task\_catalog.sh)|
...
Archivo de 4,495 líneas con 10 servicios Docker validados y funcionales, para migrar directamente a Podman 4.9.3:
...
* 4 workflows: full\_install, repair, diagnostic, backup
...
* **Nombres de archivos:** SBOS-XXX-NOMBRE-vN\_M.md (no cambiar versión sin instrucción)
...
Ver SBOS-MP06-PlanMaestroDesarrollo-v1\_0.md para el plan completo.
...
* Eliminada oracion task\_catalog.so de SBOS-018 seccion 11.1
...
Criterio: validate\_sp01.py pasa con exit 0. Ciclo A-E-L probado.
...
y validate\_sp01.py pasando con exit 0.
...
7. INVESTIGAR EN INTERNET: Cuando no estés seguro de una versión, 
   una configuración, o un comportamiento de un servicio Podman, 
   búscalo en internet antes de responder. 
   Las imágenes Podman cambian, las APIs cambian.
...
- Fichas: Bash (task\_catalog.sh) + YAML (manifest.yml, yaml\_engine.yml)
...
- Cuando generes archivos, usar create\_file con la ruta correcta.
