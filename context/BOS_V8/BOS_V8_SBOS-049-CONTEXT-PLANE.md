...
### Enriquecimiento V7: El Par Nexus (bhnexus+banexus) y el Context Plane

La conceptualización V7 del Par Nexus Soberano (BOS_V7_SBOS-NEXUS-CONCEPTUALIZACION-v3_0) profundiza en cómo el Context Plane se entrelaza con la autenticación física y la ejecución de BitMask a nivel de hardware:

**El rol del Nexus en el Context Plane**

| Componente | Participación en el Contexto |
|---|---|
| **bhnexus** (Connectivity Broker) | Router central de eventos de credencial física. Mantiene SAM-128 cache en memoria con TTL 30s para evitar consultas repetidas a bAuth. Distribuye política de invalidación cuando el RolTemplate cambia. Propaga el context.switched a todos los banexus conectados del tenant. |
| **banexus** (Edge Sentinel) | Intercepta la credencial física ANTES del SO (udev + libusb). Aplica el BitMask recibido para decidir apertura de relés (puertas, cajón POS). Cache efímero AES-256-GCM para modo offline. |

**La topología invariable del contexto físico:**
...
banexus (Fedora) ──WSS/mTLS──► bhnexus ──Unix socket──► bAuth
                                    ...
                                    ▼
                              bos Context API
                              (crea/destruye ctx_id)

VETADO:
  ✗ banexus → bAuth directamente       (rompe cadena de custodia)
  ✗ dispositivo → bhnexus sin banexus  (pierde interceptación soberana)
  ✗ dispositivo → bAuth                (pierde todo el control)
```

**Flujo soberano en <50ms** (integración Context Plane + Nexus):
...
T+8ms   bhnexus verifica contexto contra bos (ctx_id válido?)
T+12ms  bhnexus → banexus: {auth_response + actuator_commands} WSS
T+15ms  banexus activa relé (cajón/chapa) O deniega con registro
T+15ms  Evento audit_events con ctx_id preservado
```

**Actualización de política (context.push):** Cuando bAuth actualiza un RolTemplate (cambio de BitMask o contexto), el flujo de invalidación es:

```
bAuth detecta cambio
  ↓
bAuth push vía WebSocket :9450 → bhnexus
  ↓
bhnexus invalida SAM-128 en cache para usuarios afectados
  ↓
bhnexus propaga a todos los banexus conectados:
  {type: "policy_update", invalidated_users: [user-ids], new_bitmask: ...}
  ↓
banexus limpia cache efímero para los usuarios afectados
  ↓
Próximo auth_request → cache miss → consulta fresca a bAuth
```

Esto asegura que un cambio en el contexto del usuario (por ejemplo, context.switched de sucursal La Paz a sucursal Santa Cruz) se refleje inmediatamente en los permisos físicos del dispositivo.

---

### Enriquecimiento Smart*: Integración del Context Plane en Subproyectos

Los subproyectos Smart* del ecosistema SBOS integran el Context Plane de manera específica según su dominio:

**Smart Tax:** Cada operación fiscal (factura, nota de ajuste, CUF generation) lleva el `ctx_id` como campo obligatorio en la tabla de auditoría. El contexto permite responder preguntas como "¿qué usuario, desde qué POS, en qué empresa emitió esta factura?" — requisito obligatorio para fiscalización (SIAT/AFIP/SAT). El ctx_id viaja en el XML de factura como metadata de auditoría.

**Smart ORC:** Todos los endpoints de la API REST requieren `X-SBOS-CtxId` extraído del baggage de OpenTelemetry. La cadena de custodia de documentos incorpora el `ctx_id` para rastrear cada transferencia entre funcionarios. El endpoint público de verificación QR (`GET /api/v1/public/verify/{doc_id}`) registra cada acceso en `audit_events` con la IP de origen y el ctx_id si el verificador está autenticado.

**Smart CMS:** El multi-tenancy de contenido se implementa usando `ctx_id` como filtro de aislamiento: cada checkout (sesión de compra) está vinculado a un contexto operativo. Las plantillas de producto y las vistas de contenido se seleccionan según el tenant y empresa del contexto activo.

**Smart Vault Flow:** El flujo de custodia de claves RSA utiliza `ctx_id` para registrar qué usuario y contexto autorizó la rotación, exportación o destrucción de claves. Cada operación criptográfica queda vinculada al contexto empresarial del momento.
...
| Enriquecimiento V7: Nexus | BOS_V7_SBOS-NEXUS-CONCEPTUALIZACION-v3_0 | SAM-128 cache, topología invariable, flujo soberano <50ms, policy_update flow |
| Enriquecimiento Smart* | SBOS Smart Tax / ORC / CMS / Smart Vault Flow — SBOS-049 doc copies | Integración ctx_id en fiscal, ORC, CMS, vault |
...
|---|---|---|
| V6 | /opt/skull/orquestador/proyectos/desarrollo/context/sbos/Procesar/BOS_V6_SBOS-049-CONTEXT-PLANE.md | Base V6 completa — v2.0 con corrección bos como dueño, validación OTel/W3C/NIST/ISO |
| V7 Nexus | /opt/skull/orquestador/proyectos/desarrollo/context/sbos/Procesar/BOS_V7_SBOS-NEXUS-CONCEPTUALIZACION-v3_0.md | Par Nexus (bhnexus+banexus): SAM-128 cache, topología invariable, flujo soberano <50ms con ctx_id, policy_update flow |
| Smart Tax | /opt/skull/orquestador/proyectos/desarrollo/sbos/subproyectos/SBOS Smart Tax/context/SBOS_TAX_SBOS-049-CONTEXT-PLANE.md | Integración ctx_id en operaciones fiscales |
| Smart ORC | /opt/skull/orquestador/proyectos/desarrollo/sbos/subproyectos/SBOS Smart ORC/context/SBOS-049-CONTEXT-PLANE.md | Cadena de custodia con ctx_id, endpoint público de verificación |
| Smart CMS | /opt/skull/orquestador/proyectos/desarrollo/sbos/subproyectos/SBOS CMS/context/SBOS-049-CONTEXT-PLANE.md | Multi-tenancy de contenido vía ctx_id |
| Smart Vault Flow | /opt/skull/orquestador/proyectos/desarrollo/sbos/subproyectos/SBOS Smart Vault Flow/context/SBOS-049-CONTEXT-PLANE.md | Operaciones criptográficas vinculadas a ctx_id |
| Correlacion V8 | Integracion V7 Nexus + Smart* subproyectos | Contexto físico del Par Nexus, contexto fiscal, ORC, CMS y vault flow |
