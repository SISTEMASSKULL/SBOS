# CLAUDE.md — BcmsAgent (bcms)
<!-- nivel: microservicio (3) · hereda de: SBOS + Fábrica ORQUESTA -->

## Propósito propio
**bcms** — Gestión de contenido del ecosistema (CMS)

## Producto que desarrolla
Gestión de contenido del ecosistema (CMS) · **Plano:** Contenido · **Stack:** por definir

## Interfaz (ADR-020 — Interface Dual)
- Unix socket `/run/bos/bcms.sock` (0660, grupo `bos`): WebSocket RPC + JSON-RPC 2.0.
- Namespace JSON-RPC `bcms.*`. **NUNCA HTTP/TCP entre daemons** (SBOS-050 P9).

## Fronteras (ORQUESTA-051)
- **Escribo solo aquí** (`BcmsAgent`). Consulto a los hermanos por su contrato (`PROPOSITO.md`/JSON-RPC), en solo lectura.
