...
## 12. bi18n como servicio de validación y ejecución para el AtomLang

### 12.1 El problema: metadatos sin motor = texto muerto

El [Manual de Atributos v2.1.0](1.07_MANUAL-ATRIBUTOS-v2.0.md) define 25 metadatos para cada
atributo de identidad. Pero un metadato como `mask: "partial(4)"` o `display_format: "TAX_BO"`
es solo un string guardado en una fila. No produce ningún efecto por sí mismo, y nada impide
que alguien escriba `mask: "parcial(999)"` o `display_format: "INVENTADO"` — valores que en
runtime causarían errores silenciosos o resultados incorrectos.

**Sin bi18n, los metadatos del atributo son declaraciones inertes.** Con bi18n, cada valor
es verificado al escribirse y ejecutado correctamente al leerse.

### 12.2 El rol de bi18n en el ciclo de vida del atributo

bi18n actúa como el **servicio de validación y ejecución** de los metadatos del atributo, invocado
por el Motor de Identidad en dos momentos distintos:

#### En diseño (compile time) — validar que el metadata es legal

Cuando un administrador define o modifica el metadata de un atributo, bi18n verifica que
cada valor sea sintácticamente correcto y esté registrado:

```
bauth.entidad.atributo.CI.mask "partial(4)"     → bi18n.validate_mask_strategy("partial(4)")
                                                   ✅ MaskStrategy::Partial { visible_from_end: 4 }

bauth.entidad.atributo.CI.mask "parcial(999)"   → bi18n.validate_mask_strategy("parcial(999)")
                                                   ❌ MaskStrategy no existe → DENY

bauth.entidad.atributo.CI.format "ID_BO"        → bi18n.validate_display_format("ID_BO")
                                                   ✅ registrado en format_map

bauth.entidad.atributo.CI.format "INVENTADO"    → bi18n.validate_display_format("INVENTADO")
                                                   ❌ no está en format_map → DENY

bauth.entidad.atributo.nit.pattern "^\\d{7,13}$" → bi18n.validate_pattern("^\\d{7,13}$")
                                                   ✅ regex compila y es seguro

bauth.entidad.atributo.gender.canonical "M,F,NB" → bi18n.validate_canonical_values(["M","F","NB"])
                                                   ✅ lista válida
```

El administrador no puede guardar un metadata inválido porque bi18n lo rechaza en el acto.
Esto elimina toda una clase de bugs en producción.

#### En runtime (ejecución) — aplicar el metadata al valor

Cuando se lee o escribe un atributo, bi18n ejecuta el pipeline completo usando los metadatos
ya validados. Los verbos del Motor de Identidad (validate, verify, format) delegan en bi18n:

```
atributo.set("7654321-lp", CI, ID_BO)
  │
  ├── validate:  bi18n.validate_national_id("bo", "ci", "7654321-lp")
  │              → regex del TOML + canonicalValues → ✅ Permit
  │
  ├── verify:    bi18n no interviene aquí (lo hace api_segip vía Motor de Identidad)
  │              → ✅ verified_by: api_segip
  │
  └── format:    bi18n.format_value("7654321-lp", "ID_BO")
                 → "7654321 LP"

atributo.get(CI)
  │
  └── mask:      bi18n.mask_value("7654321 LP", "ID_BO")
                 → "****321-LP" (aplica partial(4) del metadata)
```

### 12.3 Qué metadatos valida y ejecuta bi18n

| Metadato del atributo | Validación en diseño | Ejecución en runtime |
|---|---|---|
| `mask` | `validate_mask_strategy()` — ¿es una estrategia conocida? ¿parámetros válidos? | `mask_value()` — aplica la estrategia al valor |
| `display_format` | `validate_display_format()` — ¿está en `format_map`? ¿es compatible con el tipo de dato? | `format_value()` — formatea con ICU4X o patrón plano |
| `canonicalValues` | `validate_canonical_values()` — ¿lista no vacía? ¿valores únicos? | `validate_enum()` — ¿el valor está en la lista? |
| `pattern` (TOML) | `validate_pattern()` — ¿regex compila? ¿es segura (sin ReDoS)? | `validate_national_id()` / `validate_phone()` — regex.match() |
| `input_mask` (TOML) | `validate_input_mask()` — ¿caracteres válidos (9, A, X, #)? | `mask_input()` — genera máscara de formulario |
| `date_granularity` | `validate_granularity()` — ¿valor conocido? | `format_date()` — aplica ICU4X con granularidad correcta |
| `locale_sensitive` | — | `localize()` — si true, aplica locale del tenant |
| `classification` | `validate_classification()` — ¿ISO 27001 válido? | BitMask + filtro de visibilidad |

### 12.4 Integración con el AtomLang

Los verbos del Motor de Identidad (`validate`, `verify`, `format`) se implementan como
**políticas AtomLang que delegan en bi18n**. No son código hardcodeado en bAuth:
...
politica "validacion_nit"
  ...
  regla "nit_formato"
    ...
    effect:
      decision: Permit
      obligation:
        validator: bi18n                          # ← delega en bi18n
  ...
  regla "nit_enmascarar"
    ...
    effect:
      decision: Permit
      obligation:
        masker: bi18n                              # ← delega en bi18n
        method: mask_value
        args: { format: "TAX_BO", strategy: "partial(4)" }
```

Esto significa que las reglas de validación y enmascaramiento se escriben en AtomLang
(como cualquier otra política de bAuth), pero la **ejecución real** la hace bi18n.
bAuth no necesita saber cómo validar un NIT boliviano ni cómo enmascarar una CI —
solo necesita saber que bi18n lo resuelve.

### 12.5 Qué pasa sin bi18n — la justificación

| Sin bi18n | Con bi18n |
|---|---|
| `mask: "cualquier_cosa"` → se guarda sin rechistar → error en runtime o peor, silencioso | `mask: "cualquier_cosa"` → bi18n lo rechaza al guardar → el metadata nunca está corrupto |
...
| Agregar un país nuevo requiere modificar bAuth | Se agrega un archivo `country-rules/{país}.toml` — cero código |
| `display_format: "INVENTADO"` → ICU4X falla o muestra basura | `display_format` solo acepta códigos del `format_map` — falla temprano |
| El dashboard muestra valores sin enmascarar (PII expuesto) | `.mask()` se aplica automáticamente al leer atributos con `classification >= CONFIDENTIAL` |
