# Anexo A.10 — Revocación y Eliminación de Accesos v1.0
## Documento de respaldo: revocación inmediata, suspensión, offboarding, revisión periódica y privilege creep
...
**Respalda a:** MANUAL-GOBERNANZA-IGA (7.01) · MANUAL-USER-TEMPLATE (1.08 §7 LEAVER) · MANUAL-AUDITORIA (5.01 — ghost accounts) · A.01 §B2/§B13 (vigencia y revisión del rol)
**Fuentes de origen (cita histórica):** `SBOS-BAUTH-ACCESS-REVOCATION-REMOVAL` v1.0
**Normas base:** NIST SP 800-53 AC-2(3) (deshabilitación) · AC-6 (least privilege / creep) · ISO 27001 A.9.2.1/A.9.2.2/A.9.2.6 · RGPD/Ley 843 (retención tras baja)

---

## 1. Propósito y cómo citarlo

Respaldo de las políticas de **salida y contracción del acceso**: revocación inmediata (<30 s),
suspensión temporal reversible, offboarding completo, revisión periódica de accesos, detección
de privilege creep, terminación de emergencia y cuentas huérfanas (ghost accounts). **Cómo
citarlo:** `A.10 §2` (vista) · secciones 2–9 del traslado.

**Autosuficiencia:** la revocación la ejecuta bAuth nativamente (invalidación de ctx → tokens
muertos — 1.08 §7.4); menciones de época bajo ADR-010 (**Keycloak y Tryton eliminados**).

## 2. Las políticas en una vista

| Política | Contenido normado | Norma |
|---|---|---|
| Revocación inmediata | **< 30 segundos** desde el disparador: sesiones invalidadas, tokens muertos, credenciales físicas desactivadas | AC-2(3) · 1.08 §7.4 |
| Suspensión temporal | Reversible (investigación, licencia) — acceso bloqueado, identidad conservada | ISO A.9.2.6 |
| Offboarding completo | La secuencia de eliminación con retención legal posterior (Bolivia: 10 años Ley 843; el uuid jamás se reutiliza) | AC-2(3) · RGPD |
| Revisión periódica | Access review con SLA y escalación; sin respuesta → acción | ISO A.9.2.1 · A.01 §B13 |
| Privilege creep | Detección de permisos no usados (umbral) → alerta/recorte | AC-6 |
| Terminación de emergencia | Inmediata, con quórum post-evento | ISO A.9.2.2 · A.03 §3 (break-glass) |
| Ghost accounts | Detección de cuentas huérfanas (sin dueño/uso) → suspensión | `aud_ghost_account` (5.01) |
| Reglas no negociables | Las del diseño (traslado, sección 9) — fail-closed | — |

## 3. Verificación de completitud

| Verificación | Resultado |
|---|---|
| Cobertura del ciclo de salida (revocar→suspender→offboard→revisar→creep→emergencia→huérfanas) | ✅ íntegro en §4 |
| Coherencia con el JML soberano | ✅ — LEAVER <30 s (1.08 §7.4) |
| Coherencia con el reconcile del rol | ✅ — TEMPORARY/EMERGENCY con de-asignación automática (A.01 §B2) |
| Materialización de auditoría | ✅ — `aud_review` / `aud_ghost_account` en DDL (5.01 §11.1) |
...
|---|---|---|
| Invalidación de sesión (ctx → tokens muertos) | `src/context/engine.rs` (invalidate) | ✅ mecanismo real |
| Sync/reconcile (JML) | `src/sync/mod.rs` (**solo 173 líneas** — el `sync/` está casi vacío, 0.00 §8) | ⚠️ delgado |
| Tablas de revisión/huérfanas | `aud_review` / `aud_ghost_account` en DDL (A.30) | ✅ DDL, ❌ motor |
| Motor de campañas / barrido de huérfanas | no existe (A.30-IG1/IG2) | ❌ |

**Hallazgo crudo:** la revocación INMEDIATA (invalidar ctx) tiene mecanismo, pero el
**offboarding automatizado, el access review y el barrido de huérfanas — los motores IGA — no
existen** (`sync/` es 173 líneas). Las tablas están (A.30), los motores faltan. El "< 30 s" es
alcanzable por el mecanismo de invalidación, pero el ciclo completo LEAVER automatizado desde
RRHH está en L1-L2.
...
|---------|-------|-------------|
| 1.1.0 | 2026-07-11 | **Añadida verificación de código real** (§4.bis: invalidación ctx real pero motores IGA (offboarding/review/barrido) ausentes; sync/ solo 173 líneas). |
| 1.0.0 | 2026-07-11 | Anexo inicial: las 8 políticas de salida/contracción del acceso en una vista con sus normas (revocación <30 s, suspensión, offboarding con retención Ley 843, access review, privilege creep AC-6, emergencia, ghost accounts), verificación de coherencia con JML/reconcile/auditoría y traslado fiel íntegro. |
