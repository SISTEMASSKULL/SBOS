...
**Versión:** 1.5.0 · **Fecha:** 2026-07-13 · **Estado: COBERTURA TOTAL** — los 38 manuales con anexo(s); 44 anexos publicados (14 de traslado A.01–A.14 + 26 de sustentación A.15–A.40 + 4 especiales A.41–A.44 + 4 AtomLang A.45–A.48)
...
| ✅ [A.45 — Fundamentos Normativos AtomLang](A.45_ANEXO-FUNDAMENTOS-NORMATIVOS-ATOMLANG-v1.0.md) | B+C | **Mapa normativo completo de los 17 constructos de AtomLang**: XACML 3.0 (PolicySet/Policy/Rule/Target/Condition/Effect/combining_algorithm), PAP/PIP/PDP/PEP (NIST SP 800-162), Zonas B6 (NIST SP 800-207 §2.2), Aplicaciones (NIST SP 800-53 AC-3 + TOGAF 10 §B.3), Dominios D00–D13 (ISO 24760-2:2025), `bauth_config_param` como PIP (NIST SP 800-162 §5), Blocks B0–B19 (convención bAuth), tabla maestra §12 con 17 constructos × 5 normas — **PUBLICADO** | 2.13 §2–§6 · 2.14 §3–§7 · 2.05 §2 |
| ✅ [A.46 — Gramática y Compilador atomc](A.46_ANEXO-ATOMLANG-GRAMATICA-COMPILADOR-v1.0.md) | A+D | **Especificación técnica completa**: EBNF canónica del árbol SOURCE (2 versiones reconciliadas en 1), reglas G-01..G-10, tipos por data_type (AMOUNT/CURRENCY/NUMERIC…), JSON Schema Atom.v1 + CompiledTree.v1, algoritmo evaluador formal (3 etapas), `aggregate-strictest` con caso de prueba, catálogo de errores ATOMC-E-011..061 + ATOMC-W-011..031 con formato de diagnóstico JSON, arquitectura interna `atomc` (3 fases), DDL `bos_atom_compiled` WORM, estado de implementación verificado (schema `atomlang` en VPS + ~45 gaps D1) — **PUBLICADO** | 2.13 §3–§6 |
| ✅ [A.47 — Clasificación y Composición del Árbol](A.47_ANEXO-CLASIFICACION-COMPOSICION-ARBOL-v1.0.md) | B+C | **Reglas de posicionamiento de cada nodo + diagnóstico D1 as-is**: árbol de decisión para clasificar PolicySet/Policy/Rule/D98 (§2.1), reglas A-01..A-04 con norma, decisión de `combining_algorithm` por tipo de contenedor, reglas R-D98-01..R-D98-05, base XACML de los 2 tipos de Atom, glosario completo 17 conceptos × 5 plataformas (XACML/NIST 800-162/AWS IAM/Azure AD/bAuth), diagnóstico nodo a nodo del árbol D1 as-is (9 diagnósticos + árbol corregido completo), 4 valores de Decision y por qué auditarlos todos, catálogo de verbos `bos_verb` — **PUBLICADO** | 2.14 §8–§10 |
| ✅ [A.48 — Parametrización bauth_config_param](A.48_ANEXO-PARAMETRIZACION-BAUTH-CONFIG-PARAM-v1.0.md) | B+C+A | **SSOT del catálogo de parámetros del PIP bAuth**: fundamento NIST SP 800-162 §5 + XACML 3.0 §7.2 + ISO 27001:2022 A.5.15, distinción PIP dinámico (Context Plane) vs PIP estático (bauth_config_param), criterios de inclusión (varía-por-tenant/región/tiempo/compliance), catálogo inicial de 20 parámetros tipados (AMOUNT/CURRENCY/INTEGER/NUMERIC_PERCENT), referenciación desde AtomLang (`@bauth_config_param.<key>`), DDL completo con `bauth_param_audit` WORM (§5), cadena de precedencia global→tenant→domain, gobernanza con rol IAM_PARAM_ADMIN, ciclo de cambio sin recompilar árbol — **PUBLICADO** | 2.13 §4.5 · 2.14 §9 |
| A.49+ | — | Cobertura restante hasta que TODA afirmación relevante tenga respaldo (auditoría §3.1 en cada revisión) | todos |
...
| 2.13 AtomLang — Lenguaje | **A.45** ✅ (fundamentos normativos) · **A.46** ✅ (gramática + compilador atomc) | ✅ |
...
| 1.5.0 | 2026-07-13 | **AtomLang A.45–A.48 publicados: 38 manuales con cobertura, 44 anexos totales.** Nuevos manuales 2.13 (AtomLang) y 2.14 (Composición del Árbol) en Fase 2 Protección. A.45 mapa normativo completo 17 constructos × 5 normas (XACML 3.0, NIST SP 800-162, NIST SP 800-207, ISO 27001:2022, ISO 24760-2:2025, TOGAF 10). A.46 especificación técnica del compilador atomc: gramática EBNF canónica unificada (2 versiones reconciliadas), catálogo ATOMC-E-011..061 + ATOMC-W-011..031, algoritmo evaluador formal con `aggregate-strictest`, DDL `bos_atom_compiled` WORM, estado verificado (~45 gaps D1 en VPS, atomc L0). A.47 clasificación y diagnóstico: árbol de decisión, reglas A-01..A-04, reglas R-D98-01..R-D98-05, diagnóstico nodo-a-nodo D1 as-is + árbol corregido, glosario 17 conceptos × 5 plataformas. A.48 SSOT bauth_config_param: 20 parámetros tipados, DDL con audit WORM, cadena de precedencia, gobernanza IAM_PARAM_ADMIN. Archivos legacy `manual-estructuracion-politicas-roles-bAuth.md`, `ATOMLANG-DSL-SPEC-v1.0.md`, `AtomLang-especificacion-completa.md`, `ATOMLANG-GAP-ANALYSIS-D1-v1.0.md` absorbidos — **pendientes de eliminación (HITL)**.
