# Anexo A.31 — Los atributos de identidad: `idn_atributo` SIN DDL
...
**Verificación de código:** `DDLs/` (búsqueda `idn_atributo`) — leída 2026-07-11
...
|---|---|---|
| `idn_atributo` / `idn_tipo_atributo` en el DDL | `grep -rn 'idn_atributo' DDLs/` | **0 menciones** — la tabla no está en el esquema |

**Traducción cruda:** el corpus documenta profusamente `idn_atributo` como el destino de los
...
Directory: *"idn_atributo sin DDL"*) — aquí confirmada con grep.
...
regla de 1.08 §3 que dice que los 1:N van a `idn_atributo`), o no se guardan. El pilar Directory
...
|---|---|---|:---:|
| **AT1** | **DDL de `idn_atributo` + `idn_tipo_atributo`** — el catálogo de tipos y la tabla de valores 1:N | SCIM (multivaluados) · ISO 24760-1 §6 · 1.07 | **P1** (pilar Directory) |
| AT2 | Migrar los campos 1:N del UserTemplate JSONB (emails/phones/addresses) a `idn_atributo` una vez exista | 1.08 §3 (regla de almacenamiento) | P2 |
...
|---|---|
| Tabla `idn_atributo` en DDL | ❌ **no existe** (0 menciones) — hallazgo crudo |
...
**Del código:** `DDLs/` (grep idn_atributo = 0). **Del proyecto:** 1.07 · 1.06 · 1.08 §3 · A.02 §B2 · 0.00 §8.
...
|---------|-------|-------------|
| 1.0.0 | 2026-07-11 | Anexo inicial (tipo D+B): el estado crudo — **`idn_atributo` tiene 0 menciones en el DDL** (verificado), la tabla documentada como destino de los campos 1:N de la identidad (A.02 §B2, D00) NO existe en el esquema (confirma la brecha de 0.00 §8 pilar Directory con grep). Brechas AT1 (DDL de idn_atributo/idn_tipo_atributo P1), AT2 (migrar 1:N del JSONB), AT3 (clasificación), AT4 (extensibilidad por tenant). |
