# Anexo A.04 — Las Cadenas de Jerarquía v1.2
## Documento de respaldo: el DAG completo de herencia de roles, sector por sector CAEB

**Tipo:** ANEXO — documento de respaldo del corpus
**Versión del anexo:** 1.0.0 · **Fecha:** 2026-07-11
**Estatus:** FUENTE AUTOSUFICIENTE — contiene las cadenas COMPLETAS de los 21 sectores (traslado fiel, §6); el origen queda como cita histórica y no es fuente de lectura
**Respalda a:** MANUAL-ROLES (1.09) §9 (herencia y árbol) · MANUAL-BITMASK (1.04 — herencia OR sobre one-hot) · A.03 (el catálogo cuyas dependencias documenta)
**Fuentes de origen (cita histórica):** `BAUTH-CADENAS-JERARQUIA` v1.2 (jun-2026)
**Normas base:** NIST RBAC §4.2 (Hierarchical RBAC — el DAG como partial order) · NIST AC-5 (SoD) · ANSI INCITS 359 §3.2 · patrón Closure Table (SQL)
...
2. [Las cadenas en una vista](#2-las-cadenas-en-una-vista)
3. [La convención de lectura y las reglas del DAG](#3-la-convención-de-lectura-y-las-reglas-del-dag)
4. [Los 21 sectores y su materialización](#4-los-21-sectores-y-su-materialización)
5. [Verificación de completitud](#5-verificación-de-completitud)
6. [Traslado fiel — las cadenas completas](#6-traslado-fiel--las-cadenas-completas)
...
Respaldo del **grafo de herencia de privilegios**: la posición de cada rol de negocio (N0–N5)
en el DAG, para cada uno de los 21 sectores CAEB. *"Ningún rol existe aislado."* **Cómo
citarlo:** `A.04 §G` (las cadenas del sector Comercio) · `A.04 §3` (las reglas).

**Autosuficiencia de bAuth:** las cadenas las administra el PrivilegeEngine nativo — toda
mención de época a motores externos en el origen se lee bajo ADR-010 (**Keycloak y Tryton ya
no forman parte de la solución**). El propio artefacto lo declaraba: *"bAuth orquesta, no es
un motor más"*.

---

## 2. Las cadenas en una vista

| Propiedad | Valor |
|---|---|
| Qué define | La jerarquía completa de dependencias por sector: cada rol con su padre en el DAG |
| Convención | `Padre ← Hijo`: el Padre **hereda** los privilegios del Hijo (la flecha apunta junior → senior) |
| Motor | Las operaciones de herencia (OR/AND NOT) operan sobre **posiciones del Rol BitMask (one-hot)** — JAMÁS sobre códigos de átomo (label) — A.03 §6 · 1.04 |
| Materialización vigente | `parent_id` + `idn_role_closure` (seed `bauth_62`): **547 aristas directas · 1126 transitivas · 1673 en closure · profundidad máx. 7 · raíz única** (evidencia VPS, G-B01-04) |

---

## 3. La convención de lectura y las reglas del DAG

Del fundamento normativo (NIST RBAC §4.2 · INCITS 359 §3.2):

1. **El grafo es un DAG**: la herencia es reflexiva, transitiva y antisimétrica — los ciclos
   están prohibidos (validación DAG-check antes de guardar, A.01 §18).
2. **Herencia con remoción controlada**: un hijo puede heredar del padre menos bits removidos
   (`&^` — A.01 §B11); las cadenas documentan la línea base.
3. **SoD sobre las cadenas** (AC-5): las incompatibilidades (A.01 §B12) se validan sobre el
   conjunto EFECTIVO (directo + heredado + delegado).
4. **La pertenencia se resuelve por closure table** — una consulta, sin recursión; el nivel
   (`hierarchy_level`) es la profundidad REAL del nodo, derivada del árbol (G-B01-04).

---

## 4. Los 21 sectores y su materialización

El artefacto documenta las cadenas de los sectores CAEB **§A–§U**: Agricultura (§A), Minería
(§B), Manufactura (§C), Electricidad/Gas (§D), Agua/Desechos (§E), Construcción (§F), Comercio
(§G — el más extenso), Transporte (§H), Alojamiento/Comidas (§I), Información/Comunicaciones
(§J), Financiero/Seguros (§K), Inmobiliario (§L), Profesional/Científico (§M), Servicios
Administrativos (§N), Administración Pública (§O), Enseñanza (§P), Salud (§Q) y los restantes
hasta §U — cadena por cadena, en §6.

**Verificación contra BD:** el árbol real materializado tiene raíz única
(ROL-SYS-SUPERUSUARIO) y distribución por niveles 0–7 (1 · 34 · 152 · 168 · 144 · 40 · 8 · 1
— G-B01-04): las cadenas del artefacto son la FUENTE de diseño; el closure de la BD es el
estado real y coincide en cardinalidad (547 aristas directas para 548 nodos = árbol conexo).
...
|---|---|
| 21/21 sectores CAEB con cadenas | ✅ (§A–§U en §6) |
| Coherencia con el catálogo (A.03) | ✅ — es su anexo declarado (v1.2 alineada al catálogo v2.1 BitMask Dual) |
| Coherencia con la BD | ✅ — 547/1126/1673, profundidad 7, raíz única (evidencia VPS G-B01-04) |
| Regla one-hot en la herencia | ✅ — el artefacto la declara en su base normativa (BitMask Dual) |
| Cifra "186 aristas" de doctrinas previas | Resuelto: cifra de época (v1.1) — **la vigente es 547 directas en BD**; el artefacto v1.2 amplió las cadenas a los 21 sectores |

---

## 6. Traslado fiel — las cadenas completas

> Extracción literal del artefacto v1.2 (1014 líneas). Menciones de época bajo la aclaración
> de §1 — bAuth es autosuficiente.
...
## 6.bis Estado de materialización en código (verificado 2026-07-11)

| Pieza | Evidencia | Estado |
|---|---|---|
| Closure table poblada | seed `bauth_62__idn_role_closure.sql` — 547 aristas directas / 1126 transitivas / **1673 en closure** (evidencia VPS G-B01-04) | ✅ sembrado |
| El algoritmo de herencia | `src/bitmask/closure.rs` + `resolver.rs` (A.17) | ✅ real |
| `hierarchy_level` derivado | `WITH RECURSIVE` en `bauth_62` (548 roles, niveles 0-7) | ✅ |

**Veredicto:** las cadenas del artefacto están **materializadas** (closure real de 1673
relaciones, coincide con las cifras del artefacto). El DAG que documenta el anexo es el que corre.

## 7. Referencias e historial

**Del proyecto:** `BAUTH-CADENAS-JERARQUIA` v1.2 (origen) · A.03 · 1.09 §9 · 1.04 · seed `bauth_62`.
**Normas:** [NIST RBAC](https://csrc.nist.gov/projects/role-based-access-control) · [INCITS 359](https://blog.ansi.org/ansi/role-based-access-control-rbac-incits-359/) · [NIST AC-5](https://csrc.nist.gov/publications/detail/sp/800/53/rev-5/final)

| Versión | Fecha | Descripción |
|---------|-------|-------------|
| 1.1.0 | 2026-07-11 | **Añadida verificación de código real** (§6.bis: closure materializado (bauth_62, 1673 relaciones) — el DAG documentado es el que corre). |
| 1.0.0 | 2026-07-11 | Anexo inicial: convención `Padre ← Hijo`, reglas del DAG (aciclicidad, remoción controlada, SoD sobre el conjunto efectivo, closure), los 21 sectores CAEB, verificación contra la BD real (547/1126/1673, profundidad 7, raíz única — la cifra "186 aristas" resuelta como de época) y traslado fiel completo. |
