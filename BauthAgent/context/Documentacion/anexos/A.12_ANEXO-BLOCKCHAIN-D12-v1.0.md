# Anexo A.12 — Blockchain como Capa de Confianza Verificable (D12) v2.1
## Documento de respaldo: suficiencia arquitectónica, las tres variantes D12 y el catálogo de productos

**Tipo:** ANEXO — documento de respaldo del corpus
**Versión del anexo:** 1.0.0 · **Fecha:** 2026-07-11
**Estatus:** FUENTE AUTOSUFICIENTE — contiene el proyecto COMPLETO (traslado fiel, §4)
**Respalda a:** MANUAL-BLOCKCHAIN-D12 (5.02) · MANUAL-DOMINIOS (1.01 — D12) · A.01 §17.2-B19 (`blockchain_access` del rol) · A.05 §D12 (átomos `blk`) · MANUAL-FIRMA (2.04 — anclaje D12↔D13)
**Fuentes de origen (cita histórica):** `SBOS-MANUAL-BAUTH-D12-BLOCKCHAIN-WALLET-CONTROL` v2.1
**Normas base:** RFC 6962 (Merkle) · NIST IR 8202 · QBFT (Besu) · EIP-155/725/735 · W3C DID · Ley 164 (anclaje con firma legal)

---

## 1. Propósito y cómo citarlo

Respaldo del proyecto D12: por qué el plano de control YA es suficiente para una billetera de
pagos (D3 límites + D11 auditoría), las **tres variantes de incorporación de blockchain**
(anclaje Merkle · liquidación on-chain · plano de control como producto), el catálogo de
productos y el patrón de implementación con roadmap. **Cómo citarlo:** `A.12 §5` (las
variantes — del traslado) · `A.12 §2` (vista).

**Autosuficiencia:** la cadena soberana (Besu QBFT) es infraestructura del stack; el acceso es
SIEMPRE vía el daemon de conectividad, jamás directo (A.01 §17.2-B19). Menciones de época bajo
ADR-010 (**Keycloak y Tryton eliminados**).

## 2. El proyecto en una vista

| Parte | Contenido |
|---|---|
| I — Suficiencia | Lo que una billetera necesita controlar YA lo dan D3 (límites/aprobaciones) + D11 (auditoría WORM): la blockchain no es prerequisito — es capa de confianza ADICIONAL |
| II — D12, tres variantes | **A: anclaje** (lotes Merkle de auditoría anclados a la cadena — verificación offline RFC 6962) · **B: liquidación** (operaciones on-chain con reconciliación) · **C: el plano de control como producto** (vender el control, no operar la billetera) |
| III — Implementación | Patrón de integración (motor Merkle, cuentas `blk_account`, anclas `blk_anchor`, reconciliación) + roadmap |
| Materialización | Tablas `blk_*` en DDL · motor Merkle en código (`merkle.rs` — RFC 6962, Keccak-256, 1M eventos, proof offline — 5.01 §11.1) · átomos D12 (A.05: `anchor_enabled`, `anchor_variant` A/B, `merkle_batch_size`…) |

## 3. Verificación de completitud

| Verificación | Resultado |
|---|---|
| Cobertura del plano D12 | ✅ — variantes A/B/C + productos + roadmap íntegros en §4 |
| Coherencia con el rol/sujeto | ✅ — B19 del rol (A.01) y wallet del sujeto (A.02 U6) materializan este diseño |
| Coherencia D12↔D13 | ✅ — el anclaje (D12) y la firma legal (D13) separados (resolución A.01 §17.4-1); la relación se declara en el anclaje con Ley 164 |
| Estado honesto | Motor Merkle ✅ código · anclaje probado, **sin producción** (madurez L2 — 0.00 §7) |

## 4. Traslado fiel — el proyecto completo
...
|---|---|---|
| Merkle Engine (RFC 6962, anclaje) | `src/domain/merkle.rs` (**205 líneas** — Keccak-256, proof) | ✅ real |
| Evaluador D12 | `src/domain/blockchain.rs` (**62 líneas** — delgado, A.21) | ⚠️ parcial |
| Tablas `blk_*` (account, anchor, merkle_batch, leaf) | DDL | ✅ |
| Anclaje en producción | probado, **sin producción** (0.00 §7 L2) | ⚠️ |

**Veredicto:** el Merkle Engine es real (205 líneas) — la pieza técnica del anclaje existe. El
evaluador de dominio D12 es delgado (62 líneas) y el anclaje no está en producción (L2). Coherente
con §3 de este anexo (madurez honesta L2).

## 5. Referencias e historial

| Versión | Fecha | Descripción |
|---------|-------|-------------|
| 1.1.0 | 2026-07-11 | **Añadida verificación de código real** (§4.bis: merkle.rs real (205 líneas) pero blockchain.rs delgado (62) y anclaje sin producción). |
| 1.0.0 | 2026-07-11 | Anexo inicial: la suficiencia arquitectónica (D3+D11 bastan; blockchain = confianza adicional), las tres variantes D12 (anclaje/liquidación/producto), materialización real (blk_*, merkle.rs, átomos A.05) con madurez honesta L2, y traslado fiel íntegro del proyecto v2.1. |
