# Anexo A.08 — Los Motores de Firma Digital v1.0
## Documento de respaldo: doble motor — PKI propia (interno) + ADSIB/SIN Bolivia (externo)

**Tipo:** ANEXO — documento de respaldo del corpus
**Versión del anexo:** 1.0.0 · **Fecha:** 2026-07-11
**Estatus:** FUENTE AUTOSUFICIENTE — contiene el diseño COMPLETO (traslado fiel, §5)
**Respalda a:** MANUAL-FIRMA-DIGITAL (2.04) · A.01 §B20 (política D13 del rol) · A.02 §19.2-U6 (certificado del sujeto) · MANUAL-TOKENS (2.03 — firma de JWT)
**Fuentes de origen (cita histórica):** `SBOS-BAUTH-DIGITAL-SIGNATURE-ENGINES` v1.0
**Normas base:** RFC 8032 (Ed25519) · FIPS 186-5 · Ley 164 Bolivia Art. 78 · DS 1793/3527 · ADSIB-FD-POLT-015 v2.3 · SIN RND 102100000011 · X.509/RFC 5280

---

## 1. Propósito y cómo citarlo

Respaldo del diseño del **doble motor de firma**: el interno (velocidad e integridad — PKI
propia vía la bóveda, EdDSA Ed25519) y el externo (validez jurídica plena ante terceros —
certificación ADSIB, RSA-SHA256). **Cómo citarlo:** `A.08 §2` (motor interno) · `A.08 §3`
(motor externo). **Frontera:** la doctrina de firma (cuándo, cómo, comparativa operativa) vive
en 2.04 — aquí el diseño de origen íntegro.

**Autosuficiencia:** ambos motores los orquesta bAuth nativamente (el interno con la bóveda del
stack; el externo con la PKI nacional). Menciones de época en el traslado bajo ADR-010
(**Keycloak y Tryton eliminados**).

## 2. El diseño en una vista

| Dimensión | Motor INTERNO | Motor EXTERNO |
|---|---|---|
| Algoritmo | EdDSA **Ed25519** (RFC 8032) | **RSA-SHA256** (X.509 ADSIB) |
| Custodia de claves | Bóveda del stack (PKI propia) | Certificado emitido por ADSIB (la persona) |
| Validez | Integridad y no repudio INTERNO del ecosistema | **Jurídica plena** (Ley 164 Art. 78) — oponible a terceros |
| Usos | Firma de contratos de rol/usuario, certificados de compliance, exports de retención (1.13 §10.3), JWT | Documentos con efecto legal, facturación SIN (CUFD/CUF), contratos con terceros |
| Renovación | Rotación por política de la bóveda | Certificado 365 días — renovación ADSIB **presencial** |
| Revocación | Deshabilitación en bóveda | CRL ADSIB (publicación ≤24 h) |

**La relación D12↔D13 (resuelta en A.01 §17.4-1 y A.02 U6):** el ROL define cuándo una
operación exige firma legal (`B20 legal_signature_policy`); el SUJETO porta el certificado
ADSIB y la wallet (`legal_signature_identity`); este anexo respalda los MOTORES con que se
ejecuta.
...
|---|---|
| Doble motor completo (arquitectura, flujos, API JSON-RPC de firma) | ✅ — íntegro en §5 (secciones 1–7 del diseño) |
| Integración con el catálogo de roles | ✅ — sección 6 del diseño (qué roles firman con qué motor) |
| Coherencia con la nomenclatura PQC | El plan post-cuántico se lee con la nomenclatura FIPS vigente (ML-DSA — resolución A.02 U2) |
| Normativa boliviana | ✅ — Ley 164/DS 1793/3527/ADSIB/SIN citadas en el diseño y catalogadas en 7.03 §7 |

## 3.bis Estado de materialización en código (verificado 2026-07-11)

| Motor | Evidencia en código | Estado |
|---|---|---|
| **INTERNO Ed25519** | `jwt_signer.rs` firma JWT con EdDSA Ed25519 usando `ring` (FIPS 186-5, RFC 8032) · `ed25519-dalek` en Cargo.toml | ✅ **REAL** — el motor interno funciona |
| Clave del motor interno | `jwt_signer.rs` cabecera: *"Modo development: clave generada en memoria. Producción: Vault PKI"* | ⚠️ **dev en memoria** — el cableado a la bóveda (Vault PKI) declarado pero **por verificar** |
| **EXTERNO ADSIB (RSA-SHA256, validez legal Ley 164)** | `grep adsib/rsa_sha src/ → 0` real (el RSA que aparece es `saml_signature.rs`, verificación de firmas SAML entrantes — NO el motor de firma legal) | ❌ **NO EXISTE en código** |

**Hallazgo crudo:** el "doble motor" está **medio implementado**. El interno Ed25519 (velocidad,
integridad interna) es real. El **externo ADSIB — la firma con validez jurídica plena (Ley 164),
la pieza más compleja y la única oponible a terceros — no está en código.** Es diseño (§5), no
implementación.

**Brechas de código:**
| # | Brecha | Prioridad |
|---|---|:---:|
| F-C1 | **Motor externo ADSIB ausente** — sin él, no hay firma con validez jurídica (facturación SIN, contratos con terceros) | P1 |
| F-C2 | Clave del motor interno en memoria (dev) — cablear Vault PKI para producción | P1 |
| F-C3 | Perfiles INT-LT (long-term, para auditoría/AU-9) por verificar en código | P2 |

## 4. Mapa anexo → manuales

§2 motores → 2.04 §3-§4 · relación D12/D13 → A.01/A.02 · API de firma → 9.02 · normativa → 7.03 §7.

## 5. Traslado fiel — el diseño completo
...
|---------|-------|-------------|
| 1.1.0 | 2026-07-11 | **Añadida verificación de código real** (§3.bis: motor interno Ed25519 REAL (jwt_signer/ring) pero motor EXTERNO ADSIB NO existe en código (F-C1 P1); clave interna en memoria dev (F-C2)). |
| 1.0.0 | 2026-07-11 | Anexo inicial: el doble motor en una vista (comparativa interno/externo con normas), la relación D12↔D13 resuelta, verificación de completitud y traslado fiel íntegro (arquitectura, ambos motores, comparativa, criterios de uso, integración con roles, API JSON-RPC). |
