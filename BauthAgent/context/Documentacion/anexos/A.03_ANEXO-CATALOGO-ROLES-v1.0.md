# Anexo A.03 — El Catálogo de Roles Empresariales v2.1
## Documento de respaldo: todos los actores del sistema, la jerarquía sistémica y el motor BitMask Dual
...
**Estatus:** FUENTE AUTOSUFICIENTE — contiene el catálogo COMPLETO (traslado fiel, §10); la documentación de origen queda como cita histórica y **no es fuente de lectura**
**Respalda a:** MANUAL-ROLES (1.09) §2.3/§3 (los 548 roles, taxonomía) · MANUAL-BITMASK (1.04) §BitMask Dual · MANUAL-DOMINIOS (1.01) · MANUAL-D99 (2.06 — roles sistémicos)
**Fuentes de origen (cita histórica):** `BAUTH-CATALOGO-ROLES-EMPRESARIALES` v2.1 (jun-2026)
**Normas base:** NIST RBAC Model (4 niveles — SBOS implementa Nivel 3 Constrained) · NIST SP 800-53 AC-2/AC-5/AC-6 · ISO 27001:2022 A.8.2 (acceso privilegiado) · NIST SP 800-63-4 (IAL2/AAL2/FAL2 privilegiados) · PAM (JIT, break-glass, session recording) · ISO 9001:2015 §3.2.4 (definición de cliente) · CAEB SIN Bolivia (RND 102100000020)
...
2. [El catálogo en una vista](#2-el-catálogo-en-una-vista)
3. [La jerarquía sistémica — SU y N1–N4](#3-la-jerarquía-sistémica--su-y-n1n4)
4. [Actores internos — el personal por sector](#4-actores-internos--el-personal-por-sector)
5. [Actores externos — los clientes del sistema (CAEB)](#5-actores-externos--los-clientes-del-sistema-caeb)
6. [El motor BitMask Dual — la corrección arquitectural v2.1](#6-el-motor-bitmask-dual--la-corrección-arquitectural-v21)
7. [Políticas mínimas de registro y plantillas](#7-políticas-mínimas-de-registro-y-plantillas)
8. [Verificación de completitud y coherencia de cifras](#8-verificación-de-completitud-y-coherencia-de-cifras)
9. [Mapa anexo → manuales](#9-mapa-anexo--manuales)
10. [Traslado fiel — el catálogo completo](#10-traslado-fiel--el-catálogo-completo)
11. [Referencias e historial](#11-referencias-e-historial)

---

## 1. Propósito del anexo y cómo citarlo

Respaldo documental del catálogo integral de actores de bAuth: quiénes participan en el
sistema (internos, externos, sistémicos, M2M), bajo qué taxonomía normada, con qué jerarquía
de administración y sobre qué motor de privilegios. **Cómo citarlo:** `A.03 §3` (jerarquía
sistémica) · `A.03 §6` (BitMask Dual) · `A.03 §10` (el catálogo íntegro).

**Frontera:** la doctrina del sistema de roles (RolTemplate, tabla, ciclo de vida, versionado)
vive en MANUAL-ROLES (1.09); la doctrina del BitMask en MANUAL-BITMASK (1.04). Este anexo
respalda con el catálogo completo y su fundamento.

**Autosuficiencia de bAuth:** toda mención a motores externos de identidad o ERP en el
artefacto de origen es de época — **Keycloak y Tryton ya no forman parte de la solución**
(ADR-010); bAuth administra nativamente la identidad, el BitMask y el enforcement. El propio
catálogo v2.1 ya lo declaraba: *"bAuth administra el BitMask, no KC ni Tryton-PDP"*. Las
apariciones de época en §10 se leen bajo esta aclaración.

---

## 2. El catálogo en una vista

| Propiedad | Valor |
|---|---|
| Qué define | **Todos los actores** del sistema empresarial: internos (personal), externos (clientes ISO 9001), sistémicos (plataforma) y M2M — organizados por sector económico **CAEB del SIN** |
| Premisa ISO 9001:2015 §3.2.4 | **Todo ente que recibe un producto o servicio del sistema es un CLIENTE**: el alumno del centro educativo, el paciente del hospital, el ciudadano de la administración — todos necesitan identidad digital en bAuth (facturas, firma, portales, derechos ARCO) |
| Para qué | Cada rol → **plantilla RolTemplate predefinida**: una empresa nueva se registra con todos sus roles operativos en <10 minutos |
| Fuentes declaradas | ISO 9001:2015 · CAEB SIN (21 secciones) · U.S. News Best Jobs 2026 · OIT LATAM 2024 · INE Bolivia 2024 · Censo 2024 · Cámaras de Comercio |
| Materialización vigente | **548 roles en BD** (`idn_role_template`, seed `bauth_48` — evidencia VPS en G-B01-03/04): niveles 0–7 del árbol real, catálogo de tipos NIST AC-2 |

---

## 3. La jerarquía sistémica — SU y N1–N4

**Rol sistémico:** identidad que opera sobre la plataforma misma. Base normativa declarada del
catálogo: NIST RBAC Nivel 3 (Constrained, con SoD estático y dinámico) · ISO 27001 A.8.2
(aprobación formal, least privilege, inventario vivo, revisión trimestral, MFA obligatorio) ·
NIST AC-2/AC-5/AC-6 · 800-63-4 (IAL2/AAL2/FAL2 para humanos privilegiados; mTLS para M2M) ·
PAM (cuentas separadas admin/diarias, JIT para SU, break-glass con aprobación post-evento,
session recording en SU/N1).

**El modelo de alcance decreciente** (delegación controlada — cada nivel recibe un subconjunto
de la autoridad del superior; **quien crea un admin no puede ser ese admin** — AC-5):

| Nivel | Alcance | Quién lo asigna | SoD | RBAC |
|---|---|---|---|---|
| **SU** Superusuario | Todo el ecosistema | Bootstrap inicial | Único activo · break-glass con aprobación post-evento | Nivel 3 |
| **N1** Plataforma | Todos los módulos, tenants, datos | SU | Admin Proyecto ≠ Admin Seguridad ≠ Admin Infra | Nivel 3 |
| **N2** Módulo/Daemon | Un daemon o subsistema | Admin Plataforma | Admin Módulo ≠ Admin Datos ≠ Admin Bóveda | Nivel 2 |
| **N3** Tenant/Sucursal | Una organización | Admin bAuth | Admin Tenant ≠ Admin Seguridad ≠ Admin Facturación | Nivel 2 |
| **N4** Bootstrap/Interno | Su propio daemon | Automático (`bosctl setup`) | Identidades M2M, sin acceso humano, mTLS obligatorio | Nivel 1 |

El detalle de cada rol sistémico (S001…, con propósito, alcance y controles) está íntegro en
§10 (traslado, sección 2 del catálogo).

---

## 4. Actores internos — el personal por sector

**Rol interno:** quien presta el servicio dentro de la organización (el "proveedor" de la
cadena de calidad ISO 9001) — relación laboral, credenciales, permisos.

**Formato de cada entrada del catálogo:** `# · Rol · Código bAuth (ROL-*) · Descripción · Tipo
(Operativo/Supervisión/Técnico/Gerencia Media/…) · Estado · Bitácora · Plantilla`. Los roles
están numerados por familias sectoriales (comercio 1–120, tributario-fiscal 121+ con fuente
SIN RND 102100000020, y así por sector CAEB — **174 roles internos** en el artefacto v2.1).
El listado completo, rol por rol, está en §10 (traslado, sección 3).

**Valor normativo:** los roles fiscales bolivianos (facturación electrónica, dosificación,
RC-IVA/IT/IUE, archivo fiscal 8 años, NITEX/exportaciones, DUI/importaciones) derivan
directamente de la normativa del SIN — no son invenciones de diseño.

---

## 5. Actores externos — los clientes del sistema (CAEB)

Por la premisa ISO 9001 §3.2.4 (§2), cada una de las **21 secciones CAEB del SIN** define sus
actores externos (cliente/alumno/paciente/feligrés/ciudadano/proveedor…), cada uno con su
código `ROL-EXT-*` y su plantilla. El listado íntegro por sección está en §10 (traslado,
sección 4). La jerarquía organizacional genérica actualizada (sección 5 del catálogo) los
integra al árbol.

**Materialización verificada:** la distribución vigente en BD por tipo NIST (G-B01-03):
INDIVIDUAL 334 · EXTERNAL 162 · M2M 29 · SYSTEM 19 · GUEST 4 = **548**.

---

## 6. El motor BitMask Dual — la corrección arquitectural v2.1

**La corrección (junio 2026, documentada en el propio catálogo):** el modelo anterior aplicaba
OR/AND directamente sobre un solo u64 con códigos de átomo — eso produce **escalamiento
silencioso de privilegios**: `1 OR 2 = 3` → el código 3 es "eliminar", un permiso que nadie
otorgó. **Raíz del error: confundir dos propósitos en la misma estructura.** El modelo vigente
(ADR-009 · MANUAL-BITMASK 1.04) separa:

| Propósito | Pregunta | Estructura | Codificación | ¿Bitwise combinable? |
|---|---|---|---|---|
| **Identificar** un átomo | ¿Qué acción es esta? | **BitMask Átomo** (64 bits) | Label encoding | ❌ NUNCA — solo igualdad y extracción de campos con máscara fija |
| **Combinar** átomos entre roles | ¿Quién tiene qué? | **Rol BitMask** (N bits) | One-hot encoding | ✅ OR / AND / XOR / AND NOT |

**La anatomía del BitMask Átomo (64 bits)** — una dirección estructurada, no una bandera:

```
Dominio Contextual (32 bits) — DÓNDE vive el átomo:
  bits 0-7  reservado · bits 8-11 dominio de soberanía (16 valores, D1–D12)
  bits 12-20 aplicación (512) · bits 21-31 grupo funcional (2.048 por app)
Dominio Lógico (32 bits) — QUÉ hace:
  bits 0-5  reservado · bits 6-7 estado de política (no aplica/pendiente/aprobado/rechazado)
  bits 8-31 código del verbo (16.777.216 átomos por grupo)
Vocabulario fijo y global: nuevo=1 · editar=2 · eliminar=3 · ver=4
```

**Herencia:** el DAG (NIST RBAC §4.2 — reflexiva, transitiva, antisimétrica, acíclico) opera
con OR **solo sobre el Rol BitMask**; la pertenencia jerárquica se resuelve con **closure
table** precomputada (el patrón SQL usado por los motores de autorización de la industria —
una consulta JOIN, sin recursión). Materializado en `idn_role_closure` (seed `bauth_62`: 547
aristas directas · 1126 transitivas · 1673 total · profundidad máx. 7).

---

## 7. Políticas mínimas de registro y plantillas

- **Políticas mínimas de registro de roles** (sección 7 del catálogo): las 9 verificaciones
  pre-registro y los requisitos por tier — íntegras en §10.
- **Plantillas predefinidas** (sección 8) + **escalabilidad** (sección "de la tienda de barrio
  a la corporación"): los roles se activan por sector y tamaño (`applies_to_size`) — el mismo
  catálogo sirve a un kiosco y a una corporación. **Áreas organizacionales 1:N** con roles
  (sección 9).
- `[época — ADR-010]` La sección 7 del artefacto incluye ejemplos de configuración de *realms*
  y políticas de bóveda del motor de identidad externo de entonces: **Keycloak ya no forma
  parte de la solución** — bAuth es autosuficiente (motor nativo, OIDC Provider propio — 2.01);
  esos ejemplos permanecen solo en el traslado §10 como registro histórico. La política de la
  bóveda (Vault PKI para certificados de daemons) sí es del stack vigente.

---

## 8. Verificación de completitud y coherencia de cifras

| Hallazgo | Verificación · resolución |
|---|---|
| **Cifras entre épocas:** el artefacto v2.1 documenta 174 internos + externos CAEB; doctrinas previas citaban "368 roles / 66 plantillas" | **La cifra vigente es la de la BD: 548 roles** (seed `bauth_48`, evidencia VPS G-B01-03/04, distribución §5). El catálogo aporta la TAXONOMÍA y las plantillas; la BD es el estado real. 1.09 §2.3 ya lo declara así |
| **Numeración duplicada** en el artefacto (dos secciones "7", "8", "9") | Sin ambigüedad de contenido (títulos distintos); en §10 se conservan tal cual (traslado fiel); las citas de este anexo usan el título, no el número |
| **Cobertura de la jerarquía** | Los 5 niveles sistémicos (SU/N1–N4) + 10 niveles organizacionales del negocio están representados en el árbol real (profundidad 0–7 en BD — la diferencia niveles-declarados vs profundidad-real la explica 1.09 §9: no todo nivel taxonómico genera un salto de árbol) |
| **BitMask Dual** | El §6 del catálogo v2.1 es LA fuente de la corrección — coherente con ADR-009 y 1.04; el anexo la respalda íntegra |
| **Menciones KC/Tryton** | Marcadas con la aclaración de autosuficiencia (§1, §7) — regla permanente del corpus |

---

## 8.bis Estado de materialización en código (verificado 2026-07-11)

| Pieza | Evidencia | Estado |
|---|---|---|
| Los 548 roles | seed `bauth_48__idn_role_template.sql` — **548 `uuidv7()`** (grep) | ✅ sembrado (no 368 de época) |
| El motor BitMask Dual (§6) | `src/bitmask/` (2.640 líneas, 11 módulos — A.17) | ✅ real, L3 |
| Closure de dependencias (§4-§5) | seed `bauth_62` (1673 relaciones, prof. 7) | ✅ |
| Catálogo de átomos que alimenta los roles | D1 generativo sí; **D2–D13 sin sembrar** (A.05/A.17-C1) | ⚠️ parcial |

**Veredicto:** el catálogo de roles y el motor que los evalúa son **reales y sembrados** (548
roles, closure, BitMask Dual). La brecha es el catálogo de átomos de dominios (D2–D13) que aún
no tiene seed — sin él, los roles existen pero los planos External no tienen bits que evaluar.

## 9. Mapa anexo → manuales

| Sección | Respalda a |
|---|---|
| §3 (jerarquía sistémica SU/N1-N4) | 2.06 D99 · 7.01 IGA (PAM) · 1.09 §3 |
| §4-§5 (actores por sector CAEB) | 1.09 §2.3/§3 · 1.12 (multi-tenant) |
| §6 (BitMask Dual) | **1.04 BitMask** · 1.03 Átomos · A.01 §B9 |
| §7 (políticas de registro, plantillas) | 1.09 §13 (gestión operativa) · 2.05 |
| §8 (verificación) | 1.09 · el doc de gaps (tracking) |
| §10 (catálogo íntegro) | Fuente autosuficiente |

---

## 10. Traslado fiel — el catálogo completo

> **Regla de autosuficiencia:** el catálogo COMPLETO (v2.1, 2255 líneas) trasladado por
> extracción literal. Los nombres de época (realms del motor externo, menciones KC/Tryton) se
> leen bajo la aclaración de §1: **ya no forman parte de la solución — bAuth es autosuficiente**.
...
## 11. Referencias e historial

**Del proyecto:** `BAUTH-CATALOGO-ROLES-EMPRESARIALES` v2.1 (origen histórico) · MANUAL-ROLES
(1.09) · MANUAL-BITMASK (1.04) · ADR-009 (BitMask Dual) · ADR-010 · seeds `bauth_48`/`bauth_62`.

**Normas:** [NIST RBAC](https://csrc.nist.gov/projects/role-based-access-control) · [NIST SP 800-53 AC-2/5/6](https://csrc.nist.gov/publications/detail/sp/800/53/rev-5/final) · [ISO 27001:2022](https://www.iso.org/standard/27001) · [NIST SP 800-63-4](https://pages.nist.gov/800-63-4/) · [ISO 9001:2015](https://www.iso.org/standard/62085.html) · [SIN Bolivia — CAEB](https://www.impuestos.gob.bo/)

| Versión | Fecha | Descripción |
|---------|-------|-------------|
| 1.1.0 | 2026-07-11 | **Añadida verificación de código real** (§8.bis: 548 roles sembrados (bauth_48) + BitMask Dual real (2.640 líneas); átomos D2-D13 sin sembrar). |
| 1.0.0 | 2026-07-11 | Anexo inicial con el patrón completo (autosuficiencia + frontera + aclaración KC/Tryton permanente). Cura el catálogo v2.1: premisa ISO 9001 §3.2.4 (todo receptor de servicio es cliente → identidad digital), jerarquía sistémica SU/N1–N4 con su base normativa (RBAC N3, A.8.2, AC-2/5/6, 800-63-4, PAM), taxonomía de actores internos (174, familias sectoriales con los roles fiscales SIN) y externos (21 secciones CAEB), **el motor BitMask Dual v2.1 íntegro** (la corrección del escalamiento silencioso `1 OR 2 = 3`; label vs one-hot; anatomía de 64 bits; herencia DAG + closure table — materializada: 1673 relaciones, profundidad 7), políticas mínimas y plantillas, verificación de coherencia de cifras (**la vigente es 548 en BD** — G-B01-03/04) y traslado fiel completo (§10, extracción literal de 2255 líneas). |
