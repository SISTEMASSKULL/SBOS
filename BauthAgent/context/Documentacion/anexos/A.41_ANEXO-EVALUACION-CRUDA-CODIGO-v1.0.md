...
| **BA7** | Migración `bauth_NN__idn_atributo.sql`: tablas `idn_identity_attribute` + `idn_tipo_atributo` (valores 1:N con clasificación y enmascaramiento) | SCIM RFC 7643 · ISO 24760-1 §6 | A.31-AT1 | ❌ **por desarrollar** — la tabla destino de los campos multivaluados del UserTemplate NO existe en el DDL |
...
(IAL + idn_identity_attribute) → BA14/BA17 (átomos + RLS) → BA8/BA13 (ADSIB + IGA) → BA5/BA9/BA16 → BA18-20.
