...
| Q3 | `logical.rs:191` — 1 referencia `TRYTON_CONTABILIDAD_NUEVO` en un test → **`cargo test` no compila** (el binario principal SÍ: `cargo check` exit 0) | **Código congelado** por orden del humano (10 de 11 referencias ya se habían corregido antes de la orden) | Al retomar código: rename 1 línea |
...
| Q9 | **7 métodos huérfanos**: `role_lifecycle.rs` (13 consultas BD) declara `bauth.role.{batch,bulk_assign,impact,lifecycle,rollback,search,temporal_assign}` pero NO hay `dispatcher.register` que los monte (grep en main.rs vacío) — el ciclo de vida de roles está escrito e inaccesible. También `domain_remaining.rs` sin registro | Código congelado | Al retomar código (**P1** — funcionalidad IGA núcleo; 9.02 §14/R2) |
...
| Q11 | **Cifra de superficie**: CLAUDE.md dice «47 handlers» y 9.01 decía «~115»; el conteo verificado es **≈141 métodos** (114 inline `dispatcher.register("` + 27 por lote en main.rs 600-630) | 9.01 ya corregido (2026-07-11); CLAUDE.md → HITL (amplía Q1) | 9.02 §2 |
