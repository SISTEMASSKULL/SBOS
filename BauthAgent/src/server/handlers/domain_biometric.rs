...
impl JsonRpcHandler for BiometricAtomsHandler {
    async fn handle(&self, _params: Value) -> Result<Value, JsonRpcError> {
        ...
        let rows: Vec<Row> = sqlx::query_as(
            "SELECT atom_slug, atom_name, app_code, group_code, verb_code, atom_position FROM bauth.privilege_atom WHERE domain_code = 5 ORDER BY atom_position"
        ).fetch_all(pg).await.map_err(|e| JsonRpcError { code: -32000, message: e.to_string(), data: None })?;
        Ok(serde_json::json!({"domain": "biometric", "domain_code": 5, "atoms": rows, "count": rows.len()}))
    }
}
